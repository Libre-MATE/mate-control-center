...
enum { CONFIG_CHANGED, NUM_SIGNALS };
...
static gboolean iter_for_face_foreach(GtkTreeModel *model, GtkTreePath *path,
                                      GtkTreeIter *iter, gpointer user_data) {
  IterForFaceData *data = user_data;
  gchar *font_name, *match_name;
  gboolean retval;

  gtk_tree_model_get(GTK_TREE_MODEL(model), iter, COLUMN_NAME, &font_name, -1);
...
gboolean font_view_model_get_iter_for_face(FontViewModel *self, FT_Face face,
  ...
  gtk_tree_model_foreach(GTK_TREE_MODEL(self), iter_for_face_foreach, data);

  found = data->found;
  if (found && iter) *iter = data->iter;
...
static void thumb_info_data_free(gpointer user_data) {
...
static GdkPixbuf *create_thumbnail(ThumbInfoData *thumb_info) {
  ...
  pixbuf = mate_desktop_thumbnail_factory_generate_thumbnail(
      factory, thumb_info->uri, g_file_info_get_content_type(info));

  if (pixbuf != NULL)
    mate_desktop_thumbnail_factory_save_thumbnail(
        factory, pixbuf, thumb_info->uri, (time_t)mtime);
  else
    mate_desktop_thumbnail_factory_create_failed_thumbnail(
        factory, thumb_info->uri, (time_t)mtime);
...
static void ensure_thumbnails_job(GTask *task, gpointer source_object,
                                  gpointer user_data,
                                  GCancellable *cancellable) {
  ...
  for (l = thumb_infos; l != NULL; l = l->next) {
    ...
    if (thumb_info->face_index == 0) {
      ...
      thumb_failed = g_file_info_get_attribute_boolean(
          info, G_FILE_ATTRIBUTE_THUMBNAILING_FAILED);
      if (thumb_failed) goto next;

      thumb_path = g_strdup(g_file_info_get_attribute_byte_string(
      ...
      thumb_info->uri =
          g_strdup_printf("%s#0x%08X", file_uri, thumb_info->face_index);
      g_free(file_uri);
      ...
      thumb_path = g_build_filename(g_get_user_cache_dir(), "thumbnails",
                                    "large", filename, NULL);
      g_free(filename);
    ...
    if (thumb_path != NULL) {
      ...
      thumb_info->pixbuf = gdk_pixbuf_new_from_stream_at_scale(
          G_INPUT_STREAM(is), 128, 128, TRUE, NULL, &error);
...
static void font_info_data_free(gpointer user_data) {
...
static void font_infos_loaded(GObject *source_object, GAsyncResult *result,
                              gpointer user_data) {
  ...
  for (l = font_infos; l != NULL; l = l->next) {
    ...
    gtk_list_store_insert_with_values(
        GTK_LIST_STORE(self), &iter, -1, COLUMN_NAME, font_info->font_name,
        COLUMN_PATH, font_info->font_path, COLUMN_FACE_INDEX,
        font_info->face_index, COLUMN_ICON, self->priv->fallback_icon,
        COLUMN_COLLATION_KEY, collation_key, -1);
...
static void load_font_infos(GTask *task, gpointer source_object,
...
static void ensure_font_list(FontViewModel *self) {
  FcPattern *pat;
  FcObjectSet *os;
  GTask *task;

  /* always reinitialize the font database */
  if (!FcInitReinitialize()) return;
  ...
  pat = FcPatternCreate();
  os =
      FcObjectSetBuild(FC_FILE, FC_INDEX, FC_FAMILY, FC_WEIGHT, FC_SLANT, NULL);
...
static gboolean ensure_font_list_idle(gpointer user_data) {
...
static int font_view_model_sort_func(GtkTreeModel *model, GtkTreeIter *a,
                                     GtkTreeIter *b, gpointer user_data) {
  gchar *key_a = NULL, *key_b = NULL;
  int retval;

  gtk_tree_model_get(model, a, COLUMN_COLLATION_KEY, &key_a, -1);
  gtk_tree_model_get(model, b, COLUMN_COLLATION_KEY, &key_b, -1);
...
static void create_file_monitors(FontViewModel *self) {
  FcConfig *config;
  FcStrList *str_list;
  FcChar8 *path;
  GFile *file;
  ...
  while ((path = FcStrListNext(str_list)) != NULL) {
    ...
    if (monitor != NULL) {
      ...
      g_signal_connect(monitor, "changed", G_CALLBACK(file_monitor_changed_cb),
                       self);
    }
...
static GdkPixbuf *get_fallback_icon(void) {
  ...
  icon_info = gtk_icon_theme_lookup_by_gicon(icon_theme, icon, 128, 0);
  g_object_unref(icon);
...
static void font_view_model_init(FontViewModel *self) {
  GType types[NUM_COLUMNS] = {G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT,
  ...
  gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(self), COLUMN_NAME,
                                       GTK_SORT_ASCENDING);
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(self), COLUMN_NAME,
                                  font_view_model_sort_func, NULL, NULL);
...
static void font_view_model_finalize(GObject *obj) {
...
static void font_view_model_class_init(FontViewModelClass *klass) {
  GObjectClass *oclass = G_OBJECT_CLASS(klass);
  oclass->finalize = font_view_model_finalize;

  signals[CONFIG_CHANGED] =
      g_signal_new("config-changed", FONT_VIEW_TYPE_MODEL, G_SIGNAL_RUN_FIRST,
