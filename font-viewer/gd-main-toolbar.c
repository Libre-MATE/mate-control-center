...
static void gd_main_toolbar_dispose(GObject *obj) {
...
static GtkWidget *get_empty_button(ChildType type) {
  GtkWidget *button;
...
static GtkWidget *get_symbolic_button(const gchar *icon_name, ChildType type) {
  GtkWidget *button, *w;
...
static GtkWidget *get_text_button(const gchar *label, ChildType type) {
  GtkWidget *button, *w;
...
static GtkSizeGroup *get_vertical_size_group(GdMainToolbar *self) {
  GtkSizeGroup *retval;
  GtkWidget *dummy;
...
void gd_main_toolbar_set_show_modes(GdMainToolbar *self, gboolean show_modes) {
  if (self->priv->show_modes == show_modes) return;
...
static void gd_main_toolbar_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
  ...
  switch (prop_id) {
    case PROP_SHOW_MODES:
      gd_main_toolbar_set_show_modes(GD_MAIN_TOOLBAR(self),
...
static void gd_main_toolbar_get_property(GObject *object, guint prop_id,
...
static void gd_main_toolbar_constructed(GObject *obj) {
  GdMainToolbar *self = GD_MAIN_TOOLBAR(obj);
  ...
  gtk_container_add(GTK_CONTAINER(self->priv->left_group),
                    self->priv->left_grid);
  ...
  gtk_label_set_ellipsize(GTK_LABEL(self->priv->detail_label),
                          PANGO_ELLIPSIZE_END);
  ...
  gtk_widget_set_no_show_all(self->priv->modes_box, TRUE);
  gtk_style_context_add_class(
      gtk_widget_get_style_context(self->priv->modes_box), "linked");
  ...
  gtk_container_add(GTK_CONTAINER(self->priv->right_group),
                    self->priv->right_grid);
  ...
  gtk_size_group_add_widget(self->priv->size_group,
                            GTK_WIDGET(self->priv->left_group));
  gtk_size_group_add_widget(self->priv->size_group,
                            GTK_WIDGET(self->priv->right_group));
}

static void gd_main_toolbar_init(GdMainToolbar *self) {
  self->priv = gd_main_toolbar_get_instance_private(self);
}

static void gd_main_toolbar_class_init(GdMainToolbarClass *klass) {
  GObjectClass *oclass;
  ...
  g_object_class_install_property(
      oclass, PROP_SHOW_MODES,
      g_param_spec_boolean("show-modes", "Show Modes", "Show Modes", FALSE,
                           G_PARAM_READWRITE));
}
...
static GtkWidget *add_button_internal(GdMainToolbar *self,
  ...
  if (icon_name != NULL) {
    ...
    if (label != NULL) gtk_widget_set_tooltip_text(button, label);
  } else if (label != NULL) {
...
void gd_main_toolbar_set_labels_menu(GdMainToolbar *self, GMenuModel *menu) {
  ...
  if (menu == NULL && ((gtk_widget_get_parent(self->priv->labels_grid) ==
                        self->priv->center_grid) ||
  ...
  if (menu != NULL) {
    ...
    gtk_style_context_add_class(
        gtk_widget_get_style_context(self->priv->center_menu),
        "selection-menu");
    gtk_widget_destroy(gtk_bin_get_child(GTK_BIN(button)));
