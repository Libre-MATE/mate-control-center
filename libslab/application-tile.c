...
static void application_tile_get_property(GObject *, guint, GValue *,
                                          GParamSpec *);
static void application_tile_set_property(GObject *, guint, const GValue *,
                                          GParamSpec *);
...
enum { PROP_0, PROP_APPLICATION_NAME, PROP_APPLICATION_DESCRIPTION };

G_DEFINE_TYPE_WITH_PRIVATE(ApplicationTile, application_tile,
                           NAMEPLATE_TILE_TYPE)
...
GtkWidget *application_tile_new_full(const gchar *desktop_item_id,
  ...
  if (!uri) {
    if (desktop_item) mate_desktop_item_unref(desktop_item);
...
static void application_tile_init(ApplicationTile *tile) {
...
static void application_tile_finalize(GObject *g_object) {
...
static void application_tile_get_property(GObject *g_obj, guint prop_id,
                                          GValue *value,
...
static void application_tile_set_property(GObject *g_obj, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *param_spec) {
  ApplicationTile *tile = APPLICATION_TILE(g_obj);
  ...
  switch (prop_id) {
    case PROP_APPLICATION_NAME:
      if (tile->name) g_free(tile->name);
      tile->name = g_strdup(g_value_get_string(value));
      break;

    case PROP_APPLICATION_DESCRIPTION:
      if (tile->description) g_free(tile->description);
...
static void application_tile_setup(ApplicationTile *this) {
  ...
  if (!priv->desktop_item) {
    ...
    if (!priv->desktop_item) return;
  }

  priv->image_id =
      g_strdup(mate_desktop_item_get_localestring(priv->desktop_item, "Icon"));
  image = themed_icon_new(priv->image_id, priv->image_size);

  gchar *filename = g_filename_from_uri(
      mate_desktop_item_get_location(priv->desktop_item), NULL, NULL);
  GKeyFile *keyfile = g_key_file_new();
  g_key_file_load_from_file(
      keyfile, filename,
      G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, NULL);

  name = g_key_file_get_locale_string(keyfile, G_KEY_FILE_DESKTOP_GROUP, "Name",
                                      NULL, NULL);
  desc = g_key_file_get_locale_string(keyfile, G_KEY_FILE_DESKTOP_GROUP,
  ...
  if (name) atk_object_set_name(accessible, name);
  if (desc) atk_object_set_description(accessible, desc);

  header = create_header(name);

  /*if no GenericName then just show and center the Name */
  if (desc && priv->show_generic_name && (!name || strcmp(name, desc) != 0))
  ...
  g_object_set(G_OBJECT(this), "nameplate-image", image, "nameplate-header",
               header, "nameplate-subheader", subheader, "context-menu",
               context_menu, "application-name", name,
  ...
  if (mate_desktop_item_get_string(priv->desktop_item, "DocPath")) {
    action =
        tile_action_new(TILE(this), help_trigger, _("Help"),
...
static GtkWidget *create_header(const gchar *name) {
  GtkWidget *header;
...
static GtkWidget *create_subheader(const gchar *desc) {
  GtkWidget *subheader;
  ...
  gtk_widget_modify_fg(
      subheader, GTK_STATE_NORMAL,
      &gtk_widget_get_style(subheader)->fg[GTK_STATE_INSENSITIVE]);

  return subheader;
}

static void start_trigger(Tile *tile, TileEvent *event, TileAction *action) {
  ApplicationTile *this = APPLICATION_TILE(tile);
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
  open_desktop_item_exec(priv->desktop_item);
}

static void help_trigger(Tile *tile, TileEvent *event, TileAction *action) {
  ApplicationTile *this = APPLICATION_TILE(tile);
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
  open_desktop_item_help(priv->desktop_item);
}

static void user_apps_trigger(Tile *tile, TileEvent *event,
                              TileAction *action) {
  ApplicationTile *this = APPLICATION_TILE(tile);
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
...
static void add_to_user_list(ApplicationTile *this) {
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
...
static void remove_from_user_list(ApplicationTile *this) {
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
...
static void startup_trigger(Tile *tile, TileEvent *event, TileAction *action) {
  ApplicationTile *this = APPLICATION_TILE(tile);
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
...
static void add_to_startup_list(ApplicationTile *this) {
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
  ...
  desktop_item_filename = g_filename_from_uri(
      mate_desktop_item_get_location(priv->desktop_item), NULL, NULL);
...
static void remove_from_startup_list(ApplicationTile *this) {
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
  ...
  ditem_filename = g_filename_from_uri(
      mate_desktop_item_get_location(priv->desktop_item), NULL, NULL);
  ...
  src_filename = g_build_filename(g_get_user_config_dir(), "autostart",
                                  ditem_basename, NULL);
...
static void update_user_list_menu_item(ApplicationTile *this) {
  ApplicationTilePrivate *priv = application_tile_get_instance_private(this);
  ...
  gtk_widget_set_sensitive(item,
                           (priv->agent_status != BOOKMARK_STORE_DEFAULT_ONLY));
}

static StartupStatus get_desktop_item_startup_status(
    MateDesktopItem *desktop_item) {
  gchar *filename;
  gchar *basename;
  ...
  filename = g_filename_from_uri(mate_desktop_item_get_location(desktop_item),
                                 NULL, NULL);
  if (!filename) return APP_NOT_ELIGIBLE;
  ...
  for (x = 0; global_dirs[x]; x++) {
    global_target =
        g_build_filename(global_dirs[x], "autostart", basename, NULL);
  ...
  if (retval != APP_NOT_ELIGIBLE) {
    ...
    for (x = 0; global_dirs[x]; x++) {
      global_target =
          g_build_filename(global_dirs[x], "mate", "autostart", basename, NULL);
      if (g_file_test(global_target, G_FILE_TEST_EXISTS)) {
        retval = APP_NOT_ELIGIBLE;
        g_free(global_target);
        break;
  ...
  if (retval != APP_NOT_ELIGIBLE) {
    user_target =
        g_build_filename(g_get_user_config_dir(), "autostart", basename, NULL);
...
static void update_startup_menu_item(ApplicationTile *this) {
  TileAction *action =
...
static void header_size_allocate_cb(GtkWidget *widget, GtkAllocation *alloc,
                                    gpointer user_data) {
  gtk_widget_set_size_request(widget, alloc->width, -1);
}

static void agent_notify_cb(GObject *g_obj, GParamSpec *pspec,
                            gpointer user_data) {
