...
gboolean theme_is_writable(const gpointer theme) {
  ...
  if (info == NULL || info->path == NULL) return FALSE;

  file = g_file_new_for_path(info->path);
  file_info = g_file_query_info(file, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE,
  ...
  if (!capplet_file_delete_recursive(dir, NULL)) {
    GtkWidget *info_dialog =
        gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
                               GTK_BUTTONS_OK, _("Theme cannot be deleted"));
...
gboolean packagekit_available(void) {
  ...
  variant = g_dbus_proxy_call_sync(
      proxy, "NameHasOwner", g_variant_new("(s)", "org.freedesktop.PackageKit"),
      G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
  if (variant == NULL) {
    g_warning(
        "Could not ask org.freedesktop.DBus if PackageKit is available: %s",
        error->message);
    g_error_free(error);
...
void theme_install_file(GtkWindow *parent, const gchar *path) {
  ...
  proxy = g_dbus_proxy_new_sync(connection, G_DBUS_PROXY_FLAGS_NONE, NULL,
                                ...
                                "/org/freedesktop/PackageKit",
                                "org.freedesktop.PackageKit", NULL, &error);
  if (proxy == NULL) {
  ...
  if (variant == NULL) {
    GtkWidget *dialog = gtk_message_dialog_new(
        ...
        GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, _("Could not install theme engine"));
