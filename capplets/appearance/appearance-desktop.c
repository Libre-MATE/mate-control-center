...
enum { TARGET_URI_LIST, TARGET_BGIMAGE };
...
static void select_item(AppearanceData *data, MateWPItem *item,
                        gboolean scroll) {
  GtkTreePath *path;
...
static void wp_props_load_wallpaper(gchar *key, MateWPItem *item,
                                    AppearanceData *data) {
  GtkTreeIter iter;
  GtkTreePath *path;
  GdkPixbuf *pixbuf;
  ...
  gtk_list_store_set(GTK_LIST_STORE(data->wp_model), &iter, 0, pixbuf, 1, item,
                     -1);
...
static void wp_option_menu_set(AppearanceData *data, int value,
                               gboolean shade_type) {
  if (shade_type) {
    gtk_combo_box_set_active(GTK_COMBO_BOX(data->wp_color_menu), value);
...
static void wp_set_sensitivities(AppearanceData *data) {
  ...
  if (item != NULL) filename = item->filename;
...
static void wp_shade_type_changed(GtkWidget *combobox, AppearanceData *data) {
  MateWPItem *item;
  GtkTreeIter iter;
  GdkPixbuf *pixbuf;
  ...
  item->shade_type =
      gtk_combo_box_get_active(GTK_COMBO_BOX(data->wp_color_menu));
...
static void wp_color_changed(AppearanceData *data, gboolean update) {
  MateWPItem *item;
  ...
  gtk_color_chooser_get_rgba(GTK_COLOR_CHOOSER(data->wp_pcpicker),
                             item->pcolor);
  gtk_color_chooser_get_rgba(GTK_COLOR_CHOOSER(data->wp_scpicker),
                             item->scolor);
...
static void wp_scolor_changed(GtkWidget *widget, AppearanceData *data) {
...
static void wp_uri_changed(const gchar *uri, AppearanceData *data) {
  ...
  if (selected != NULL && strcmp(selected->filename, uri) != 0) {
    if (item == NULL) item = wp_add_image(data, uri);

    if (item) select_item(data, item, TRUE);
  }
}

static void wp_file_changed(GSettings *settings, gchar *key,
                            AppearanceData *data) {
  gchar *uri;
  gchar *wpfile;
...
static void wp_options_changed(GSettings *settings, gchar *key,
                               AppearanceData *data) {
  MateWPItem *item;
...
static void wp_shading_changed(GSettings *settings, gchar *key,
                               AppearanceData *data) {
  MateWPItem *item;
...
static void wp_color1_changed(GSettings *settings, gchar *key,
                              AppearanceData *data) {
  GdkRGBA color;
  gchar *colorhex;
...
static void wp_color2_changed(GSettings *settings, gchar *key,
                              AppearanceData *data) {
  GdkRGBA color;
  gchar *colorhex;
...
static void wp_props_wp_selected(GtkTreeSelection *selection,
                                 AppearanceData *data) {
  MateWPItem *item;
...
static void wp_create_filechooser(AppearanceData *data) {
  const char *start_dir, *pictures = NULL;
  GtkFileFilter *filter;

  data->wp_filesel = GTK_FILE_CHOOSER(gtk_file_chooser_dialog_new(
      _("Add Wallpaper"), GET_WINDOW("appearance_window"),
      GTK_FILE_CHOOSER_ACTION_OPEN, "gtk-cancel", GTK_RESPONSE_CANCEL,
      "gtk-open", GTK_RESPONSE_OK, NULL));

  gtk_dialog_set_default_response(GTK_DIALOG(data->wp_filesel),
                                  GTK_RESPONSE_OK);
  ...
  if (g_file_test(BACKGROUND_DATADIR, G_FILE_TEST_IS_DIR)) {
    gtk_file_chooser_add_shortcut_folder(data->wp_filesel, BACKGROUND_DATADIR,
                                         NULL);
...
static void wp_file_open_dialog(GtkWidget *widget, AppearanceData *data) {
  ...
  if (!data->wp_filesel) wp_create_filechooser(data);
...
static void wp_drag_received(GtkWidget *widget, GdkDragContext *context, gint x,
                             gint y, GtkSelectionData *selection_data,
                             guint info, guint time, AppearanceData *data) {
...
static void wp_drag_get_data(GtkWidget *widget, GdkDragContext *context,
                             GtkSelectionData *selection_data, guint type,
                             guint time, AppearanceData *data) {
...
static gboolean wp_view_tooltip_cb(GtkWidget *widget, gint x, gint y,
                                   gboolean keyboard_mode, GtkTooltip *tooltip,
                                   AppearanceData *data) {
  GtkTreeIter iter;
  MateWPItem *item;

  if (gtk_icon_view_get_tooltip_context(data->wp_view, &x, &y, keyboard_mode,
                                        NULL, NULL, &iter)) {
...
static gint wp_list_sort(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                         AppearanceData *data) {
  ...
  if (uri) {
    ...
    if (mime_type) {
      pixbuf = mate_desktop_thumbnail_factory_generate_thumbnail(
          data->thumb_factory, uri, mime_type);
      g_free(mime_type);
    }
    ...
    if (pixbuf != NULL) {
      ...
      gtk_image_set_from_icon_name(GTK_IMAGE(data->wp_image), "dialog-question",
...
static gboolean reload_item(GtkTreeModel *model, GtkTreePath *path,
                            GtkTreeIter *iter, AppearanceData *data) {
  MateWPItem *item;
  GdkPixbuf *pixbuf;
...
static gdouble get_monitor_aspect_ratio_for_widget(GtkWidget *widget) {
  ...
  monitor = gdk_display_get_monitor_at_window(gtk_widget_get_display(widget),
...
static void reload_wallpapers(AppearanceData *data) {
  ...
  gtk_tree_model_foreach(data->wp_model, (GtkTreeModelForeachFunc)reload_item,
                         data);
}
...
static gboolean wp_load_stuffs(void *user_data) {
  ...
  g_hash_table_foreach(data->wp_hash, (GHFunc)wp_props_load_wallpaper, data);

  style = g_settings_get_string(data->wp_settings, WP_OPTIONS_KEY);
  if (style == NULL) style = g_strdup("none");
  ...
  if (item != NULL) {
    ...
    if (item) select_item(data, item, FALSE);
  }
...
static void wp_select_after_realize(GtkWidget *widget, AppearanceData *data) {
  ...
  item = get_selected_item(data, NULL);
  if (item == NULL) item = g_hash_table_lookup(data->wp_hash, "(none)");
...
static void create_button_images(AppearanceData *data) {
  ...
  pb = gtk_icon_set_render_icon(icon_set, style, GTK_TEXT_DIR_RTL,
                                GTK_STATE_NORMAL, GTK_ICON_SIZE_MENU, widget,
                                NULL);
  pb2 = gtk_icon_set_render_icon(icon_set, style, GTK_TEXT_DIR_LTR,
                                 GTK_STATE_NORMAL, GTK_ICON_SIZE_MENU, widget,
                                 NULL);
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (i > 0)
      gdk_pixbuf_composite(pb, pixbuf, 0, 0, w, h, 0, 0, 1, 1,
                           GDK_INTERP_NEAREST, 255);
    if (i < 2)
      gdk_pixbuf_composite(pb2, pixbuf, w, 0, w, h, w, 0, 1, 1,
                           GDK_INTERP_NEAREST, 255);
...
static void next_frame(AppearanceData *data, GtkCellRenderer *cr,
                       gint direction) {
  MateWPItem *item;
  GtkTreeIter iter;
  GdkPixbuf *pixbuf, *pb;
  ...
  if (frame >= 0)
    pixbuf = mate_wp_item_get_frame_thumbnail(item, data->thumb_factory,
                                              data->thumb_width,
                                              data->thumb_height, frame);
  if (pixbuf) {
    gtk_list_store_set(GTK_LIST_STORE(data->wp_model), &iter, 0, pixbuf, -1);
    g_object_unref(pixbuf);
    data->frame = frame;
  ...
  pb = buttons[1];
  if (direction < 0) {
    if (frame == 0) pb = buttons[0];
  } else {
    pixbuf = mate_wp_item_get_frame_thumbnail(item, data->thumb_factory,
                                              data->thumb_width,
                                              data->thumb_height, frame + 1);
    if (pixbuf)
      g_object_unref(pixbuf);
    else
      pb = buttons[2];
...
static gboolean wp_button_press_cb(GtkWidget *widget, GdkEventButton *event,
                                   AppearanceData *data) {
  ...
  if (gtk_icon_view_get_item_at_pos(GTK_ICON_VIEW(widget), button_event->x,
                                    button_event->y, NULL, &cell)) {
...
static void buttons_cell_data_func(GtkCellLayout *layout, GtkCellRenderer *cell,
...
void desktop_init(AppearanceData *data, const gchar **uris) {
  ...
  g_signal_connect(data->wp_settings, "changed::" WP_FILE_KEY,
                   G_CALLBACK(wp_file_changed), data);
  g_signal_connect(data->wp_settings, "changed::" WP_OPTIONS_KEY,
                   G_CALLBACK(wp_options_changed), data);
  g_signal_connect(data->wp_settings, "changed::" WP_SHADING_KEY,
                   G_CALLBACK(wp_shading_changed), data);
  g_signal_connect(data->wp_settings, "changed::" WP_PCOLOR_KEY,
                   G_CALLBACK(wp_color1_changed), data);
  g_signal_connect(data->wp_settings, "changed::" WP_SCOLOR_KEY,
                   G_CALLBACK(wp_color2_changed), data);

  data->wp_model =
      GTK_TREE_MODEL(gtk_list_store_new(2, GDK_TYPE_PIXBUF, G_TYPE_POINTER));
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(data->wp_view), cr, "pixbuf",
                                 0, NULL);

  cr = gtk_cell_renderer_pixbuf_new();
  create_button_images(data);
  g_object_set(cr, "mode", GTK_CELL_RENDERER_MODE_ACTIVATABLE, "pixbuf",
               buttons[0], NULL);
  ...
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(data->wp_model), 1,
                                  (GtkTreeIterCompareFunc)wp_list_sort, data,
                                  NULL);

  gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(data->wp_model), 1,
                                       GTK_SORT_ASCENDING);
  ...
  gtk_drag_source_set(GTK_WIDGET(data->wp_view), GDK_BUTTON1_MASK, drag_types,
                      G_N_ELEMENTS(drag_types), GDK_ACTION_COPY);
  g_signal_connect(data->wp_view, "drag-data-get", (GCallback)wp_drag_get_data,
                   data);
  ...
  g_signal_connect(data->wp_scpicker, "color-set", (GCallback)wp_scolor_changed,
                   data);

  data->wp_pcpicker = appearance_capplet_get_widget(data, "wp_pcpicker");

  g_signal_connect(data->wp_pcpicker, "color-set", (GCallback)wp_scolor_changed,
                   data);

  add_button = appearance_capplet_get_widget(data, "wp_add_button");

  g_signal_connect(add_button, "clicked", (GCallback)wp_file_open_dialog, data);
  ...
  data->screen_monitors_handler = g_signal_connect(
      gtk_widget_get_screen(GTK_WIDGET(data->wp_view)), "monitors-changed",
      G_CALLBACK(screen_monitors_changed), data);
  data->screen_size_handler = g_signal_connect(
      gtk_widget_get_screen(GTK_WIDGET(data->wp_view)), "size-changed",
      G_CALLBACK(screen_monitors_changed), data);
...
void desktop_shutdown(AppearanceData *data) {
  ...
  if (data->screen_monitors_handler > 0) {
    g_signal_handler_disconnect(
        gtk_widget_get_screen(GTK_WIDGET(data->wp_view)),
