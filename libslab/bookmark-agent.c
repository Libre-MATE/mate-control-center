...
enum { PROP_0, PROP_ITEMS, PROP_STATUS };
...
static inline gpointer bookmark_agent_get_instance_private(
    BookmarkAgent *this) {
  return (G_STRUCT_MEMBER_P(this, BookmarkAgent_private_offset));
}

GType bookmark_agent_get_type() {
  static GType g_define_type_id = 0;

  if (G_UNLIKELY(g_define_type_id == 0)) {
    static const GTypeInfo info = {sizeof(BookmarkAgentClass),
                                   (GBaseInitFunc)bookmark_agent_base_init,
                                   NULL,
                                   (GClassInitFunc)bookmark_agent_class_init,
                                   NULL,
                                   NULL,
                                   sizeof(BookmarkAgent),
                                   0,
                                   (GInstanceInitFunc)bookmark_agent_init,
                                   NULL};

    g_define_type_id =
        g_type_register_static(G_TYPE_OBJECT, "BookmarkAgent", &info, 0);
...
void bookmark_agent_add_item(BookmarkAgent *this, const BookmarkItem *item) {
  ...
  g_bookmark_file_add_application(priv->store, item->uri, item->app_name,
                                  item->app_exec);
...
void bookmark_agent_move_item(BookmarkAgent *this, const gchar *uri,
                              const gchar *uri_new) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);

  GError *error = NULL;

  if (!TYPE_IS_RECENT(priv->type)) return;

  gtk_recent_manager_move_item(gtk_recent_manager_get_default(), uri, uri_new,
                               &error);
  if (error) {
...
void bookmark_agent_purge_items(BookmarkAgent *this) {
  ...
  if (TYPE_IS_RECENT(priv->type)) {
    for (i = 0; i < uris_len; i++) {
      gtk_recent_manager_remove_item(gtk_recent_manager_get_default(), uris[i],
                                     &error);
      if (error) {
        g_warning("Unable to remove [%s] from %s: %s", priv->store_path,
...
void bookmark_agent_remove_item(BookmarkAgent *this, const gchar *uri) {
  ...
  if (!bookmark_agent_has_item(this, uri)) return;

  if (TYPE_IS_RECENT(priv->type)) {
    gtk_recent_manager_remove_item(gtk_recent_manager_get_default(), uri,
                                   &error);
    if (error) {
      g_warning("Unable to remove [%s] from %s: %s", priv->store_path, uri,
                error->message);
      g_error_free(error);
    }
  } else {
    ...
    if (rank >= 0) {
      ...
      for (i = 0; uris && uris[i]; ++i) {
        ...
        if (rank_i > rank) set_rank(this, uris[i], rank_i - 1);
      }
...
void bookmark_agent_reorder_items(BookmarkAgent *this, const gchar **uris) {
  ...
  for (i = 0; uris && uris[i]; ++i) set_rank(this, uris[i], i);
...
static GList *make_items_from_bookmark_file(BookmarkAgent *this,
                                            GBookmarkFile *store) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
  gchar **uris;
  gint i;
  ...
  for (i = 0; uris && uris[i]; ++i) {
    ...
    if (priv->type == BOOKMARK_STORE_RECENT_APPS)
      include =
          g_bookmark_file_has_group(store, uris[i], "recently-used-apps", NULL);
    else
...
#if GLIB_CHECK_VERSION(2, 66, 0)
      item->mtime =
          g_bookmark_file_get_modified_date_time(store, uris[i], NULL);
#else
...
void bookmark_agent_update_from_bookmark_file(BookmarkAgent *this,
                                              GBookmarkFile *store) {
...
static void bookmark_agent_base_init(BookmarkAgentClass *this_class) {
  gint i;

  for (i = 0; i < BOOKMARK_STORE_N_TYPES; ++i) instances[i] = NULL;
}

static void bookmark_agent_class_init(BookmarkAgentClass *this_class) {
  ...
  if (BookmarkAgent_private_offset != 0)
    g_type_class_adjust_private_offset(this_class,
                                       &BookmarkAgent_private_offset);
  ...
  items_pspec =
      g_param_spec_pointer(BOOKMARK_AGENT_ITEMS_PROP, BOOKMARK_AGENT_ITEMS_PROP,
                           "the null-terminated list which contains the "
                           "bookmark items in this store",
                           G_PARAM_READABLE | G_PARAM_STATIC_NAME |
                               G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB);

  status_pspec = g_param_spec_int(
      BOOKMARK_AGENT_STORE_STATUS_PROP, BOOKMARK_AGENT_STORE_STATUS_PROP,
      "the status of the store", BOOKMARK_STORE_DEFAULT_ONLY,
      BOOKMARK_STORE_USER, BOOKMARK_STORE_DEFAULT,
...
static void bookmark_agent_init(BookmarkAgent *this) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
...
static BookmarkAgent *bookmark_agent_new(BookmarkStoreType type) {
  BookmarkAgent *this;
  BookmarkAgentPrivate *priv;
  ...
  switch (type) {
    ...
    case BOOKMARK_STORE_USER_DIRS:
      ...
      priv->gtk_store_path = g_build_filename(
          g_get_user_config_dir(), "gtk-3.0", GTK_BOOKMARKS_FILE, NULL);
      gtk_store_file = g_file_new_for_path(priv->gtk_store_path);
      priv->gtk_store_monitor =
          g_file_monitor_file(gtk_store_file, 0, NULL, NULL);
    ...
    case BOOKMARK_STORE_RECENT_DOCS:
      ...
      priv->store_path =
          g_build_filename(g_get_user_data_dir(), "recently-used.xbel", NULL);
  ...
  if (type == BOOKMARK_STORE_USER_APPS || type == BOOKMARK_STORE_USER_DOCS ||
    ...
    priv->user_store_path = g_build_filename(g_get_user_data_dir(), PACKAGE,
                                             priv->store_filename, NULL);
...
BookmarkAgent *bookmark_agent_get_instance(BookmarkStoreType type) {
  ...
  if (!instances[type]) {
    ...
    g_object_weak_ref(G_OBJECT(instances[type]), weak_destroy_cb,
...
static void get_property(GObject *g_obj, guint prop_id, GValue *value,
                         GParamSpec *pspec) {
...
static void set_property(GObject *g_obj, guint prop_id, const GValue *value,
                         GParamSpec *pspec) {
  /* no writeable properties */
}

static void finalize(GObject *g_obj) {
  BookmarkAgent *this = BOOKMARK_AGENT(g_obj);
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
  ...
  if (priv->store_monitor) {
    g_signal_handlers_disconnect_by_func(priv->store_monitor, store_monitor_cb,
                                         this);
  ...
  if (priv->user_store_monitor) {
    g_signal_handlers_disconnect_by_func(priv->user_store_monitor,
                                         store_monitor_cb, this);
    g_file_monitor_cancel(priv->user_store_monitor);
    g_object_unref(priv->user_store_monitor);
  }

  if (priv->gtk_store_monitor) {
    g_signal_handlers_disconnect_by_func(priv->gtk_store_monitor,
                                         store_monitor_cb, this);
...
static void update_agent(BookmarkAgent *this) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);

  if (priv->update_path) priv->update_path(this);

  if (priv->load_store) priv->load_store(this);

  update_items(this);
}

static void update_items(BookmarkAgent *this) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
  ...
  for (i = 0; uris && uris[i]; ++i) {
    ...
    if (uris_ordered[rank]) {
      ...
      for (rank = 0; rank < n_uris; ++rank)
        if (!uris_ordered[rank]) break;

      g_warning(
          "store corruption [%s] - multiple uris with same rank (%d): [%s] "
          "[%s], moving latter to %d",
  ...
  if (priv->n_items != n_uris) needs_update = TRUE;
  ...
  for (i = 0; !needs_update && uris_ordered && uris_ordered[i]; ++i) {
    if (priv->type == BOOKMARK_STORE_USER_DIRS) {
      ...
      if (!new_title && !old_title) {
        if (strcmp(priv->items[i]->uri, uris_ordered[i])) needs_update = TRUE;
  ...
  if (needs_update) {
    ...
    for (i = 0; uris_ordered && uris_ordered[i]; ++i) {
      ...
      priv->items[i]->title =
          g_bookmark_file_get_title(priv->store, uris_ordered[i], NULL);
      priv->items[i]->mime_type =
          g_bookmark_file_get_mime_type(priv->store, uris_ordered[i], NULL);
#if GLIB_CHECK_VERSION(2, 66, 0)
      priv->items[i]->mtime = g_bookmark_file_get_modified_date_time(
          priv->store, uris_ordered[i], NULL);
#else
      priv->items[i]->mtime =
          g_bookmark_file_get_modified(priv->store, uris_ordered[i], NULL);
#endif
      priv->items[i]->app_name = NULL;
      priv->items[i]->app_exec = NULL;

      g_bookmark_file_get_icon(priv->store, uris_ordered[i],
                               &priv->items[i]->icon, NULL, NULL);
    }

    /* Since the bookmark store for recently-used items is updated by the caller
     * of BookmarkAgent, we don't emit notifications in that case.  The caller
     * will know when to update itself.
     */
    if (!TYPE_IS_RECENT(priv->type))
      g_object_notify(G_OBJECT(this), BOOKMARK_AGENT_ITEMS_PROP);
  }

  if (store_corrupted) save_store(this);

  g_strfreev(uris);
  g_free(uris_ordered);
}

static void save_store(BookmarkAgent *this) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
...
static gint get_rank(BookmarkAgent *this, const gchar *uri) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
  ...
  for (i = 0; groups && groups[i]; ++i) {
    if (g_str_has_prefix(groups[i], "rank-")) {
      if (rank >= 0)
        g_warning("store corruption - multiple ranks for same uri: [%s] [%s]",
                  priv->store_path, uri);
...
static void set_rank(BookmarkAgent *this, const gchar *uri, gint rank) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
  ...
  if (!(priv->reorderable && bookmark_agent_has_item(this, uri))) return;
...
static void load_xbel_store(BookmarkAgent *this) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
  ...
  if (!success) {
    ...
    if (error) {
      g_debug("Couldn't load bookmark file [%s]: %s", priv->store_path,
              error->message);
      g_error_free(error);
    } else {
  ...
  for (i = 0; uris && uris[i]; ++i) priv->create_item(this, uris[i]);

  g_strfreev(uris);
}

static void load_places_store(BookmarkAgent *this) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
...
static void update_user_spec_path(BookmarkAgent *this) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
  ...
  use_user_path = priv->user_modifiable &&
                  (priv->needs_sync ||
                   g_file_test(priv->user_store_path, G_FILE_TEST_EXISTS));
  ...
  if (priv->status != status) {
    ...
    if (priv->status == BOOKMARK_STORE_DEFAULT) {
      ...
      priv->user_store_monitor =
          g_file_monitor_file(user_store_file, 0, NULL, NULL);
      if (priv->user_store_monitor) {
  ...
  if (g_strcmp0(priv->store_path, path)) {
    ...
    if (priv->store_path) {
      ...
      priv->store_monitor = g_file_monitor_file(store_file, 0, NULL, NULL);
      if (priv->store_monitor) {
...
static void save_xbel_store(BookmarkAgent *this) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);

  GError *error = NULL;

  if (g_bookmark_file_to_file(priv->store, priv->store_path, &error)) return;

  if (error) {
    g_warning("Couldn't save bookmark file [%s]: %s", priv->store_path,
              error->message);
    g_error_free(error);
  } else {
...
static void create_app_item(BookmarkAgent *this, const gchar *uri) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
...
static void create_doc_item(BookmarkAgent *this, const gchar *uri) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);

  gchar *uri_new = NULL;

  if ((strcmp(uri, "BLANK_SPREADSHEET") == 0) ||
      (strcmp(uri, "BLANK_DOCUMENT") == 0)) {
    gchar *template = NULL;
    gchar *file;

    gchar *dir = g_strdup(g_get_user_special_dir(G_USER_DIRECTORY_DOCUMENTS));
    if (!dir) dir = g_build_filename(g_get_home_dir(), "Documents", NULL);
...
static void create_dir_item(BookmarkAgent *this, const gchar *uri) {
  BookmarkAgentPrivate *priv = bookmark_agent_get_instance_private(this);
  ...
  if (strcmp(uri, "HOME") == 0) {
    ...
    if (!path) path = g_build_filename(g_get_home_dir(), "Documents", NULL);
    name = _("Documents");
    uri_new = g_filename_to_uri(path, NULL, NULL);
  } else if (strcmp(uri, "DESKTOP") == 0) {
    path = g_strdup(g_get_user_special_dir(G_USER_DIRECTORY_DESKTOP));
    if (!path) path = g_build_filename(g_get_home_dir(), "Desktop", NULL);
    name = _("Desktop");
    uri_new = g_filename_to_uri(path, NULL, NULL);
    icon = "user-desktop";
    ...
    path = g_build_filename(g_get_user_data_dir(), "caja", "searches", &uri[21],
                            NULL);
  ...
  if (icon) g_bookmark_file_set_icon(priv->store, uri, icon, "image/png");

  if (name) g_bookmark_file_set_title(priv->store, uri, name);
...
static void store_monitor_cb(GFileMonitor *mon, GFile *f1, GFile *f2,
                             GFileMonitorEvent event_type, gpointer user_data) {
  update_agent(BOOKMARK_AGENT(user_data));
}

static void weak_destroy_cb(gpointer data, GObject *g_obj) {
