...
static AppearanceData *init_appearance_data(int *argc, char ***argv,
  ...
  /* set up the data */
  ui = gtk_builder_new_from_resource(
      "/org/mate/mcc/appearance/data/appearance.ui");
...
static void main_window_response(GtkWidget *widget, gint response_id,
                                 AppearanceData *data) {
  if (response_id == GTK_RESPONSE_CLOSE ||
    ...
    if (data->caja_settings) g_object_unref(data->caja_settings);
...
int main(int argc, char **argv) {
  ...
  GOptionEntry option_entries[] = {
      {"install-theme", 'i', G_OPTION_FLAG_IN_MAIN, G_OPTION_ARG_FILENAME,
       &install_filename, N_("Specify the filename of a theme to install"),
       N_("filename")},
      ...
      {G_OPTION_REMAINING, 0, G_OPTION_FLAG_IN_MAIN,
       G_OPTION_ARG_FILENAME_ARRAY, &wallpaper_files, NULL,
       N_("[WALLPAPER...]")},
  ...
  g_signal_connect_after(w, "response", (GCallback)main_window_response, data);
  ...
  if (start_page != NULL) {
    ...
    if (w != NULL) {
      ...
      if (pindex != -1) gtk_notebook_set_current_page(notebook, pindex);
    }
    g_free(page_name);
  }
