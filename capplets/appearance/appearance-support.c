...
static gboolean is_program_in_path(const char *program) {
...
static gboolean metacity_is_running(void) {
...
static void metacity_theme_apply(const gchar *theme, const gchar *font) {
  /* set theme, we use gconf and gsettings binaries to avoid schemas and
   * versions issues */
  if (is_program_in_path("gconftool-2")) {
    gchar *gconf_cmd = NULL;

    gconf_cmd = g_strdup_printf(
        "gconftool-2 --set --type string /apps/metacity/general/theme '%s'",
        theme);
    g_spawn_command_line_async(gconf_cmd, NULL);
    g_free(gconf_cmd);

    gconf_cmd = g_strdup_printf(
        "gconftool-2 --set --type string /apps/metacity/general/titlebar_font "
        "'%s'",
        font);
    g_spawn_command_line_async(gconf_cmd, NULL);
    g_free(gconf_cmd);
  ...
  if (is_program_in_path("gsettings")) {
    ...
    /* for GNOME3 */
    gsettings_cmd = g_strdup_printf(
        "gsettings set org.gnome.desktop.wm.preferences theme '%s'", theme);
    ...
    /* for metacity >= 3.16 */
    gsettings_cmd =
        g_strdup_printf("gsettings set org.gnome.metacity theme '%s'", theme);
    g_spawn_command_line_async(gsettings_cmd, NULL);
    g_free(gsettings_cmd);

    /* for metacity >= 3.20 */
    gsettings_cmd = g_strdup_printf(
        "gsettings set org.gnome.metacity.theme name '%s'", theme);
    g_spawn_command_line_async(gsettings_cmd, NULL);
    g_free(gsettings_cmd);
  }
}

static void marco_theme_changed(GSettings *settings, gchar *key,
                                AppearanceData *data) {
  gchar *theme = NULL;
  gchar *font = NULL;
...
void support_shutdown(AppearanceData *data) { /* nothing to do */ }
