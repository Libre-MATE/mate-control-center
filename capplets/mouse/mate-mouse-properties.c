...
static gboolean test_maybe_timeout(struct test_data_t *data) {
...
static gboolean event_box_button_press_event(GtkWidget *widget,
  ...
  switch (double_click_state) {
    case DOUBLE_CLICK_TEST_OFF:
      ...
      test_maybe_timeout_id = g_timeout_add(
          double_click_time, (GSourceFunc)test_maybe_timeout, &data);
      break;
    case DOUBLE_CLICK_TEST_MAYBE:
      if (event->time - double_click_timestamp < double_click_time) {
        ...
        test_on_timeout_id =
            g_timeout_add(2500, (GSourceFunc)test_maybe_timeout, &data);
      }
      break;
  ...
  switch (double_click_state) {
    case DOUBLE_CLICK_TEST_ON:
      gtk_image_set_from_resource(GTK_IMAGE(image),
                                  "/org/mate/mcc/mouse/double-click-on.svg");
      break;
    case DOUBLE_CLICK_TEST_MAYBE:
      gtk_image_set_from_resource(GTK_IMAGE(image),
                                  "/org/mate/mcc/mouse/double-click-maybe.svg");
      break;
    case DOUBLE_CLICK_TEST_OFF:
      gtk_image_set_from_resource(GTK_IMAGE(image),
                                  "/org/mate/mcc/mouse/double-click-off.svg");
...
static void orientation_radio_button_release_event(GtkWidget *widget,
                                                   GdkEventButton *event) {
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), TRUE);
}

static void orientation_radio_button_toggled(GtkToggleButton *togglebutton,
                                             GtkBuilder *dialog) {
  gboolean left_handed;
  left_handed = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(gtk_builder_get_object(dialog, "left_handed_radio")));
...
static void synaptics_check_capabilities(GtkBuilder *dialog) {
  ...
  prop = XInternAtom(GDK_DISPLAY_XDISPLAY(display), "Synaptics Capabilities",
                     True);
  if (!prop) return;

  devicelist = XListInputDevices(GDK_DISPLAY_XDISPLAY(display), &numdevices);
  for (i = 0; i < numdevices; i++) {
    if (devicelist[i].use != IsXExtensionPointer) continue;

    gdk_x11_display_error_trap_push(display);
    XDevice *device =
        XOpenDevice(GDK_DISPLAY_XDISPLAY(display), devicelist[i].id);
    if (gdk_x11_display_error_trap_pop(display)) continue;

    gdk_x11_display_error_trap_push(display);
    if ((XGetDeviceProperty(GDK_DISPLAY_XDISPLAY(display), device, prop, 0, 2,
                            False, XA_INTEGER, &realtype, &realformat, &nitems,
...
static void accel_profile_combobox_changed_callback(GtkWidget *combobox,
                                                    void *data) {
...
static void comboxbox_changed(GtkWidget *combobox, GtkBuilder *dialog,
                              const char *key) {
  gint value = gtk_combo_box_get_active(GTK_COMBO_BOX(combobox));
  gint value2, value3;
  GtkLabel *warn =
      GTK_LABEL(gtk_builder_get_object(dialog, "multi_finger_warning"));
...
static void comboxbox_two_finger_changed_callback(GtkWidget *combobox,
                                                  void *data) {
  comboxbox_changed(combobox, GTK_BUILDER(data), "two-finger-click");
}

static void comboxbox_three_finger_changed_callback(GtkWidget *combobox,
                                                    void *data) {
...
static void setup_dialog(GtkBuilder *dialog) {
  ...
  /* explicitly connect to button-release so that you can change orientation
   * with either button */
  ...
  g_settings_bind(mouse_settings, "middle-button-enabled",
                  WID("middle_button_emulation_toggle"), "active",
                  G_SETTINGS_BIND_DEFAULT);

  /* Middle Button Paste */
  g_settings_bind(interface_settings, "gtk-enable-primary-paste",
                  WID("middle_button_paste_toggle"), "active",
                  G_SETTINGS_BIND_DEFAULT);

  /* Double-click time */
  g_settings_bind(mouse_settings, DOUBLE_CLICK_KEY,
                  gtk_range_get_adjustment(
                      GTK_RANGE(gtk_builder_get_object(dialog, "delay_scale"))),
                  "value", G_SETTINGS_BIND_DEFAULT);

  gtk_image_set_from_resource(
      GTK_IMAGE(gtk_builder_get_object(dialog, "double_click_image")),
      "/org/mate/mcc/mouse/double-click-off.svg");
  g_object_set_data(gtk_builder_get_object(dialog, "double_click_eventbox"),
                    "image", WID("double_click_image"));
  g_signal_connect(gtk_builder_get_object(dialog, "double_click_eventbox"),
                   "button_press_event",
  ...
  g_settings_bind(mouse_settings, "motion-acceleration",
                  gtk_range_get_adjustment(
                      GTK_RANGE(gtk_builder_get_object(dialog, "accel_scale"))),
                  "value", G_SETTINGS_BIND_DEFAULT);
  g_settings_bind(mouse_settings, "motion-threshold",
                  gtk_range_get_adjustment(GTK_RANGE(
                      gtk_builder_get_object(dialog, "sensitivity_scale"))),
                  "value", G_SETTINGS_BIND_DEFAULT);

  g_signal_connect(WID("mouse_accel_profile"), "changed",
                   G_CALLBACK(accel_profile_combobox_changed_callback), NULL);
  gtk_combo_box_set_active(
      GTK_COMBO_BOX(gtk_builder_get_object(dialog, "mouse_accel_profile")),
  ...
  if (touchpad_is_present() == FALSE)
    gtk_notebook_remove_page(
        GTK_NOTEBOOK(gtk_builder_get_object(dialog, "prefs_widget")), -1);
  else {
    g_settings_bind(touchpad_settings, "touchpad-enabled",
                    gtk_builder_get_object(dialog, "touchpad_enable"), "active",
                    G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(touchpad_settings, "touchpad-enabled",
                    gtk_builder_get_object(dialog, "vbox_touchpad_general"),
                    "sensitive", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(touchpad_settings, "touchpad-enabled",
                    gtk_builder_get_object(dialog, "vbox_touchpad_scrolling"),
                    "sensitive", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(
        touchpad_settings, "touchpad-enabled",
        gtk_builder_get_object(dialog, "vbox_touchpad_pointer_speed"),
        "sensitive", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(touchpad_settings, "disable-while-typing",
                    gtk_builder_get_object(dialog, "disable_w_typing_toggle"),
                    "active", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(touchpad_settings, "tap-to-click",
                    gtk_builder_get_object(dialog, "tap_to_click_toggle"),
                    "active", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(touchpad_settings, "vertical-edge-scrolling",
                    gtk_builder_get_object(dialog, "vert_edge_scroll_toggle"),
                    "active", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(touchpad_settings, "horizontal-edge-scrolling",
                    gtk_builder_get_object(dialog, "horiz_edge_scroll_toggle"),
                    "active", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(
        touchpad_settings, "vertical-two-finger-scrolling",
        gtk_builder_get_object(dialog, "vert_twofinger_scroll_toggle"),
        "active", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(
        touchpad_settings, "horizontal-two-finger-scrolling",
        gtk_builder_get_object(dialog, "horiz_twofinger_scroll_toggle"),
        "active", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(touchpad_settings, "natural-scroll",
                    gtk_builder_get_object(dialog, "natural_scroll_toggle"),
                    "active", G_SETTINGS_BIND_DEFAULT);

    char *emulation_values[] = {_("Disabled"), _("Left button"),
                                _("Middle button"), _("Right button")};

    GtkWidget *two_click_comboxbox = gtk_combo_box_text_new();
    GtkWidget *three_click_comboxbox = gtk_combo_box_text_new();
    gtk_box_pack_start(
        GTK_BOX(gtk_builder_get_object(dialog, "hbox_two_finger_click")),
        two_click_comboxbox, FALSE, FALSE, 6);
    gtk_box_pack_start(
        GTK_BOX(gtk_builder_get_object(dialog, "hbox_three_finger_click")),
        three_click_comboxbox, FALSE, FALSE, 6);
    int i;
    for (i = 0; i < 4; i++) {
      gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(two_click_comboxbox),
                                     emulation_values[i]);
      gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(three_click_comboxbox),
                                     emulation_values[i]);
    }

    g_signal_connect(two_click_comboxbox, "changed",
                     G_CALLBACK(comboxbox_two_finger_changed_callback), dialog);
    g_signal_connect(three_click_comboxbox, "changed",
                     G_CALLBACK(comboxbox_three_finger_changed_callback),
                     dialog);
    gtk_combo_box_set_active(
        GTK_COMBO_BOX(two_click_comboxbox),
        g_settings_get_int(touchpad_settings, "two-finger-click"));
    gtk_combo_box_set_active(
        GTK_COMBO_BOX(three_click_comboxbox),
        g_settings_get_int(touchpad_settings, "three-finger-click"));
    ...
    g_settings_bind(touchpad_settings, "motion-acceleration",
                    gtk_range_get_adjustment(GTK_RANGE(gtk_builder_get_object(
                        dialog, "touchpad_accel_scale"))),
                    "value", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(touchpad_settings, "motion-threshold",
                    gtk_range_get_adjustment(GTK_RANGE(gtk_builder_get_object(
                        dialog, "touchpad_sensitivity_scale"))),
                    "value", G_SETTINGS_BIND_DEFAULT);
...
int main(int argc, char **argv) {
  ...
  GOptionEntry cap_options[] = {
      {"show-page", 'p', G_OPTION_FLAG_IN_MAIN, G_OPTION_ARG_STRING,
       ...
       N_("Specify the name of the page to show (general)"), N_("page")},
  ...
  dialog = gtk_builder_new_from_resource(
      "/org/mate/mcc/mouse/mate-mouse-properties.ui");

  setup_dialog(dialog);

  dialog_win = WID("mouse_properties_dialog");
  g_signal_connect(dialog_win, "response", G_CALLBACK(dialog_response_cb),
                   NULL);

  GtkNotebook *nb =
      GTK_NOTEBOOK(gtk_builder_get_object(dialog, "prefs_widget"));
  ...
  if (start_page != NULL) {
    ...
    if (w != NULL) {
      ...
      if (pindex != -1) gtk_notebook_set_current_page(nb, pindex);
    }
    g_free(page_name);
  }
