...
static GdkPixbuf *create_tile_pixbuf(GdkPixbuf *dest_pixbuf,
                                     ...
                                     GdkRectangle *field_geom, guint alpha,
                                     GdkColor *bg_color) {
  ...
  if (dest_pixbuf == NULL)
    dest_pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8,
                                 field_geom->width, field_geom->height);

  if (need_composite && use_simple)
    colorv = ((bg_color->red & 0xff00) << 8) | (bg_color->green & 0xff00) |
    ...
    for (cx = 0; cx < field_geom->width; cx += pwidth) {
      if (need_composite && !use_simple)
        gdk_pixbuf_composite(src_pixbuf, dest_pixbuf, cx, cy,
                             ...
                             MIN(pheight, field_geom->height - cy), cx, cy, 1.0,
                             1.0, GDK_INTERP_BILINEAR, alpha);
      else if (need_composite && use_simple)
        gdk_pixbuf_composite_color(src_pixbuf, dest_pixbuf, cx, cy,
                                   MIN(pwidth, field_geom->width - cx),
                                   MIN(pheight, field_geom->height - cy), cx,
                                   cy, 1.0, 1.0, GDK_INTERP_BILINEAR, alpha,
                                   65536, 65536, 65536, colorv, colorv);
      else
        gdk_pixbuf_copy_area(
            src_pixbuf, 0, 0, MIN(pwidth, field_geom->width - cx),
            MIN(pheight, field_geom->height - cy), dest_pixbuf, cx, cy);
    }
  }
...
static gboolean window_draw_event(GtkWidget *widget, cairo_t *cr,
                                  gpointer data) {
  int width;
  int height;
...
static void set_pixmap_background(GtkWidget *window) {
  GdkScreen *screen;
  ...
  tmp_pixbuf = gdk_pixbuf_get_from_window(gdk_screen_get_root_window(screen), 0,
                                          0, width, height);
  ...
  tile_pixbuf = create_tile_pixbuf(NULL, pixbuf, &rect, 155, &color);

  g_object_unref(pixbuf);

  gdk_pixbuf_composite(tile_pixbuf, tmp_pixbuf, 0, 0, width, height, 0, 0,
                       scale, scale, GDK_INTERP_NEAREST, 225);
