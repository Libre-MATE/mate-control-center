...
static gboolean device_has_property(XDevice *device,
  ...
  if ((XGetDeviceProperty(GDK_DISPLAY_XDISPLAY(display), device, prop, 0, 1,
...
XDevice *device_is_touchpad(XDeviceInfo *deviceinfo) {
  ...
  display = gdk_display_get_default();
  if (deviceinfo->type !=
      XInternAtom(GDK_DISPLAY_XDISPLAY(display), XI_TOUCHPAD, True))
    return NULL;

  gdk_x11_display_error_trap_push(display);
  device = XOpenDevice(GDK_DISPLAY_XDISPLAY(display), deviceinfo->id);
  if (gdk_x11_display_error_trap_pop(display) || (device == NULL)) return NULL;
...
gboolean touchpad_is_present(void) {
  ...
  device_info = XListInputDevices(
      GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), &n_devices);
