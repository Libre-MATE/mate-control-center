...
gboolean load_image_by_id(GtkImage *image, GtkIconSize size,
  ...
  if (g_path_is_absolute(id)) {
    ...
    pixbuf = gdk_pixbuf_new_from_file_at_size(id, width * scale_factor,
                                              height * scale_factor, NULL);
    ...
    if (/* file extensions are not copesetic with loading by "name" */
        g_str_has_suffix(id, ".png") || g_str_has_suffix(id, ".svg") ||
    ...
    surface =
        gtk_icon_theme_load_surface(icon_theme, id, width, scale_factor, NULL,
