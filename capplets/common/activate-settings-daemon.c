...
static void popup_error_message(void) {
  ...
  dialog = gtk_message_dialog_new(
      ...
      _("Unable to start the settings manager 'mate-settings-daemon'.\n"
        ...
        "take effect. This could "
        "indicate a problem with DBus, or a non-MATE (e.g. KDE) settings "
        "manager may already "
...
gboolean activate_settings_daemon(void) {
  ...
  proxy = g_dbus_proxy_new_for_bus_sync(
      ...
      "org.mate.SettingsDaemon", "/org/mate/SettingsDaemon",
      "org.mate.SettingsDaemon", NULL, &error);
  if (proxy == NULL) {
    popup_error_message();
    g_error_free(error);
    return FALSE;
  ...
  ret = g_dbus_proxy_call_sync(proxy, "Awake", g_variant_new("()"),
