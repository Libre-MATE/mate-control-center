...
static GdkFilterReturn drw_selection_filter(GdkXEvent *xevent, GdkEvent *event,
                                            gpointer data);
static void drw_selection_negotiate(DrwSelection *drw_selection);

static void drw_selection_reset(DrwSelection *drw_selection) {
  if (drw_selection->owner_window) {
    gdk_window_remove_filter(drw_selection->owner_window, drw_selection_filter,
                             drw_selection);
...
static void drw_selection_clear(GtkWidget *widget, GdkEventSelection *event,
                                gpointer user_data) {
...
static gboolean drw_selection_find_existing(DrwSelection *drw_selection) {
  GdkDisplay *display;
  Window old;
  Display *xdisplay;
  ...
  old = XGetSelectionOwner(xdisplay, gdk_x11_get_xatom_by_name(SELECTION_NAME));
  if (old) {
    XSelectInput(xdisplay, old, StructureNotifyMask);
    drw_selection->owner_window =
        gdk_x11_window_foreign_new_for_display(gdk_display_get_default(), old);
  }
  XSync(xdisplay, False);

  if (gdk_x11_display_error_trap_pop(display) == 0 &&
      drw_selection->owner_window) {
    gdk_window_add_filter(drw_selection->owner_window, drw_selection_filter,
                          drw_selection);
...
static gboolean drw_selection_claim(DrwSelection *drw_selection) {
