...
enum { INVALID_THEME_NAME };

/* taken from mate-desktop-item.c */
static gchar *escape_string_and_dup(const gchar *s) {
...
static gboolean check_theme_name(const gchar *theme_name, GError **error) {
  if (theme_name == NULL) {
    g_set_error(error, error_quark, INVALID_THEME_NAME,
...
static gchar *str_remove_slash(const gchar *src) {
  const gchar *i;
  gchar *rtn;
  gint len = 0;
  i = src;
...
static gboolean setup_directory_structure(const gchar *theme_name,
                                          GError **error) {
  ...
  if (!g_file_test(dir, G_FILE_TEST_EXISTS)) g_mkdir(dir, 0775);
  g_free(dir);
  ...
  dir = g_build_filename(g_get_home_dir(), ".themes", theme_name_dir,
                         "index.theme", NULL);
  ...
  if (g_file_test(dir, G_FILE_TEST_EXISTS)) {
    ...
    dialog = (GtkDialog *)gtk_message_dialog_new(
        NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_QUESTION, GTK_BUTTONS_CANCEL,
        _("The theme already exists. Would you like to replace it?"));
    button =
        gtk_dialog_add_button(dialog, _("_Overwrite"), GTK_RESPONSE_ACCEPT);
...
static gboolean write_theme_to_disk(MateThemeMetaInfo *theme_info,
  ...
  str = g_strdup_printf(
      theme_header, theme_name, theme_description, theme_info->gtk_theme_name,
      theme_info->marco_theme_name, theme_info->icon_theme_name);

  output = G_OUTPUT_STREAM(
      g_file_replace(tmp_file, NULL, FALSE, G_FILE_CREATE_NONE, NULL, NULL));
  ...
  if (theme_info->cursor_theme_name) {
    str = g_strdup_printf(
        "CursorTheme=%s\n"
        "CursorSize=%i\n",
        theme_info->cursor_theme_name, theme_info->cursor_size);
    g_output_stream_write(output, str, strlen(str), NULL, NULL);
    g_free(str);
  }
  ...
  g_file_move(tmp_file, target_file, G_FILE_COPY_OVERWRITE, NULL, NULL, NULL,
              NULL);
...
static gboolean save_theme_to_disk(MateThemeMetaInfo *theme_info,
                                   const gchar *theme_name,
                                   const gchar *theme_description,
                                   gboolean save_background,
                                   gboolean save_notification, GError **error) {
  if (!check_theme_name(theme_name, error)) return FALSE;

  if (!setup_directory_structure(theme_name, error)) return FALSE;

  if (!write_theme_to_disk(theme_info, theme_name, theme_description,
                           save_background, save_notification, error))
    return FALSE;
...
static void save_dialog_response(GtkWidget *save_dialog, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    g_free(buffer_text);
    theme_info = (MateThemeMetaInfo *)g_object_get_data(G_OBJECT(save_dialog),
                                                        "meta-theme-info");
    ...
    if (save_theme_to_disk(theme_info, theme_name, theme_description,
                           save_background, save_notification, &error)) {
      /* remove the custom theme */
      GtkTreeIter iter;

      if (theme_find_in_model(GTK_TREE_MODEL(data->theme_store), "__custom__",
                              &iter))
...
static void entry_text_changed(GtkEditable *editable, AppearanceData *data) {
  const gchar *text;
  GtkWidget *button;
...
void theme_save_dialog_run(MateThemeMetaInfo *theme_info,
  ...
  if (data->theme_save_dialog == NULL) {
    data->theme_save_dialog =
        appearance_capplet_get_widget(data, "theme_save_dialog");

    g_signal_connect(data->theme_save_dialog, "response",
                     (GCallback)save_dialog_response, data);
    g_signal_connect(data->theme_save_dialog, "delete-event",
  ...
  g_object_set_data(G_OBJECT(data->theme_save_dialog), "meta-theme-info",
                    theme_info);
