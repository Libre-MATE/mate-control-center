...
static void QuitApp(TimeAdmin *ta) {
  if (ta->UpdateTimeId > 0) g_source_remove(ta->UpdateTimeId);

  if (ta->ApplyId > 0) g_source_remove(ta->ApplyId);
...
static void ChangeTimeValue(GtkSpinButton *spin_button, gpointer data) {
  TimeAdmin *ta = (TimeAdmin *)data;
  if (TimeoutFlag == 0) {
...
static gboolean on_window_quit(GtkWidget *widget, GdkEvent *event,
                               gpointer user_data) {
...
static void on_permission_changed(GPermission *permission, GParamSpec *pspec,
                                  gpointer data) {
...
static void InitMainWindow(TimeAdmin *ta) {
  ...
  gtk_builder_add_callback_symbols(
      builder, "on_window_quit", G_CALLBACK(on_window_quit),
      "on_button1_clicked", G_CALLBACK(RunTimeZoneDialog), "on_button2_clicked",
      G_CALLBACK(SaveModifyTime), "on_button3_clicked", G_CALLBACK(CloseWindow),
      "on_spin1_changed", G_CALLBACK(ChangeTimeValue), "on_spin2_changed",
      G_CALLBACK(ChangeTimeValue), "on_spin3_changed",
      G_CALLBACK(ChangeTimeValue), "on_switch1_state_set",
      G_CALLBACK(ChangeNtpSync), NULL);
  ...
  ta->Permission =
      polkit_permission_new_sync(TIME_ADMIN_PERMISSION, NULL, NULL, &error);
  if (ta->Permission == NULL) {
    g_warning("Failed to acquire %s: %s", TIME_ADMIN_PERMISSION,
              error->message);
    g_error_free(error);
  }
  gtk_lock_button_set_permission(GTK_LOCK_BUTTON(ta->ButtonLock),
                                 ta->Permission);
  g_signal_connect(ta->Permission, "notify", G_CALLBACK(on_permission_changed),
                   ta);
 ...
 *
 * Explain: Check whether the process has been started,If the process is not
 *started, record the current process ID =====>"/tmp/user-admin.pid"
 *
 * Input:
 *
 *
...
static gboolean InitDbusProxy(TimeAdmin *ta) {
  ...
  ta->proxy = g_dbus_proxy_new_sync(ta->Connection, G_DBUS_PROXY_FLAGS_NONE,
                                    NULL, "org.freedesktop.timedate1",
                                    "/org/freedesktop/timedate1",
                                    "org.freedesktop.timedate1", NULL, &error);
  if (ta->proxy == NULL) {
...
int main(int argc, char **argv) {
  ...
  if (ProcessRuning() == TRUE) exit(0);
  if (InitDbusProxy(&ta) == FALSE) exit(0);
