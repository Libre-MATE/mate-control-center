...
enum { TARGET_URI_LIST, TARGET_NS_URL };

static const GtkTargetEntry drop_types[] = {
    {"text/uri-list", 0, TARGET_URI_LIST}, {"_NETSCAPE_URL", 0, TARGET_NS_URL}};
...
static void theme_thumbnail_update(GdkPixbuf *pixbuf, gchar *theme_name,
                                   AppearanceData *data, gboolean cache) {
  GtkTreeIter iter;
  ...
  if (theme_find_in_model(model, theme_name, &iter)) {
    ...
    if (cache && (mtime = theme_get_mtime(theme_name)) != -1) {
      ...
      mate_desktop_thumbnail_factory_save_thumbnail(data->thumb_factory, pixbuf,
                                                    path, mtime);
...
static GdkPixbuf *theme_get_thumbnail_from_cache(MateThemeMetaInfo *info,
                                                 AppearanceData *data) {
  GdkPixbuf *thumb = NULL;
  gchar *path, *thumb_filename;
  time_t mtime;

  if (info == data->theme_custom) return NULL;
  ...
  thumb_filename =
      mate_desktop_thumbnail_factory_lookup(data->thumb_factory, path, mtime);
  g_free(path);
...
static void theme_thumbnail_done_cb(GdkPixbuf *pixbuf, gchar *theme_name,
                                    AppearanceData *data) {
  theme_thumbnail_update(pixbuf, theme_name, data, TRUE);
}

static void theme_thumbnail_generate(MateThemeMetaInfo *info,
                                     AppearanceData *data) {
  ...
  if (thumb != NULL) {
    ...
    generate_meta_theme_thumbnail_async(
        info, (ThemeThumbnailFunc)theme_thumbnail_done_cb, data, NULL);
  }
}

static void theme_changed_on_disk_cb(MateThemeCommonInfo *theme,
  ...
  if (theme->type == MATE_THEME_TYPE_METATHEME) {
    ...
    if (change_type == MATE_THEME_CHANGE_CREATED) {
      gtk_list_store_insert_with_values(
          data->theme_store, NULL, 0, COL_LABEL, meta->readable_name, COL_NAME,
          meta->name, COL_THUMBNAIL, data->theme_icon, -1);
      theme_thumbnail_generate(meta, data);
    } else if (change_type == MATE_THEME_CHANGE_DELETED) {
      GtkTreeIter iter;

      if (theme_find_in_model(GTK_TREE_MODEL(data->theme_store), meta->name,
                              &iter)) {
...
static MateThemeMetaInfo *theme_load_from_gsettings(AppearanceData *data) {
  MateThemeMetaInfo *theme;
  gchar *scheme;
  ...
  theme->gtk_theme_name =
      g_settings_get_string(data->interface_settings, GTK_THEME_KEY);
  if (theme->gtk_theme_name == NULL) theme->gtk_theme_name = g_strdup("Menta");
  ...
  theme->marco_theme_name =
      g_settings_get_string(data->marco_settings, MARCO_THEME_KEY);
  if (theme->marco_theme_name == NULL)
    theme->marco_theme_name = g_strdup("Menta");

  theme->icon_theme_name =
      g_settings_get_string(data->interface_settings, ICON_THEME_KEY);
  ...
  if (mate_gsettings_schema_exists(NOTIFICATION_SCHEMA)) {
    ...
    theme->notification_theme_name =
        g_settings_get_string(notification_settings, NOTIFICATION_THEME_KEY);
    g_object_unref(notification_settings);
  } else
    theme->notification_theme_name = NULL;

  theme->cursor_theme_name =
      g_settings_get_string(data->mouse_settings, CURSOR_THEME_KEY);
  theme->cursor_size =
  ...
  theme->application_font =
      g_settings_get_string(data->interface_settings, GTK_FONT_KEY);

  return theme;
}

static gchar *theme_get_selected_name(GtkIconView *icon_view,
                                      AppearanceData *data) {
  gchar *name = NULL;
...
static const MateThemeMetaInfo *theme_get_selected(GtkIconView *icon_view,
                                                   AppearanceData *data) {
...
static void theme_select_iter(GtkIconView *icon_view, GtkTreeIter *iter) {
  GtkTreePath *path;
...
static void theme_select_name(GtkIconView *icon_view, const gchar *theme) {
  GtkTreeIter iter;
  GtkTreeModel *model = gtk_icon_view_get_model(icon_view);
...
static gboolean theme_is_equal(const MateThemeMetaInfo *a,
...
static void theme_set_custom_from_theme(const MateThemeMetaInfo *info,
                                        AppearanceData *data) {
  MateThemeMetaInfo *custom = data->theme_custom;
  GtkIconView *icon_view =
      GTK_ICON_VIEW(appearance_capplet_get_widget(data, "theme_list"));
  GtkTreeModel *model;
  GtkTreeIter iter;
  GtkTreePath *path;
  ...
  if (!theme_find_in_model(model, custom->name, &iter)) {
    ...
    gtk_list_store_insert_with_values(
        data->theme_store, &child, 0, COL_LABEL, custom->readable_name,
        COL_NAME, custom->name, COL_THUMBNAIL, data->theme_icon, -1);
...
static void theme_message_area_response_cb(GtkWidget *w, gint response_id,
  ...
  theme = theme_get_selected(
      GTK_ICON_VIEW(appearance_capplet_get_widget(data, "theme_list")), data);
  if (!theme) return;
  ...
  switch (response_id) {
    ...
    case RESPONSE_REVERT_FONT:
      if (data->revert_application_font != NULL) {
        g_settings_set_string(data->interface_settings, GTK_FONT_KEY,
                              data->revert_application_font);
        g_free(data->revert_application_font);
        data->revert_application_font = NULL;
      }

      if (data->revert_documents_font != NULL) {
        g_settings_set_string(data->interface_settings, DOCUMENT_FONT_KEY,
      ...
      if (data->caja_settings && data->revert_desktop_font != NULL) {
        g_settings_set_string(data->caja_settings, DESKTOP_FONT_KEY,
                              data->revert_desktop_font);
        g_free(data->revert_desktop_font);
        data->revert_desktop_font = NULL;
      }

      if (data->revert_windowtitle_font != NULL) {
        g_settings_set_string(data->marco_settings, WINDOW_TITLE_FONT_KEY,
                              data->revert_windowtitle_font);
        g_free(data->revert_windowtitle_font);
        data->revert_windowtitle_font = NULL;
      }

      if (data->revert_monospace_font != NULL) {
        g_settings_set_string(data->interface_settings, MONOSPACE_FONT_KEY,
    ...
    case RESPONSE_APPLY_FONT:
      ...
      if (theme->documents_font) {
        tmpfont =
            g_settings_get_string(data->interface_settings, DOCUMENT_FONT_KEY);
        if (tmpfont != NULL) {
          g_free(data->revert_documents_font);
      ...
      if (theme->windowtitle_font) {
        tmpfont =
            g_settings_get_string(data->marco_settings, WINDOW_TITLE_FONT_KEY);
        if (tmpfont != NULL) {
          g_free(data->revert_windowtitle_font);
      ...
      if (theme->monospace_font) {
        tmpfont =
            g_settings_get_string(data->interface_settings, MONOSPACE_FONT_KEY);
        if (tmpfont != NULL) {
          g_free(data->revert_monospace_font);
    ...
    case RESPONSE_INSTALL_ENGINE:
      ...
      if (engine_path != NULL) {
        theme_install_file(
            GTK_WINDOW(gtk_widget_get_toplevel(data->install_button)),
            engine_path);
        g_free(engine_path);
      }
...
static void theme_message_area_update(AppearanceData *data) {
  ...
  if (data->theme_message_area == NULL) {
    ...
    data->apply_background_button =
        gtk_info_bar_add_button(GTK_INFO_BAR(data->theme_message_area),
                                _("Apply Background"), RESPONSE_APPLY_BG);
    data->apply_font_button =
        gtk_info_bar_add_button(GTK_INFO_BAR(data->theme_message_area),
                                _("Apply Font"), RESPONSE_APPLY_FONT);
    data->revert_font_button =
        gtk_info_bar_add_button(GTK_INFO_BAR(data->theme_message_area),
                                _("Revert Font"), RESPONSE_REVERT_FONT);
    data->install_button =
        gtk_info_bar_add_button(GTK_INFO_BAR(data->theme_message_area),
                                _("Install"), RESPONSE_INSTALL_ENGINE);
    ...
    data->theme_info_icon = gtk_image_new_from_icon_name("dialog-information",
                                                         GTK_ICON_SIZE_DIALOG);
    data->theme_error_icon =
    ...
    content =
        gtk_info_bar_get_content_area(GTK_INFO_BAR(data->theme_message_area));
    gtk_container_add(GTK_CONTAINER(content), hbox);

    parent = appearance_capplet_get_widget(data, "theme_list_vbox");
    gtk_box_pack_start(GTK_BOX(parent), data->theme_message_area, FALSE, FALSE,
                       0);
  }
  ...
  if (show_error)
    ...
    message =
        _("The current theme suggests a background and a font. Also, the last "
          "applied font suggestion can be reverted.");
  else if (show_apply_background && show_revert_font)
    message =
        _("The current theme suggests a background. Also, the last applied "
    ...
    message =
        _("The current theme suggests a font. Also, the last applied font "
  ...
  if (show_error &&
      g_error_matches(error, MATE_THEME_ERROR,
  ...
  if (show_error || show_apply_background || show_apply_font ||
      show_revert_font) {
...
static void theme_selection_changed_cb(GtkWidget *icon_view,
                                       AppearanceData *data) {
  GList *selection;
  MateThemeMetaInfo *theme = NULL;
  ...
  if (selection) {
    ...
    gtk_widget_set_sensitive(
        appearance_capplet_get_widget(data, "theme_delete"),
        theme_is_writable(theme));
    gtk_widget_set_sensitive(appearance_capplet_get_widget(data, "theme_save"),
                             is_custom);
  }
}
...
static void theme_save_cb(GtkWidget *button, AppearanceData *data) {
...
static void theme_delete_cb(GtkWidget *button, AppearanceData *data) {
  ...
  if (selected) {
    ...
    if (name != NULL && strcmp(name, data->theme_custom->name) &&
...
static void theme_details_changed_cb(AppearanceData *data) {
  ...
  if (!done) /* didn't find a match, set or update custom */
    theme_set_custom_from_theme(gsettings_theme, data);

  mate_theme_meta_info_free(gsettings_theme);
}

static void theme_setting_changed_cb(GObject *settings, GParamSpec *pspec,
                                     AppearanceData *data) {
  theme_details_changed_cb(data);
}

static void theme_gsettings_changed(GSettings *settings, gchar *key,
                                    AppearanceData *data) {
  theme_details_changed_cb(data);
}

static gint theme_list_sort_func(MateThemeMetaInfo *a, MateThemeMetaInfo *b) {
  return strcmp(a->readable_name, b->readable_name);
}

static gint theme_store_sort_func(GtkTreeModel *model, GtkTreeIter *a,
...
static void theme_drag_data_received_cb(GtkWidget *widget,
  ...
  if (!(info == TARGET_URI_LIST || info == TARGET_NS_URL)) return;
...
static void background_or_font_changed(GSettings *settings, gchar *key,
                                       AppearanceData *data) {
  theme_message_area_update(data);
}
...
void themes_init(AppearanceData *data) {
  ...
  data->theme_icon = gdk_pixbuf_new_from_file(
      MATECC_PIXMAP_DIR "/theme-thumbnailing.png", NULL);
  data->theme_store = theme_store = gtk_list_store_new(
      NUM_COLS, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING);

  /* set up theme list */
  theme_list = mate_theme_meta_info_find_all();
  mate_theme_info_register_theme_change(
      (ThemeChangedCallback)theme_changed_on_disk_cb, data);
  ...
  for (l = theme_list; l; l = l->next) {
    ...
    gtk_list_store_insert_with_values(theme_store, NULL, 0, COL_LABEL,
                                      info->readable_name, COL_NAME, info->name,
                                      COL_THUMBNAIL, data->theme_icon, -1);
  ...
  if (!meta_theme) {
    ...
    gtk_list_store_insert_with_values(
        theme_store, NULL, 0, COL_LABEL, meta_theme->readable_name, COL_NAME,
        meta_theme->name, COL_THUMBNAIL, data->theme_icon, -1);
  ...
  g_object_set(renderer, "xpad", 5, "ypad", 5, "xalign", 0.5, "yalign", 1.0,
               NULL);
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(icon_view), renderer, FALSE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(icon_view), renderer, "pixbuf",
                                 COL_THUMBNAIL, NULL);

  renderer = gtk_cell_renderer_text_new();
  g_object_set(renderer, "alignment", PANGO_ALIGN_CENTER, "wrap-mode",
               PANGO_WRAP_WORD_CHAR, "wrap-width",
               gtk_icon_view_get_item_width(icon_view), "width",
               gtk_icon_view_get_item_width(icon_view), "xalign", 0.5, "yalign",
               0.0, NULL);

  gtk_cell_layout_pack_end(GTK_CELL_LAYOUT(icon_view), renderer, FALSE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(icon_view), renderer, "markup",
                                 COL_LABEL, NULL);
  ...
  gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(sort_model), COL_LABEL,
                                  theme_store_sort_func, NULL, NULL);
  ...
  g_signal_connect(icon_view, "selection-changed",
                   (GCallback)theme_selection_changed_cb, data);
  g_signal_connect_after(icon_view, "realize", (GCallback)theme_select_name,
                         meta_theme->name);
  ...
  gtk_drag_dest_set(w, GTK_DEST_DEFAULT_ALL, drop_types,
                    G_N_ELEMENTS(drop_types),
                    GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_MOVE);
  g_signal_connect(w, "drag-data-received",
                   (GCallback)theme_drag_data_received_cb, data);
  if (is_locked_down()) gtk_widget_set_sensitive(w, FALSE);
  ...
  g_signal_connect(data->marco_settings, "changed::" MARCO_THEME_KEY,
                   G_CALLBACK(theme_gsettings_changed), data);
  g_signal_connect(data->mouse_settings, "changed::" CURSOR_THEME_KEY,
                   G_CALLBACK(theme_gsettings_changed), data);
  g_signal_connect(data->mouse_settings, "changed::" CURSOR_SIZE_KEY,
                   G_CALLBACK(theme_gsettings_changed), data);
  g_signal_connect(data->wp_settings, "changed::" WP_FILE_KEY,
                   G_CALLBACK(background_or_font_changed), data);
  g_signal_connect(data->interface_settings, "changed::" GTK_FONT_KEY,
                   G_CALLBACK(background_or_font_changed), data);
  g_signal_connect(data->interface_settings, "changed::" DOCUMENT_FONT_KEY,
                   G_CALLBACK(background_or_font_changed), data);

  if (data->caja_settings)
    g_signal_connect(data->caja_settings, "changed::" DESKTOP_FONT_KEY,
                     G_CALLBACK(background_or_font_changed), data);

  g_signal_connect(data->marco_settings, "changed::" WINDOW_TITLE_FONT_KEY,
                   G_CALLBACK(background_or_font_changed), data);
  g_signal_connect(data->interface_settings, "changed::" MONOSPACE_FONT_KEY,
                   G_CALLBACK(background_or_font_changed), data);

  settings = gtk_settings_get_default();
  g_signal_connect(settings, "notify::gtk-color-scheme",
                   (GCallback)theme_setting_changed_cb, data);
...
void themes_shutdown(AppearanceData *data) {
  ...
  if (data->theme_icon) g_object_unref(data->theme_icon);
  if (data->theme_save_dialog) gtk_widget_destroy(data->theme_save_dialog);
  g_free(data->revert_application_font);
  g_free(data->revert_documents_font);
  g_free(data->revert_desktop_font);
  g_free(data->revert_windowtitle_font);
