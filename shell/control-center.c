...
void handle_static_action_clicked(Tile* tile, TileEvent* event, gpointer data) {
  ...
  MateDesktopItem* item =
      (MateDesktopItem*)g_object_get_data(G_OBJECT(tile), APP_ACTION_KEY);
  GSettings* settings;
  GApplication* app;
...
static void activate(GtkApplication* app) {
  ...
  AppShellData* app_data =
      appshelldata_new("matecc.menu", GTK_ICON_SIZE_DND, FALSE, TRUE, 0);
  ...
  if (list) {
    ...
    create_main_window(app_data, "MyControlCenter", _("Control Center"),
                       "preferences-desktop", 975, 600, hidden);
    gtk_application_add_window(app, GTK_WINDOW(app_data->main_app));
  }
}

static void quit(GApplication* app) { g_application_quit(app); }
...
int main(int argc, char* argv[]) {
  ...
  GOptionEntry options[] = {
      {"hide", 0, 0, G_OPTION_ARG_NONE, &hidden,
       N_("Hide on start (useful to preload the shell)"), NULL},
  ...
  if (!gtk_init_with_args(&argc, &argv, NULL, options, GETTEXT_PACKAGE,
                          &error)) {
    g_printerr("%s\n", error->message);
    g_error_free(error);
    return 1;
