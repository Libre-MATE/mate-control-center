...
enum { LOCATION_CHANGED, LAST_SIGNAL };

G_DEFINE_TYPE(TimezoneMap, timezone_map, GTK_TYPE_WIDGET)
static guint signals[LAST_SIGNAL];
static TimezoneMapOffset color_codes[] = {
    {-11.0, 43, 0, 0, 255},     {-10.0, 85, 0, 0, 255},
    {-9.5, 102, 255, 0, 255},   {-9.0, 128, 0, 0, 255},
    {-8.0, 170, 0, 0, 255},     {-7.0, 212, 0, 0, 255},
    {-6.0, 255, 0, 1, 255}, /* north */
    {-6.0, 255, 0, 0, 255}, /* south */
    {-5.0, 255, 42, 42, 255},   {-4.5, 192, 255, 0, 255},
    {-4.0, 255, 85, 85, 255},   {-3.5, 0, 255, 0, 255},
    {-3.0, 255, 128, 128, 255}, {-2.0, 255, 170, 170, 255},
    {-1.0, 255, 213, 213, 255}, {0.0, 43, 17, 0, 255},
    {1.0, 85, 34, 0, 255},      {2.0, 128, 51, 0, 255},
    {3.0, 170, 68, 0, 255},     {3.5, 0, 255, 102, 255},
    {4.0, 212, 85, 0, 255},     {4.5, 0, 204, 255, 255},
    {5.0, 255, 102, 0, 255},    {5.5, 0, 102, 255, 255},
    {5.75, 0, 238, 207, 247},   {6.0, 255, 127, 42, 255},
    {6.5, 204, 0, 254, 254},    {7.0, 255, 153, 85, 255},
    {8.0, 255, 179, 128, 255},  {9.0, 255, 204, 170, 255},
    {9.5, 170, 0, 68, 250},     {10.0, 255, 230, 213, 255},
    {10.5, 212, 124, 21, 250},  {11.0, 212, 170, 0, 255},
    {11.5, 249, 25, 87, 253},   {12.0, 255, 204, 0, 255},
    {12.75, 254, 74, 100, 248}, {13.0, 255, 85, 153, 250},
    {-100, 0, 0, 0, 0}};
static void cc_timezone_map_dispose(GObject *object) {
...
static void timezone_map_finalize(GObject *object) {
  TimezoneMap *self = TIMEZONEMAP(object);
...
static void cc_timezone_map_get_preferred_width(GtkWidget *widget,
  ...
  if (natural != NULL) *natural = size;
}

static void cc_timezone_map_get_preferred_height(GtkWidget *widget,
                                                 gint *minimum, gint *natural) {
  TimezoneMap *map = TIMEZONEMAP(widget);
  gint size;
...
static void cc_timezone_map_size_allocate(GtkWidget *widget,
                                          GtkAllocation *allocation) {
  TimezoneMap *map = TIMEZONEMAP(widget);
  GdkPixbuf *pixbuf;

  if (map->background) g_object_unref(map->background);
  ...
  map->background = gdk_pixbuf_scale_simple(
      pixbuf, allocation->width, allocation->height, GDK_INTERP_BILINEAR);

  if (map->color_map) g_object_unref(map->color_map);

  map->color_map =
      gdk_pixbuf_scale_simple(map->orig_color_map, allocation->width,
  ...
  GTK_WIDGET_CLASS(timezone_map_parent_class)
      ->size_allocate(widget, allocation);
}

static void cc_timezone_map_realize(GtkWidget *widget) {
  GdkWindowAttr attr = {
      0,
  };
  GtkAllocation allocation;
  ...
  attr.y = allocation.y;
  attr.event_mask =
      gtk_widget_get_events(widget) | GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK;
...
static gdouble convert_longitude_to_x(gdouble longitude, gint map_width) {
...
static gdouble radians(gdouble degrees) { return (degrees / 360.0) * G_PI * 2; }

static gdouble convert_latitude_to_y(gdouble latitude, gdouble map_height) {
  ...
  map_range =
      fabs(1.25 * log(tan(G_PI_4 + 0.4 * radians(bottom_lat))) - top_offset);
  y = fabs(y - top_offset);
  y = y / map_range;
  y = y * map_height;
  return y;
...
static void draw_text_bubble(cairo_t *cr, GtkWidget *widget, gdouble pointx,
                             gdouble pointy) {
  ...
  cairo_arc(cr, width - corner_radius, corner_radius, corner_radius,
            radians(-90), radians(0));
  cairo_arc(cr, width - corner_radius, height - corner_radius, corner_radius,
            radians(0), radians(90));
  cairo_arc(cr, corner_radius, height - corner_radius, corner_radius,
            radians(90), radians(180));
  cairo_arc(cr, corner_radius, corner_radius, corner_radius, radians(180),
            radians(270));
  cairo_close_path(cr);
...
static gboolean cc_timezone_map_draw(GtkWidget *widget, cairo_t *cr) {
  TimezoneMap *map = TIMEZONEMAP(widget);
  ...
  if (gtk_widget_is_sensitive(widget)) {
    file = g_strdup_printf(
        TIMPZONEDIR "timezone_%s.png",
        g_ascii_formatd(buf, sizeof(buf), "%g", map->selected_offset));
  } else {
    file = g_strdup_printf(
        TIMPZONEDIR "timezone_%s_dim.png",
        g_ascii_formatd(buf, sizeof(buf), "%g", map->selected_offset));
  }
  ...
  if (map->location) {
    ...
    if (map->pin) {
      gdk_cairo_set_source_pixbuf(cr, map->pin, pointx - PIN_HOT_POINT_X,
...
static void update_cursor(GtkWidget *widget) {
  GdkWindow *window;
...
static void cc_timezone_map_state_flags_changed(GtkWidget *widget,
...
static gint sort_locations(TzLocation *a, TzLocation *b) {
...
static void set_location(TimezoneMap *map, TzLocation *location) {
  g_autoptr(TzInfo) info = NULL;

  map->location = location;
  info = tz_info_from_location(map->location);
  map->selected_offset =
      tz_location_get_utc_offset(map->location) / (60.0 * 60.0) +
      ((info->daylight) ? -1.0 : 0.0);

  g_signal_emit(map, signals[LOCATION_CHANGED], 0, map->location, NULL);
}

static gboolean button_press_event(TimezoneMap *map, GdkEventButton *event) {
  gint x, y;
  guchar r, g, b, a;
  guchar *pixels;
  gint rowstride;
...
static void timezone_map_class_init(TimezoneMapClass *klass) {
  ...
  signals[LOCATION_CHANGED] = g_signal_new(
      "location-changed", TYPE_TIMEZONE_MAP, G_SIGNAL_RUN_FIRST, 0, NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
}

static void timezone_map_init(TimezoneMap *map) {
  GError *err = NULL;
  ...
  map->orig_background_dim =
      gdk_pixbuf_new_from_file(TIMPZONEDIR "bg_dim.png", &err);
  ...
  g_signal_connect_object(map, "button-press-event",
                          G_CALLBACK(button_press_event), map,
                          G_CONNECT_SWAPPED);
}
gboolean timezone_map_set_timezone(TimezoneMap *map, const gchar *timezone) {
  ...
  if (ret) gtk_widget_queue_draw(GTK_WIDGET(map));
