...
enum { DESCRIPTION_COLUMN, KEYENTRY_COLUMN, N_COLUMNS };
...
static char *binding_name(guint keyval, guint keycode,
                          EggVirtualModifierType mask, gboolean translate) {
...
static gboolean binding_from_string(const char *str, guint *accelerator_key,
  ...
  egg_accelerator_parse_virtual(str, accelerator_key, keycode,
                                accelerator_mods);
...
static void accel_set_func(GtkTreeViewColumn *tree_column,
  ...
  if (key_entry == NULL)
    ...
    g_object_set(cell, "visible", TRUE, "editable", FALSE, "accel_key",
                 key_entry->keyval, "accel_mask", key_entry->mask, "keycode",
                 key_entry->keycode, "style", PANGO_STYLE_ITALIC, NULL);
  else
    g_object_set(cell, "visible", TRUE, "editable", TRUE, "accel_key",
                 key_entry->keyval, "accel_mask", key_entry->mask, "keycode",
                 key_entry->keycode, "style", PANGO_STYLE_NORMAL, NULL);
}

static void description_set_func(GtkTreeViewColumn *tree_column,
                                 GtkCellRenderer *cell, GtkTreeModel *model,
                                 GtkTreeIter *iter, gpointer data) {
  KeyEntry *key_entry;
  ...
  if (key_entry != NULL)
    g_object_set(cell, "editable", FALSE, "text",
                 key_entry->description != NULL ? key_entry->description
                                                : _("<Unknown Action>"),
                 NULL);
  else
...
static gboolean keybinding_key_changed_foreach(GtkTreeModel *model,
                                               GtkTreePath *path,
                                               GtkTreeIter *iter,
                                               gpointer user_data) {
  KeyEntry *key_entry;
  ...
  gtk_tree_model_get(key_entry->model, iter, KEYENTRY_COLUMN, &tmp_key_entry,
                     -1);
...
static void keybinding_key_changed(GSettings *settings, gchar *key,
                                   KeyEntry *key_entry) {
  gchar *key_value;

  key_value = g_settings_get_string(settings, key);

  binding_from_string(key_value, &key_entry->keyval, &key_entry->keycode,
                      &key_entry->mask);
  key_entry->editable = g_settings_is_writable(settings, key);

  /* update the model */
  gtk_tree_model_foreach(key_entry->model, keybinding_key_changed_foreach,
                         key_entry);
}

static void keybinding_description_changed(GSettings *settings, gchar *key,
                                           KeyEntry *key_entry) {
  gchar *key_value;
...
static void keybinding_command_changed(GSettings *settings, gchar *key,
                                       KeyEntry *key_entry) {
  gchar *key_value;
...
static int keyentry_sort_func(GtkTreeModel *model, GtkTreeIter *a,
  ...
  key_entry_a = NULL;
  gtk_tree_model_get(model, a, KEYENTRY_COLUMN, &key_entry_a, -1);

  key_entry_b = NULL;
  gtk_tree_model_get(model, b, KEYENTRY_COLUMN, &key_entry_b, -1);
  ...
  if (key_entry_a && key_entry_b) {
    if ((key_entry_a->keyval || key_entry_a->keycode) &&
      ...
      name_a = binding_name(key_entry_a->keyval, key_entry_a->keycode,
                            key_entry_a->mask, TRUE);

      name_b = binding_name(key_entry_b->keyval, key_entry_b->keycode,
                            key_entry_b->mask, TRUE);
...
static void clear_old_model(GtkBuilder *builder) {
  GtkWidget *tree_view;
  ...
  if (model == NULL) {
    /* create a new model */
    model = (GtkTreeModel *)gtk_tree_store_new(N_COLUMNS, G_TYPE_STRING,
                                               G_TYPE_POINTER);

    gtk_tree_sortable_set_sort_func(GTK_TREE_SORTABLE(model), KEYENTRY_COLUMN,
                                    keyentry_sort_func, NULL, NULL);
    ...
    for (valid = gtk_tree_model_get_iter_first(model, &iter); valid;
      ...
      if (key_entry != NULL) {
        g_signal_handler_disconnect(key_entry->settings,
                                    key_entry->gsettings_cnxn);
        if (key_entry->gsettings_cnxn_desc != 0)
          g_signal_handler_disconnect(key_entry->settings,
                                      key_entry->gsettings_cnxn_desc);
        if (key_entry->gsettings_cnxn_cmd != 0)
          g_signal_handler_disconnect(key_entry->settings,
                                      key_entry->gsettings_cnxn_cmd);

        g_object_unref(key_entry->settings);
        if (key_entry->gsettings_path) g_free(key_entry->gsettings_path);
...
static gboolean key_match(GtkTreeModel *model, GtkTreePath *path,
                          GtkTreeIter *iter, gpointer data) {
  ...
  gtk_tree_model_get(model, iter, KEYENTRY_COLUMN, &element, -1);

  if (!element) return FALSE;

  if (!element->settings || !G_IS_SETTINGS(element->settings)) return FALSE;
...
static gboolean key_is_already_shown(GtkTreeModel *model,
...
static gboolean count_rows_foreach(GtkTreeModel *model, GtkTreePath *path,
                                   GtkTreeIter *iter, gpointer data) {
  gint *rows = data;
...
static void ensure_scrollbar(GtkBuilder *builder, int i) {
  if (i == MAX_ELEMENTS_BEFORE_SCROLLING) {
    GtkRequisition rectangle;
    GObject *actions_swindow =
        gtk_builder_get_object(builder, "actions_swindow");
    GtkWidget *treeview = _gtk_builder_get_widget(builder, "shortcut_treeview");
...
static void find_section(GtkTreeModel *model, GtkTreeIter *iter,
                         const char *title) {
  GtkTreeStore *store;
  gboolean success;
  ...
  gtk_tree_store_set(store, iter, DESCRIPTION_COLUMN, title, -1);
}

static void append_keys_to_tree(GtkBuilder *builder, const gchar *title,
                                const gchar *schema, const gchar *package,
  ...
  model = gtk_tree_view_get_model(
      GTK_TREE_VIEW(gtk_builder_get_object(builder, "shortcut_treeview")));
  ...
  for (j = 0; keys_list[j].name != NULL; j++) {
    ...
    if (!should_show_key(&keys_list[j])) continue;

    if (key_is_already_shown(model, &keys_list[j])) continue;
    ...
    if (keys_list[j].description_key != NULL) {
      ...
      description =
          g_settings_get_string(settings, keys_list[j].description_key);
    } else {
    ...
    if (keys_list[j].description_key != NULL) {
      ...
      key_entry->desc_editable =
          g_settings_is_writable(settings, key_entry->desc_gsettings_key);
      gchar *gsettings_signal =
          g_strconcat("changed::", key_entry->desc_gsettings_key, NULL);
      key_entry->gsettings_cnxn_desc = g_signal_connect(
          settings, gsettings_signal,
          G_CALLBACK(keybinding_description_changed), key_entry);
      g_free(gsettings_signal);
    }
    if (keys_list[j].cmd_key != NULL) {
      key_entry->cmd_gsettings_key = g_strdup(keys_list[j].cmd_key);
      key_entry->cmd_editable =
          g_settings_is_writable(settings, key_entry->cmd_gsettings_key);
      gchar *gsettings_signal =
          g_strconcat("changed::", key_entry->cmd_gsettings_key, NULL);
      key_entry->gsettings_cnxn_cmd =
          g_signal_connect(settings, gsettings_signal,
                           G_CALLBACK(keybinding_command_changed), key_entry);
      g_free(gsettings_signal);
    }
    ...
    key_entry->gsettings_cnxn =
        g_signal_connect(settings, gsettings_signal,
                         G_CALLBACK(keybinding_key_changed), key_entry);
    g_free(gsettings_signal);
    ...
    gtk_tree_store_set(GTK_TREE_STORE(model), &iter, KEYENTRY_COLUMN, key_entry,
                       -1);
...
static void parse_start_tag(GMarkupParseContext *ctx, const gchar *element_name,
  ...
  if (g_str_equal(element_name, "KeyListEntries")) {
    ...
    while (*attr_names && *attr_values) {
      if (g_str_equal(*attr_names, "name")) {
        if (**attr_values) name = *attr_values;
      } else if (g_str_equal(*attr_names, "wm_name")) {
        if (**attr_values) wm_name = *attr_values;
      } else if (g_str_equal(*attr_names, "package")) {
        if (**attr_values) package = *attr_values;
      } else if (g_str_equal(*attr_names, "schema")) {
        if (**attr_values) schema = *attr_values;
      }
      ++attr_names;
      ++attr_values;
    }
    ...
    if (name) {
      if (keylist->name) g_warning("Duplicate section name");
      g_free(keylist->name);
      keylist->name = g_strdup(name);
    }
    if (wm_name) {
      if (keylist->wm_name) g_warning("Duplicate window manager name");
      g_free(keylist->wm_name);
      keylist->wm_name = g_strdup(wm_name);
    }
    if (package) {
      if (keylist->package) g_warning("Duplicate gettext package name");
      g_free(keylist->package);
      keylist->package = g_strdup(package);
    }
    if (schema) {
      if (keylist->schema) g_warning("Duplicate schema name");
  ...
  if (!g_str_equal(element_name, "KeyListEntry") || attr_names == NULL ||
      attr_values == NULL)
    return;
...
static gboolean strv_contains(char **strv, char *str) {
  char **p;
...
static void append_keys_to_tree_from_file(GtkBuilder *builder,
                                          const char *filename,
                                          char **wm_keybindings) {
  ...
  if (!g_file_get_contents(filename, &buf, &buf_len, &err)) return;
  ...
  g_free(keylist->name);
  g_free(keylist->package);
  for (i = 0; keys[i].name != NULL; i++) g_free(keys[i].name);
  g_free(keylist);
}

static void append_keys_to_tree_from_gsettings(GtkBuilder *builder,
                                               const gchar *gsettings_path) {
  gchar **custom_list;
  GArray *entries;
  KeyListEntry key;
  gint i;
  ...
  if (custom_list != NULL) {
    for (i = 0; custom_list[i] != NULL; i++) {
      key.gsettings_path =
          g_strdup_printf("%s%s", gsettings_path, custom_list[i]);
  ...
  if (entries->len > 0) {
    ...
    append_keys_to_tree(builder, _("Custom Shortcuts"),
                        CUSTOM_KEYBINDING_SCHEMA, NULL, keys);
...
static void reload_key_entries(GtkBuilder *builder) {
  ...
  for (i = 0; data_dirs[i] != NULL; i++) {
    ...
    dir_path = g_build_filename(data_dirs[i], "mate-control-center",
                                "keybindings", NULL);
    ...
    for (name = g_dir_read_name(dir); name; name = g_dir_read_name(dir)) {
      if (g_str_has_suffix(name, ".xml") == FALSE) continue;

      if (g_hash_table_lookup(loaded_files, name) != NULL) {
        g_debug("Not loading %s, it was already loaded from another directory",
                name);
        continue;
      }
  ...
  for (l = list; l != NULL; l = l->next) {
    ...
    path = g_build_filename(g_hash_table_lookup(loaded_files, l->data), l->data,
                            NULL);
...
static void key_entry_controlling_key_changed(GSettings *settings, gchar *key,
                                              gpointer user_data) {
  reload_key_entries(user_data);
}

static gboolean cb_check_for_uniqueness(GtkTreeModel *model, GtkTreePath *path,
                                        GtkTreeIter *iter, KeyEntry *new_key) {
  KeyEntry *element;
...
static const guint forbidden_keyvals[] = {
    ...
    GDK_KEY_Page_Up, GDK_KEY_Page_Down, GDK_KEY_End, GDK_KEY_Tab,

    /* Return */
    GDK_KEY_KP_Enter, GDK_KEY_Return,

    GDK_KEY_space, GDK_KEY_Mode_switch};
...
static void show_error(GtkWindow *parent, GError *err) {
  ...
  dialog = gtk_message_dialog_new(
      parent, GTK_DIALOG_DESTROY_WITH_PARENT | GTK_DIALOG_MODAL,
      GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, _("Error saving the new shortcut"));
...
static void accel_edited_callback(GtkCellRendererText *cell,
                                  const char *path_string, guint keyval,
                                  EggVirtualModifierType mask, guint keycode,
                                  gpointer data) {
  ...
  if (keyval != 0 || keycode != 0) /* any number of keys can be disabled */
  {
    gtk_tree_model_foreach(
        model, (GtkTreeModelForeachFunc)cb_check_for_uniqueness, &tmp_key);
  }
  ...
  if (tmp_key.mask == 0 && tmp_key.keycode != 0) {
    if ((tmp_key.keyval >= GDK_KEY_a && tmp_key.keyval <= GDK_KEY_z) ||
        ...
        (tmp_key.keyval >= GDK_KEY_kana_fullstop &&
         tmp_key.keyval <= GDK_KEY_semivoicedsound) ||
        (tmp_key.keyval >= GDK_KEY_Arabic_comma &&
         tmp_key.keyval <= GDK_KEY_Arabic_sukun) ||
        (tmp_key.keyval >= GDK_KEY_Serbian_dje &&
         tmp_key.keyval <= GDK_KEY_Cyrillic_HARDSIGN) ||
        (tmp_key.keyval >= GDK_KEY_Greek_ALPHAaccent &&
         tmp_key.keyval <= GDK_KEY_Greek_omega) ||
        (tmp_key.keyval >= GDK_KEY_hebrew_doublelowline &&
         tmp_key.keyval <= GDK_KEY_hebrew_taf) ||
        (tmp_key.keyval >= GDK_KEY_Thai_kokai &&
         tmp_key.keyval <= GDK_KEY_Thai_lekkao) ||
        (tmp_key.keyval >= GDK_KEY_Hangul &&
         tmp_key.keyval <= GDK_KEY_Hangul_Special) ||
        (tmp_key.keyval >= GDK_KEY_Hangul_Kiyeog &&
         tmp_key.keyval <= GDK_KEY_Hangul_J_YeorinHieuh) ||
      ...
      dialog = gtk_message_dialog_new(
          ...
          _("The shortcut \"%s\" cannot be used because it will become "
      ...
      egg_cell_renderer_keys_set_accelerator(
          EGG_CELL_RENDERER_KEYS(cell), key_entry->keyval, key_entry->keycode,
          key_entry->mask);
      return;
    }
  }
  ...
  if (!tmp_key.editable) {
    ...
    gtk_message_dialog_format_secondary_text(
        ...
        _("If you reassign the shortcut to \"%s\", the \"%s\" shortcut "
          "will be disabled."),
        key_entry->description ? key_entry->description
                               : key_entry->gsettings_key,
        tmp_key.description ? tmp_key.description : tmp_key.gsettings_key);

    gtk_dialog_add_button(GTK_DIALOG(dialog), _("_Reassign"),
                          GTK_RESPONSE_ACCEPT);
...
static void accel_cleared_callback(GtkCellRendererText *cell,
                                   const char *path_string, gpointer data) {
  GtkTreeView *view = (GtkTreeView *)data;
  GtkTreePath *path = gtk_tree_path_new_from_string(path_string);
  KeyEntry *key_entry;
  GtkTreeIter iter;
...
static void description_edited_callback(GtkCellRendererText *renderer,
  ...
  /* sanity check */
  if (key_entry == NULL || key_entry->desc_gsettings_key == NULL) return;

  if (!g_settings_set_string(key_entry->settings, key_entry->desc_gsettings_key,
                             new_text))
...
static gboolean real_start_editing_cb(IdleData *idle_data) {
  gtk_widget_grab_focus(GTK_WIDGET(idle_data->tree_view));
  gtk_tree_view_set_cursor(idle_data->tree_view, idle_data->path,
                           idle_data->column, TRUE);
  gtk_tree_path_free(idle_data->path);
  g_free(idle_data);
  return FALSE;
}

static gboolean edit_custom_shortcut(KeyEntry *key) {
  gint result;
  const gchar *text;
  gboolean ret;
...
static gboolean remove_custom_shortcut(GtkTreeModel *model, GtkTreeIter *iter) {
  GtkTreeIter parent;
  KeyEntry *key;
  ...
  if (key->command == NULL) return FALSE;
...
static void update_custom_shortcut(GtkTreeModel *model, GtkTreeIter *iter) {
  KeyEntry *key;
  ...
  if (key->command == NULL || key->command[0] == '\0') {
    ...
    gtk_tree_store_set(GTK_TREE_STORE(model), iter, KEYENTRY_COLUMN, key, -1);
    if (key->description != NULL)
      g_settings_set_string(key->settings, key->desc_gsettings_key,
                            key->description);
    else
...
static gchar *find_free_gsettings_path(GError **error) {
  gchar **existing_dirs;
  gchar *dir = NULL;
  gchar *fulldir = NULL;
  int i;
  ...
  if (i == MAX_CUSTOM_SHORTCUTS) {
    g_free(dir);
    dir = NULL;
    g_set_error_literal(error, g_quark_from_string("Keyboard Shortcuts"), 0,
...
static void add_custom_shortcut(GtkTreeView *tree_view, GtkTreeModel *model) {
  KeyEntry *key_entry;
  GtkTreeIter iter;
  ...
  if (dir == NULL) {
    show_error(GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(tree_view))),
               error);
  ...
  if (edit_custom_shortcut(key_entry) && key_entry->command &&
      key_entry->command[0]) {
    ...
    /* store in gsettings */
    key_entry->settings = g_settings_new_with_path(CUSTOM_KEYBINDING_SCHEMA,
                                                   key_entry->gsettings_path);
    g_settings_set_string(key_entry->settings, key_entry->gsettings_key,
                          "disabled");
    g_settings_set_string(key_entry->settings, key_entry->desc_gsettings_key,
                          key_entry->description);
    g_settings_set_string(key_entry->settings, key_entry->cmd_gsettings_key,
                          key_entry->command);

    /* add gsettings watches */
    key_entry->gsettings_cnxn_desc =
        g_signal_connect(key_entry->settings, "changed::name",
                         G_CALLBACK(keybinding_description_changed), key_entry);
    key_entry->gsettings_cnxn_cmd =
        g_signal_connect(key_entry->settings, "changed::action",
                         G_CALLBACK(keybinding_command_changed), key_entry);
    key_entry->gsettings_cnxn =
        g_signal_connect(key_entry->settings, "changed::binding",
                         G_CALLBACK(keybinding_key_changed), key_entry);
...
static void start_editing_kb_cb(GtkTreeView *treeview, GtkTreePath *path,
  ...
  if (key->desc_editable && column == gtk_tree_view_get_column(treeview, 0)) {
    ...
    gtk_widget_grab_focus(GTK_WIDGET(treeview));
    gtk_tree_view_set_cursor(treeview, path,
                             gtk_tree_view_get_column(treeview, 1), TRUE);
  }
}

static gboolean start_editing_cb(GtkTreeView *tree_view, GdkEventButton *event,
                                 gpointer user_data) {
  GtkTreePath *path;
  GtkTreeViewColumn *column;
  ...
  if (gtk_tree_view_get_path_at_pos(tree_view, (gint)event->x, (gint)event->y,
    ...
    if (key->desc_editable &&
      ...
      idle_data->path = path;
      idle_data->column =
          key->desc_editable ? column : gtk_tree_view_get_column(tree_view, 1);
 ...
 * of the widgets */
static gboolean maybe_block_accels(GtkWidget *widget, GdkEventKey *event,
                                   gpointer user_data) {
  if (block_accels) {
...
static void cb_dialog_response(GtkWidget *widget, gint response_id,
                               gpointer data) {
  ...
  if (response_id == GTK_RESPONSE_HELP) {
    capplet_help(GTK_WINDOW(widget), "goscustdesk-39");
...
static void selection_changed(GtkTreeSelection *selection, gpointer data) {
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    gtk_tree_model_get(model, &iter, KEYENTRY_COLUMN, &key, -1);
    if (key && key->command != NULL && key->editable) can_remove = TRUE;
  }

  gtk_widget_set_sensitive(button, can_remove);
}

static void cb_app_dialog_response(GtkWidget *dialog, gint response_id,
                                   gpointer data) {
...
static void setup_dialog(GtkBuilder *builder, GSettings *marco_settings) {
  ...
  g_signal_connect(treeview, "button_press_event", G_CALLBACK(start_editing_cb),
                   builder);
  g_signal_connect(treeview, "row-activated", G_CALLBACK(start_editing_kb_cb),
                   NULL);

  renderer = gtk_cell_renderer_text_new();

  g_signal_connect(renderer, "edited", G_CALLBACK(description_edited_callback),
                   treeview);

  column = gtk_tree_view_column_new_with_attributes(
      _("Action"), renderer, "text", DESCRIPTION_COLUMN, NULL);
  gtk_tree_view_column_set_cell_data_func(column, renderer,
                                          description_set_func, NULL, NULL);
  ...
  renderer =
      (GtkCellRenderer *)g_object_new(EGG_TYPE_CELL_RENDERER_KEYS, "accel_mode",
                                      EGG_CELL_RENDERER_KEYS_MODE_X, NULL);

  g_signal_connect(renderer, "accel_edited", G_CALLBACK(accel_edited_callback),
                   treeview);
  ...
  column =
      gtk_tree_view_column_new_with_attributes(_("Shortcut"), renderer, NULL);
  gtk_tree_view_column_set_cell_data_func(column, renderer, accel_set_func,
                                          NULL, NULL);
  gtk_tree_view_column_set_resizable(column, FALSE);
  ...
  g_signal_connect(marco_settings, "changed::num-workspaces",
                   G_CALLBACK(key_entry_controlling_key_changed), builder);
  ...
  g_signal_connect(widget, "key_press_event", G_CALLBACK(maybe_block_accels),
                   NULL);
  ...
  custom_shortcut_dialog =
      _gtk_builder_get_widget(builder, "custom-shortcut-dialog");
  custom_shortcut_name_entry =
      _gtk_builder_get_widget(builder, "custom-shortcut-name-entry");
  custom_shortcut_command_entry =
      _gtk_builder_get_widget(builder, "custom-shortcut-command-entry");
  ...
  widget =
      _gtk_builder_get_widget(builder, "custom-shortcut-application-dialog");
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(gtk_dialog_run),
                           widget);
  g_signal_connect(widget, "response", G_CALLBACK(cb_app_dialog_response),
                   NULL);
...
static void on_window_manager_change(const char *wm_name, GtkBuilder *builder) {
  reload_key_entries(builder);
}
...
int main(int argc, char *argv[]) {
  ...
  builder = gtk_builder_new_from_resource(
      "/org/mate/mcc/keybindings/mate-keybinding-properties.ui");
