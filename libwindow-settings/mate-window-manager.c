...
GObject *mate_window_manager_new(MateDesktopItem *it) {
  ...
  module_name =
      g_module_build_path(MATE_WINDOW_MANAGER_MODULE_PATH, settings_lib);

  module = g_module_open(module_name, G_MODULE_BIND_LAZY);
  if (module == NULL) {
    g_warning("Couldn't load window manager settings module `%s' (%s)",
  ...
  success =
      g_module_symbol(module, "window_manager_new", (gpointer *)&wm_new_func);

  if ((!success) || wm_new_func == NULL) {
    g_warning(
        "Couldn't load window manager settings module `%s`, couldn't find "
        "symbol \'window_manager_new\'",
        module_name);
    g_free(module_name);
    return NULL;
  }
  ...
  (MATE_WINDOW_MANAGER(wm))->p->window_manager_name =
      g_strdup(mate_desktop_item_get_string(it, MATE_DESKTOP_ITEM_NAME));
...
void mate_window_manager_get_double_click_actions(
    MateWindowManager *wm, const MateWMDoubleClickAction **actions,
    int *n_actions) {
  MateWindowManagerClass *klass = MATE_WINDOW_MANAGER_GET_CLASS(wm);
...
static void mate_window_manager_init(MateWindowManager *mate_window_manager,
                                     MateWindowManagerClass *class) {
  mate_window_manager->p = g_new0(MateWindowManagerPrivate, 1);
}

static void mate_window_manager_finalize(GObject *object) {
...
enum { SETTINGS_CHANGED, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {0};

static void mate_window_manager_class_init(MateWindowManagerClass *class) {
  ...
  signals[SETTINGS_CHANGED] =
      g_signal_new("settings_changed", G_OBJECT_CLASS_TYPE(class),
...
GType mate_window_manager_get_type(void) {
  ...
  if (!mate_window_manager_type) {
    ...
    mate_window_manager_type = g_type_register_static(
        G_TYPE_OBJECT, "MateWindowManager", &mate_window_manager_info, 0);
  }
...
void mate_wm_settings_free(MateWMSettings *settings) {
  ...
  if (settings->flags & MATE_WM_SETTING_FONT) g_free((void *)settings->font);
  if (settings->flags & MATE_WM_SETTING_MOUSE_MOVE_MODIFIER)
    g_free((void *)settings->mouse_move_modifier);
  if (settings->flags & MATE_WM_SETTING_THEME) g_free((void *)settings->theme);
