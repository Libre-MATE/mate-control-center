...
#define EXIT_SHELL_ON_ACTION_UPGRADE_UNINSTALL \
  "cc-exit-shell-on-action-upgrade-uninstall"
...
static GtkWidget *create_groups_section(AppShellData *app_data,
                                        const gchar *title);
static GtkWidget *create_actions_section(
    AppShellData *app_data, const gchar *title,
    void (*actions_handler)(Tile *, TileEvent *, gpointer));

static void generate_category(const char *category,
                              MateMenuTreeDirectory *root_dir,
                              AppShellData *app_data, gboolean recursive);
static void generate_launchers(MateMenuTreeDirectory *root_dir,
                               AppShellData *app_data, CategoryData *cat_data,
                               gboolean recursive);
...
static gboolean handle_filter_changed(NldSearchBar *search_bar,
...
static void matemenu_tree_changed_callback(MateMenuTree *tree,
                                           gpointer user_data);
...
void hide_shell(AppShellData *app_data) {
  gtk_window_get_position(GTK_WINDOW(app_data->main_app),
                          &app_data->main_app_window_x,
                          &app_data->main_app_window_y);
...
void show_shell(AppShellData *app_data) {
  ...
  if (!app_data->static_actions)
    gtk_widget_hide(
        app_data
            ->actions_section); /* don't show unless a launcher is selected */

  if (app_data->main_app_window_shown_once)
    gtk_window_move(GTK_WINDOW(app_data->main_app), app_data->main_app_window_x,
                    app_data->main_app_window_y);
...
gboolean create_main_window(AppShellData *app_data, const gchar *app_name,
  ...
  g_signal_connect(main_app, "delete-event", G_CALLBACK(main_delete_callback),
                   app_data);
...
static gboolean return_first_entry(gpointer key, gpointer value,
                                   gpointer unused) {
...
void layout_shell(AppShellData *app_data, const gchar *filter_title,
  ...
  if (WidthOfScreen(gdk_x11_screen_get_xscreen(gdk_screen_get_default())) <=
      SIZING_SCREEN_WIDTH_LARGE) {
    if (WidthOfScreen(gdk_x11_screen_get_xscreen(gdk_screen_get_default())) <=
        SIZING_SCREEN_WIDTH_MEDIUM)
  ...
  actions_section =
      create_actions_section(app_data, actions_title, actions_handler);
...
static gboolean relayout_shell_partial(gpointer user_data) {
  AppShellData *app_data = (AppShellData *)user_data;
  GtkBox *vbox = APP_RESIZER(app_data->category_layout)->child;
  CategoryData *data;

  if (app_data->stop_incremental_relayout) return FALSE;
  ...
  if (app_data->incremental_relayout_cat_list != NULL) {
    ...
    if (data->filtered_launcher_list != NULL) {
      ...
      gtk_box_pack_start(vbox, GTK_WIDGET(data->section), TRUE, TRUE, 0);
...
static void relayout_shell_incremental(AppShellData *app_data) {
  GtkBox *vbox = APP_RESIZER(app_data->category_layout)->child;
  ...
  if (app_data->cached_tables_list) g_list_free(app_data->cached_tables_list);
...
static void populate_groups_section(AppShellData *app_data) {
  ...
  do {
    ...
    if (NULL != data->filtered_launcher_list) {
      gtk_widget_set_state_flags(GTK_WIDGET(data->group_launcher),
                                 GTK_STATE_FLAG_NORMAL, FALSE);
      gtk_box_pack_start(vbox, GTK_WIDGET(data->group_launcher), FALSE, FALSE,
                         0);
    }
...
static void handle_group_clicked(Tile *tile, TileEvent *event,
  ...
  gint clicked_pos = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(tile), GROUP_POSITION_NUMBER_KEY));
  ...
  do {
    ...
    gint pos = GPOINTER_TO_INT(g_object_get_data(
        G_OBJECT(cat_data->group_launcher), GROUP_POSITION_NUMBER_KEY));
    if (pos == clicked_pos) {
...
static gboolean handle_filter_changed_delayed(gpointer user_data) {
  AppShellData *app_data = (AppShellData *)user_data;
  ...
  /*  showing the updates incremtally is very visually distracting. Much worse
     than just blanking until the incremental work is done and then doing one
     show. It would be nice to optimize this though somehow and not even show
     any change but the cursor change until all the work is done. But since we
     do the work incrementally in an idle loop I don't know how else besides
     hiding to not show incremental updates
   */
  /* gdk_window_freeze_updates(app_data->category_layout->window); */
  gtk_widget_hide(app_data->category_layout);
  app_data->busy_cursor = gdk_cursor_new_for_display(
      gtk_widget_get_display(app_data->shell), GDK_WATCH);
...
static gboolean handle_filter_changed(NldSearchBar *search_bar,
  ...
  if (app_data->filter_string) g_free(app_data->filter_string);
...
static void generate_filtered_lists(gpointer catdata, gpointer user_data) {
  ...
  do {
    ...
    gtk_widget_set_state_flags(GTK_WIDGET(launcher), GTK_STATE_FLAG_NORMAL,
                               FALSE);
    filename = g_object_get_data(G_OBJECT(launcher),
                                 TILE_EXEC_NAME); /* do I need to free this */
    ...
    if (g_strrstr(temp1, filter_string) ||
        (launcher->description && g_strrstr(temp2, filter_string)) ||
...
static void delete_old_data(AppShellData *app_data) {
  GList *temp;
  GList *cat_list;
...
static void create_application_category_sections(AppShellData *app_data) {
  ...
  g_assert(app_data != NULL);
  g_assert(app_data->categories_list !=
           NULL); /* Fixme - pop up a dialog box and then close */
  ...
  do {
    ...
    markup = g_markup_printf_escaped(
        "<span size=\"x-large\" weight=\"bold\">%s</span>", data->category);
    data->section = SLAB_SECTION(slab_section_new_with_markup(markup, Style2));

    /* as we filter these will be added/removed from parent container and we
     * dont want them destroyed */
...
static void show_no_results_message(AppShellData *app_data,
  ...
  if (!app_data->filtered_out_everything_widget) {
    ...
    gtk_widget_set_halign(app_data->filtered_out_everything_widget,
                          GTK_ALIGN_CENTER);
    ...
    gtk_box_pack_start(GTK_BOX(app_data->filtered_out_everything_widget), image,
                       FALSE, FALSE, 0);

    label = gtk_label_new(NULL);
    gtk_label_set_line_wrap(GTK_LABEL(label), TRUE);
    gtk_box_pack_start(GTK_BOX(app_data->filtered_out_everything_widget), label,
                       TRUE, TRUE, 15);
  ...
  str2 =
      g_strdup_printf(_("Your filter \"%s\" does not match any items."), str1);
  ...
  gtk_label_set_use_markup(app_data->filtered_out_everything_widget_label,
                           TRUE);
  gtk_box_pack_start(GTK_BOX(containing_vbox),
                     app_data->filtered_out_everything_widget, TRUE, TRUE, 0);
  g_free(str1);
  g_free(str2);
  g_free(markup);
}
...
static void populate_application_category_section(AppShellData *app_data,
  ...
  app_resizer_layout_table_default(APP_RESIZER(app_data->category_layout),
                                   table, launcher_list);
}
...
static void matemenu_tree_changed_callback(MateMenuTree *old_tree,
                                           gpointer user_data) {
  /*
  This method only gets called on the first change (matemenu appears to ignore
  subsequent) until we reget the root dir which we can't do in this method
  because if we do for some reason this method then gets called multiple times
  for one actual change. This actually is okay because it's probably a good idea
  to wait a couple seconds to regenerate the categories in case there are
  multiple quick changes being made, no sense regenerating multiple times.
  */
  GError *error = NULL;
  ...
  if (!matemenu_tree_load_sync(app_data->tree, &error)) {
    ...
    g_timeout_add_full(G_PRIORITY_DEFAULT_IDLE, 3000,
                       (GSourceFunc)regenerate_categories, user_data, NULL);
  }
}

AppShellData *appshelldata_new(const gchar *menu_name, GtkIconSize icon_size,
                               gboolean show_tile_generic_name,
                               gboolean exit_on_close,
                               gint new_apps_max_items) {
...
void generate_categories(AppShellData *app_data) {
  ...
  if (!app_data->tree) {
    ...
    app_data->tree =
        matemenu_tree_new(app_data->menu_name, MATEMENU_TREE_FLAGS_NONE);
    g_signal_connect(app_data->tree, "changed",
                     G_CALLBACK(matemenu_tree_changed_callback), app_data);
    if (!matemenu_tree_load_sync(app_data->tree, &error)) {
      g_warning("Menu tree loading got error:%s\n", error->message);
      g_error_free(error);
      g_object_unref(app_data->tree);
  ...
  if (app_data->tree == NULL || root_dir == NULL) {
    GtkWidget *dialog = gtk_message_dialog_new(
        ...
        GTK_BUTTONS_CLOSE, "Failure loading - %s", app_data->menu_name);
  ...
  if (need_misc) generate_category(_("Other"), root_dir, app_data, FALSE);
...
static gboolean check_specific_apps_hack(MateDesktopItem *item) {
  ...
  if (!got_lockdown_value) {
    ...
    command_line_lockdown =
        g_settings_get_boolean(lockdown_settings, COMMAND_LINE_LOCKDOWN_KEY);
    g_object_unref(lockdown_settings);
  }

  /* This seems like an ugly hack but it's the way it's currently done in the
   * old control center */
...
static void generate_launchers(MateMenuTreeDirectory *root_dir,
  ...
  while ((type = matemenu_tree_iter_next(iter)) != MATEMENU_TREE_ITEM_INVALID) {
    ...
    switch (type) {
      case MATEMENU_TREE_ITEM_DIRECTORY:
        item = matemenu_tree_iter_get_directory(iter);
        /* g_message ("Found sub-category %s", matemenu_tree_directory_get_name
         * (item)); */
        if (recursive) generate_launchers(item, app_data, cat_data, TRUE);
        matemenu_tree_item_unref(item);
        break;
      case MATEMENU_TREE_ITEM_ENTRY:
        item = matemenu_tree_iter_get_entry(iter);
        /* g_message ("Found item name is:%s",
         * matemenu_tree_entry_get_desktop_file_id(item)); */
        desktop_file = matemenu_tree_entry_get_desktop_file_path(item);
        if (desktop_file) {
          if (g_hash_table_lookup(app_data->hash, desktop_file)) {
            break; /* duplicate */
          }
          /* Fixme - make sure it's safe to store this without duping it. As far
             as I can tell it is safe as long as I don't hang on to this
             anylonger than I hang on to the MateMenuTreeEntry* which brings up
             another point - am I supposed to free these or does freeing the top
             level recurse
             */
...
static void generate_new_apps(AppShellData *app_data) {
  ...
  all_apps_file_name =
      g_build_filename(g_get_user_config_dir(), "mate", "ab-newapps.txt", NULL);

  if (!g_file_get_contents(all_apps_file_name, &all_apps, NULL, &error)) {
    /* If file does not exist, this is the first time this user has run this,
    ...
    for (launchers = cat_data->launcher_list; launchers;
      ...
      if (!g_hash_table_lookup(all_apps_cache, uri)) {
        ...
        if (g_hash_table_lookup(new_apps_dups, uri)) {
          /* if a desktop file is in 2 or more top level categories, only show
           * it once */
          break;
        }
        ...
        if (!got_new_apps) {
          ...
          new_apps_category->category = g_strdup(app_data->new_apps->name);
          app_data->new_apps->garray = g_array_sized_new(
              FALSE, TRUE, sizeof(NewAppData *), app_data->new_apps->max_items);

          /* should not need this, but a bug in glib does not actually clear the
           * elements until you call this method */
          g_array_set_size(app_data->new_apps->garray,
                           app_data->new_apps->max_items);
          got_new_apps = TRUE;
        }

        file = g_file_new_for_uri(uri);
        info = g_file_query_info(file, G_FILE_ATTRIBUTE_TIME_MODIFIED, 0, NULL,
                                 NULL);
        ...
        filetime = (long)g_file_info_get_attribute_uint64(
            info, G_FILE_ATTRIBUTE_TIME_MODIFIED);
        g_object_unref(info);
        g_object_unref(file);

        for (x = 0; x < app_data->new_apps->max_items; x++) {
          NewAppData *temp_data = (NewAppData *)g_array_index(
              app_data->new_apps->garray, NewAppData *, x);
          if (!temp_data ||
              filetime > temp_data->time) /* if this slot is empty or we are
                                             newer than this slot */
          {
            NewAppData *temp = g_new0(NewAppData, 1);
            temp->time = filetime;
            temp->item = item;
            g_array_insert_val(app_data->new_apps->garray, x, temp);
            break;
          }
        }
      }
  ...
  if (got_new_apps) {
    for (x = 0; x < app_data->new_apps->max_items; x++) {
      NewAppData *data = (NewAppData *)g_array_index(app_data->new_apps->garray,
                                                     NewAppData *, x);
      if (data) {
        insert_launcher_into_category(new_apps_category, data->item, app_data);
        g_free(data);
      } else
        break;
    }
...
static void insert_launcher_into_category(CategoryData *cat_data,
  ...
  launcher = application_tile_new_full(
      mate_desktop_item_get_location(desktop_item), app_data->icon_size,
      app_data->show_tile_generic_name);
  gtk_widget_set_size_request(launcher, SIZING_TILE_WIDTH, -1);

  filepath = g_strdup(
      mate_desktop_item_get_string(desktop_item, MATE_DESKTOP_ITEM_EXEC));
  g_strdelimit(filepath, " ",
               '\0'); /* just want the file name - no args or replacements */
  filename = g_strrstr(filepath, "/");
  if (filename) g_stpcpy(filepath, filename + 1);
  ...
  g_signal_connect(launcher, "tile-activated", G_CALLBACK(tile_activated_cb),
                   app_data);

  /* Note that this will handle the case of the action being launched via the
     side panel as well as directly from the context menu of an individual
     launcher, because they both funnel through tile_button_action_activate.
  */
  g_signal_connect(launcher, "tile-action-triggered",
                   G_CALLBACK(handle_menu_action_performed), app_data);

  /* These will be inserted/removed from tables as the filter changes and we
   * dont want them */
  /* destroyed when they are removed */
  g_object_ref(launcher);

  /* use alphabetical order instead of the matemenu order. We group all sub
  items in each top level category together, ignoring sub menus, so we also
  ignore sub menu layout hints */
...
static void handle_launcher_single_clicked(Tile *launcher, gpointer data) {
  ...
  tile_trigger_action(launcher,
                      launcher->actions[APPLICATION_TILE_ACTION_START]);
...
static void handle_menu_action_performed(Tile *launcher, TileEvent *event,
  ...
  if (action == launcher->actions[APPLICATION_TILE_ACTION_START]) {
    ...
    temp = EXIT_SHELL_ON_ACTION_ADD_REMOVE;
           ...
           action ==
               launcher->actions[APPLICATION_TILE_ACTION_UNINSTALL_PACKAGE]) {
