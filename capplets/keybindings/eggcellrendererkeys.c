...
static void egg_cell_renderer_keys_class_init(
    EggCellRendererKeysClass *cell_keys_class);
...
static void egg_cell_renderer_keys_get_property(GObject *object, guint param_id,
                                                GValue *value,
                                                GParamSpec *pspec);
static void egg_cell_renderer_keys_set_property(GObject *object, guint param_id,
                                                const GValue *value,
                                                GParamSpec *pspec);
...
GType egg_cell_renderer_keys_get_type(void) {
  ...
  if (!cell_keys_type) {
    ...
    cell_keys_type = g_type_register_static(
        GTK_TYPE_CELL_RENDERER_TEXT, "EggCellRendererKeys", &cell_keys_info, 0);
  }
...
static void marshal_VOID__STRING_UINT_FLAGS_UINT(GClosure *closure,
                                                 GValue *return_value,
  ...
  typedef void (*GMarshalFunc_VOID__STRING_UINT_FLAGS_UINT)(
      gpointer data1, const char *arg_1, guint arg_2, int arg_3, guint arg_4,
      gpointer data2);
  ...
  callback =
      (GMarshalFunc_VOID__STRING_UINT_FLAGS_UINT)(marshal_data ? marshal_data
                                                               : cc->callback);

  callback(data1, g_value_get_string(param_values + 1),
           g_value_get_uint(param_values + 2),
           g_value_get_flags(param_values + 3),
           g_value_get_uint(param_values + 4), data2);
}
...
static void egg_cell_renderer_keys_class_init(
  ...
  GTK_CELL_RENDERER_CLASS(cell_keys_class)->start_editing =
  ...
  g_object_class_install_property(
      object_class, PROP_ACCEL_KEY,
      g_param_spec_uint("accel_key", _("Accelerator key"), _("Accelerator key"),
                        0, G_MAXINT, 0, G_PARAM_READABLE | G_PARAM_WRITABLE));

  g_object_class_install_property(
      object_class, PROP_ACCEL_MASK,
      g_param_spec_flags("accel_mask", _("Accelerator modifiers"),
                         _("Accelerator modifiers"), GDK_TYPE_MODIFIER_TYPE, 0,
                         G_PARAM_READABLE | G_PARAM_WRITABLE));

  g_object_class_install_property(
      object_class, PROP_KEYCODE,
      g_param_spec_uint("keycode", _("Accelerator keycode"),
                        _("Accelerator keycode"), 0, G_MAXINT, 0,
                        G_PARAM_READABLE | G_PARAM_WRITABLE));

  /* FIXME: Register the enum when moving to GTK+ */
  g_object_class_install_property(
      object_class, PROP_ACCEL_MODE,
      g_param_spec_int("accel_mode", _("Accel Mode"),
                       _("The type of accelerator."), 0, 2, 0,
                       G_PARAM_READABLE | G_PARAM_WRITABLE));

  g_signal_new("accel_edited", EGG_TYPE_CELL_RENDERER_KEYS, G_SIGNAL_RUN_LAST,
               G_STRUCT_OFFSET(EggCellRendererKeysClass, accel_edited), NULL,
               NULL, marshal_VOID__STRING_UINT_FLAGS_UINT, G_TYPE_NONE, 4,
               G_TYPE_STRING, G_TYPE_UINT, GDK_TYPE_MODIFIER_TYPE, G_TYPE_UINT);

  g_signal_new("accel_cleared", EGG_TYPE_CELL_RENDERER_KEYS, G_SIGNAL_RUN_LAST,
               G_STRUCT_OFFSET(EggCellRendererKeysClass, accel_cleared), NULL,
...
static gchar *convert_keysym_state_to_string(guint keysym, guint keycode,
...
static void egg_cell_renderer_keys_set_property(GObject *object, guint param_id,
  ...
  switch (param_id) {
    case PROP_ACCEL_KEY:
      egg_cell_renderer_keys_set_accelerator(keys, g_value_get_uint(value),
                                             keys->keycode, keys->accel_mask);
      break;

    case PROP_ACCEL_MASK:
      egg_cell_renderer_keys_set_accelerator(
          keys, keys->accel_key, keys->keycode, g_value_get_flags(value));
      break;
    case PROP_KEYCODE:
      egg_cell_renderer_keys_set_accelerator(
          keys, keys->accel_key, g_value_get_uint(value), keys->accel_mask);
      break;
...
static void egg_cell_renderer_keys_get_size(GtkCellRenderer *cell,
  ...
  (*GTK_CELL_RENDERER_CLASS(parent_class)->get_size)(
      cell, widget, cell_area, x_offset, y_offset, width, height);
  /* FIXME: need to take the cell_area et al. into account */
  if (width) *width = MAX(*width, requisition.width);
  if (height) *height = MAX(*height, requisition.height);
}
...
static gboolean grab_key_callback(GtkWidget *widget, GdkEventKey *event,
                                  void *data) {
  ...
  gdk_keymap_translate_keyboard_state(
      ...
      event->hardware_keycode, event->state, event->group, NULL, NULL, NULL,
      &consumed_modifiers);
  ...
  ignored_modifiers |= GDK_BUTTON1_MASK | GDK_BUTTON2_MASK | GDK_BUTTON3_MASK |
                       GDK_BUTTON4_MASK | GDK_BUTTON5_MASK;

  /* filter consumed/ignored modifiers */
  if (keys->accel_mode == EGG_CELL_RENDERER_KEYS_MODE_GTK) {
    accel_mods = event->state & GDK_MODIFIER_MASK &
...
out:
  ...
  if (edited) {
    g_signal_emit_by_name(G_OBJECT(keys), "accel_edited", path, accel_keyval,
                          accel_mods, event->hardware_keycode);
  } else if (cleared) {
...
static void pointless_eventbox_start_editing(GtkCellEditable *cell_editable,
                                             GdkEvent *event) {
...
static GType pointless_eventbox_subclass_get_type(void) {
  ...
  if (!eventbox_type) {
    static const GTypeInfo eventbox_info = {sizeof(GtkEventBoxClass),
                                            NULL, /* base_init */
                                            NULL, /* base_finalize */
                                            NULL,
                                            NULL, /* class_finalize */
    ...
    static const GInterfaceInfo cell_editable_info = {
        (GInterfaceInitFunc)pointless_eventbox_cell_editable_init, NULL, NULL};

    eventbox_type = g_type_register_static(
        GTK_TYPE_EVENT_BOX, "EggCellEditableEventBox", &eventbox_info, 0);

    g_type_add_interface_static(eventbox_type, GTK_TYPE_CELL_EDITABLE,
                                &cell_editable_info);
  }
...
static void override_color(GtkWidget *widget, GdkRGBA *rgba) {
  gchar *css;
  GtkCssProvider *provider;

  provider = gtk_css_provider_new();

  css = g_strdup_printf("* { color: %s;}", gdk_rgba_to_string(rgba));
  gtk_css_provider_load_from_data(provider, css, -1, NULL);
  g_free(css);
...
static GtkCellEditable *egg_cell_renderer_keys_start_editing(
  ...
  if (g_value_get_boolean(&celltext_editable) == FALSE) return NULL;
  ...
  if (gdk_seat_grab(seat, gtk_widget_get_window(widget),
                    GDK_SEAT_CAPABILITY_ALL, FALSE, NULL, event, NULL,
                    NULL) != GDK_GRAB_SUCCESS)
    return NULL;

  keys->grab_widget = widget;

  g_signal_connect(widget, "key_press_event", G_CALLBACK(grab_key_callback),
                   keys);
  ...
  gtk_style_context_get_color(gtk_widget_get_style_context(widget),
                              GTK_STATE_INSENSITIVE, &label_color);
  ...
  g_object_set_data_full(G_OBJECT(keys->edit_widget),
                         EGG_CELL_RENDERER_TEXT_PATH, g_strdup(path), g_free);

  gtk_widget_show_all(keys->edit_widget);

  g_signal_connect(keys->edit_widget, "unrealize", G_CALLBACK(ungrab_stuff),
                   keys);
...
void egg_cell_renderer_keys_set_accelerator(EggCellRendererKeys *keys,
  ...
  if (changed) {
    ...
    text = convert_keysym_state_to_string(keys->accel_key, keys->keycode,
                                          keys->accel_mask);
