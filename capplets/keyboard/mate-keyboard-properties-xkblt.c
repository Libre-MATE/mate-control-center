...
void clear_xkb_elements_list(GSList *list) {
  while (list != NULL) {
    GSList *p = list;
    list = list->next;
    g_free(p->data);
...
static gint find_selected_layout_idx(GtkBuilder *dialog) {
  GtkTreeSelection *selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(WID("xkb_layouts_selected")));
...
static void xkb_layouts_enable_disable_buttons(GtkBuilder *dialog) {
  ...
  GtkTreeModel *selected_layouts_model =
      gtk_tree_view_get_model(GTK_TREE_VIEW(selected_layouts_tree));
  ...
  if (disable_buttons_sensibility_update) return;

  gtk_widget_set_sensitive(
      add_layout_btn,
      (n_selected_layouts < max_selected_layouts || max_selected_layouts == 0));
  gtk_widget_set_sensitive(
      del_layout_btn,
      (n_selected_layouts > 1) && (n_selected_selected_layouts > 0));
  gtk_widget_set_sensitive(show_layout_btn, (n_selected_selected_layouts > 0));
  gtk_widget_set_sensitive(move_up_layout_btn, sidx > 0);
  gtk_widget_set_sensitive(move_down_layout_btn,
                           sidx >= 0 && sidx < (n_selected_layouts - 1));
}

static void xkb_layouts_dnd_data_get(GtkWidget *widget, GdkDragContext *dc,
  ...
  gtk_selection_data_set(selection_data, GDK_SELECTION_TYPE_INTEGER, 32,
                         (guchar *)&idx, sizeof(idx));
}

static void xkb_layouts_dnd_data_received(GtkWidget *widget, GdkDragContext *dc,
                                          gint x, gint y,
                                          GtkSelectionData *selection_data,
                                          guint info, guint t,
                                          GtkBuilder *dialog) {
  ...
  if (!gtk_tree_view_get_dest_row_at_pos(GTK_TREE_VIEW(tree_view), x, y, &path,
                                         &pos)) {
    /* Move to the very end */
    layouts_list = g_slist_append(layouts_list, g_strdup(id));
    ...
    if (sidx != didx) {
      layouts_list = g_slist_insert(layouts_list, g_strdup(id), didx);
      xkb_layouts_set_selected_list(layouts_list);
    }
  }
  g_free(id);
...
void xkb_layouts_prepare_selected_tree(GtkBuilder *dialog) {
  GtkListStore *list_store =
      gtk_list_store_new(3, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_BOOLEAN);
  GtkWidget *tree_view = WID("xkb_layouts_selected");
  GtkTreeSelection *selection;
  GtkTargetEntry self_drag_target = {"xkb_layouts_selected",
                                     GTK_TARGET_SAME_WIDGET, 0};
  GtkTreeViewColumn *desc_column;

  text_renderer = GTK_CELL_RENDERER(gtk_cell_renderer_text_new());

  desc_column = gtk_tree_view_column_new_with_attributes(
      _("Layout"), text_renderer, "text", SEL_LAYOUT_TREE_COL_DESCRIPTION,
      "sensitive", SEL_LAYOUT_TREE_COL_ENABLED, NULL);
  ...
  /* Setting up DnD */
  gtk_drag_source_set(tree_view, GDK_BUTTON1_MASK, &self_drag_target, 1,
                      GDK_ACTION_MOVE);
  gtk_drag_source_set_icon_name(tree_view, "input-keyboard");
  gtk_drag_dest_set(tree_view, GTK_DEST_DEFAULT_ALL, &self_drag_target, 1,
                    GDK_ACTION_MOVE);
...
gchar *xkb_layout_description_utf8(const gchar *visible) {
  char *l, *sl, *v, *sv;
  if (matekbd_keyboard_config_get_descriptions(config_registry, visible, &sl,
                                               &l, &sv, &v))
...
void xkb_layouts_fill_selected_tree(GtkBuilder *dialog) {
  ...
  GtkListStore *list_store = GTK_LIST_STORE(
      gtk_tree_view_get_model(GTK_TREE_VIEW(WID("xkb_layouts_selected"))));
  int counter = 0;
  ...
  for (cur_layout = layouts; cur_layout != NULL;
    ...
    gtk_list_store_set(list_store, &iter, SEL_LAYOUT_TREE_COL_DESCRIPTION,
                       utf_visible, SEL_LAYOUT_TREE_COL_ID, cur_layout->data,
...
static void add_selected_layout(GtkWidget *button, GtkBuilder *dialog) {
  xkb_layout_choose(dialog);
}

static void show_selected_layout(GtkWidget *button, GtkBuilder *dialog) {
  gint idx = find_selected_layout_idx(dialog);
...
static void remove_selected_layout(GtkWidget *button, GtkBuilder *dialog) {
  gint idx = find_selected_layout_idx(dialog);
...
static void move_up_selected_layout(GtkWidget *button, GtkBuilder *dialog) {
  gint idx = find_selected_layout_idx(dialog);
  ...
  if (idx != -1) {
    ...
    layouts_list = g_slist_remove_link(layouts_list, node2Remove);
    layouts_list = g_slist_insert(layouts_list, node2Remove->data, idx - 1);
    g_slist_free_1(node2Remove);
...
static void move_down_selected_layout(GtkWidget *button, GtkBuilder *dialog) {
  gint idx = find_selected_layout_idx(dialog);
...
void xkb_layouts_register_buttons_handlers(GtkBuilder *dialog) {
  g_signal_connect(gtk_builder_get_object(dialog, "xkb_layouts_add"), "clicked",
                   G_CALLBACK(add_selected_layout), dialog);
  g_signal_connect(gtk_builder_get_object(dialog, "xkb_layouts_show"),
                   "clicked", G_CALLBACK(show_selected_layout), dialog);
  g_signal_connect(gtk_builder_get_object(dialog, "xkb_layouts_remove"),
                   "clicked", G_CALLBACK(remove_selected_layout), dialog);
  g_signal_connect(gtk_builder_get_object(dialog, "xkb_layouts_move_up"),
                   "clicked", G_CALLBACK(move_up_selected_layout), dialog);
  g_signal_connect(gtk_builder_get_object(dialog, "xkb_layouts_move_down"),
                   "clicked", G_CALLBACK(move_down_selected_layout), dialog);
}

static void xkb_layouts_update_list(GSettings *settings, gchar *key,
                                    GtkBuilder *dialog) {
  xkb_layouts_fill_selected_tree(dialog);
  enable_disable_restoring(dialog);
}

void xkb_layouts_register_gsettings_listener(GtkBuilder *dialog) {
