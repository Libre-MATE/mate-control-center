...
static void sample_draw(GtkWidget *darea, cairo_t *cr) {
  cairo_surface_t *surface =
      g_object_get_data(G_OBJECT(darea), "sample-surface");
...
typedef enum { HINT_NONE, HINT_SLIGHT, HINT_MEDIUM, HINT_FULL } Hinting;

typedef enum { RGBA_RGB, RGBA_BGR, RGBA_VRGB, RGBA_VBGR } RgbaOrder;

static void set_fontoptions(PangoContext *context, Antialiasing antialiasing,
                            Hinting hinting) {
...
static void setup_font_sample(GtkWidget *darea, Antialiasing antialiasing,
                              Hinting hinting) {
  const char *str =
      "<span font=\"18\" style=\"normal\">abcfgop AO </span>"
  ...
  g_object_set_data_full(G_OBJECT(darea), "sample-surface", surface,
...
/*
 * Code implementing a group of radio buttons with different cairo option
 * combinations. If one of the buttons is matched by the GSettings key, we pick
 * it. Otherwise we show the group as inconsistent.
 */
...
static void font_render_load(GSettings *settings) {
  Antialiasing antialiasing;
  Hinting hinting;
...
static void font_render_changed(GSettings *settings, gchar *key,
                                gpointer user_data) {
  font_render_load(settings);
}

static void font_radio_toggled(GtkToggleButton *toggle_button, FontPair *pair) {
  if (!in_change) {
...
static void setup_font_pair(GtkWidget *radio, GtkWidget *darea,
                            Antialiasing antialiasing, Hinting hinting) {
  ...
  g_signal_connect(radio, "toggled", G_CALLBACK(font_radio_toggled), pair);
}

static void marco_titlebar_load_sensitivity(AppearanceData *data) {
...
static void marco_changed(GSettings *settings, gchar *entry,
                          gpointer user_data) {
...
static void enum_group_load(EnumGroup *group) {
  ...
  for (tmp_list = group->items; tmp_list; tmp_list = tmp_list->next) {
    ...
    if (val == item->value) gtk_toggle_button_set_active(item->widget, TRUE);
  }

  in_change = FALSE;
}

static void enum_group_changed(GSettings *settings, gchar *key,
                               gpointer user_data) {
  enum_group_load(user_data);
}

static void enum_item_toggled(GtkToggleButton *toggle_button, EnumItem *item) {
...
static EnumGroup *enum_group_create(GSettings *settings,
  ...
  while (widget) {
    ...
    g_signal_connect(item->widget, "toggled", G_CALLBACK(enum_item_toggled),
                     item);
  ...
  group->settings_signal_id = g_signal_connect(
      settings, signal_name, G_CALLBACK(enum_group_changed), group);
  g_free(signal_name);

  return group;
}

static void enum_group_destroy(gpointer data) {
 ...
 */
static void dpi_load(GSettings *settings, GtkSpinButton *spinner) {
  GdkScreen *screen;
  gint scale;
  gdouble dpi;
  ...
  if (dpi == 0) dpi = get_dpi_from_x_server();
...
static void dpi_changed(GSettings *settings, gchar *key, AppearanceData *data) {
  GtkWidget *spinner;
  GtkWidget *toggle;
  gdouble dpi;
...
static void monitors_changed(GdkScreen *screen, AppearanceData *data) {
  GtkWidget *widget;
  widget = appearance_capplet_get_widget(data, "dpi_spinner");
  dpi_load(data->font_settings, GTK_SPIN_BUTTON(widget));
}

static void dpi_value_changed(GtkSpinButton *spinner, AppearanceData *data) {
...
static gboolean dpi_value_reset(GtkSwitch *toggle, gboolean state,
                                AppearanceData *data) {
  GtkWidget *spinner;
  spinner = appearance_capplet_get_widget(data, "dpi_spinner");
...
static void cb_details_response(GtkDialog *dialog, gint response_id) {
...
static void cb_add_new_font(GtkWidget *button, AppearanceData *data) {
  GtkWidget *dialog;
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Select Font"), GET_WINDOW("appearance_window"), action, _("_Cancel"),
...
static void cb_show_details(GtkWidget *button, AppearanceData *data) {
  if (!data->font_details) {
    ...
    g_signal_connect(spinner, "value-changed", G_CALLBACK(dpi_value_changed),
                     data);
    g_signal_connect(toggle, "state-set", G_CALLBACK(dpi_value_reset), data);
    ...
    setup_font_sample(
        appearance_capplet_get_widget(data, "antialias_none_sample"),
        ANTIALIAS_NONE, HINT_SLIGHT);
    setup_font_sample(
        appearance_capplet_get_widget(data, "antialias_grayscale_sample"),
        ANTIALIAS_GRAYSCALE, HINT_SLIGHT);
    setup_font_sample(
        appearance_capplet_get_widget(data, "antialias_subpixel_sample"),
        ANTIALIAS_RGBA, HINT_SLIGHT);

    group = enum_group_create(
        data->font_settings, FONT_ANTIALIASING_KEY,
        appearance_capplet_get_widget(data, "antialias_none_radio"),
        ANTIALIAS_NONE,
        appearance_capplet_get_widget(data, "antialias_grayscale_radio"),
        ANTIALIAS_GRAYSCALE,
        appearance_capplet_get_widget(data, "antialias_subpixel_radio"),
        ANTIALIAS_RGBA, NULL);
    data->font_groups = g_slist_prepend(data->font_groups, group);

    setup_font_sample(appearance_capplet_get_widget(data, "hint_none_sample"),
                      ANTIALIAS_RGBA, HINT_NONE);
    setup_font_sample(appearance_capplet_get_widget(data, "hint_slight_sample"),
                      ANTIALIAS_RGBA, HINT_SLIGHT);
    setup_font_sample(appearance_capplet_get_widget(data, "hint_medium_sample"),
                      ANTIALIAS_RGBA, HINT_MEDIUM);
    setup_font_sample(appearance_capplet_get_widget(data, "hint_full_sample"),
                      ANTIALIAS_RGBA, HINT_FULL);

    group = enum_group_create(
        data->font_settings, FONT_HINTING_KEY,
    ...
    gtk_image_set_from_file(
        GTK_IMAGE(appearance_capplet_get_widget(data, "subpixel_bgr_image")),
        MATECC_PIXMAP_DIR "/subpixel-bgr.png");
    gtk_image_set_from_file(
        GTK_IMAGE(appearance_capplet_get_widget(data, "subpixel_vrgb_image")),
        MATECC_PIXMAP_DIR "/subpixel-vrgb.png");
    gtk_image_set_from_file(
        GTK_IMAGE(appearance_capplet_get_widget(data, "subpixel_vbgr_image")),
        MATECC_PIXMAP_DIR "/subpixel-vbgr.png");

    group = enum_group_create(
        data->font_settings, FONT_RGBA_ORDER_KEY,
    ...
    g_signal_connect(data->font_details, "delete_event", G_CALLBACK(gtk_true),
                     NULL);
  }
...
void font_init(AppearanceData *data) {
  ...
  g_settings_bind(data->interface_settings, GTK_FONT_KEY, G_OBJECT(widget),
                  "font-name", G_SETTINGS_BIND_DEFAULT);

  widget = appearance_capplet_get_widget(data, "document_font");
  g_settings_bind(data->interface_settings, DOCUMENT_FONT_KEY, G_OBJECT(widget),
                  "font-name", G_SETTINGS_BIND_DEFAULT);

  widget = appearance_capplet_get_widget(data, "desktop_font");

  if (data->caja_settings)
    g_settings_bind(data->caja_settings, DESKTOP_FONT_KEY, G_OBJECT(widget),
                    "font-name", G_SETTINGS_BIND_DEFAULT);
  else
    gtk_widget_set_sensitive(widget, FALSE);

  widget = appearance_capplet_get_widget(data, "window_title_font");
  g_settings_bind(data->marco_settings, WINDOW_TITLE_FONT_KEY, G_OBJECT(widget),
                  "font-name", G_SETTINGS_BIND_DEFAULT);

  widget = appearance_capplet_get_widget(data, "monospace_font");
  g_settings_bind(data->interface_settings, MONOSPACE_FONT_KEY,
                  G_OBJECT(widget), "font-name", G_SETTINGS_BIND_DEFAULT);

  g_signal_connect(data->marco_settings,
                   "changed::" WINDOW_TITLE_USES_SYSTEM_KEY,
                   G_CALLBACK(marco_changed), data);

  g_signal_connect(appearance_capplet_get_widget(data, "add_new_font"),
                   "clicked", G_CALLBACK(cb_add_new_font), data);
  ...
  g_signal_connect(data->font_settings, "changed",
                   G_CALLBACK(font_render_changed), NULL);
