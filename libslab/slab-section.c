...
static void slab_section_init(SlabSection *section) {
  section->title = NULL;
  section->contents = NULL;
}

static void slab_section_finalize(GObject *obj) {
  g_assert(IS_SLAB_SECTION(obj));
  (*G_OBJECT_CLASS(slab_section_parent_class)->finalize)(obj);
}

static void slab_section_set_title_color(GtkWidget *widget) {
  switch (SLAB_SECTION(widget)->style) {
    case Style1:
      gtk_widget_modify_fg(
          SLAB_SECTION(widget)->title, GTK_STATE_NORMAL,
...
static void slab_section_style_set(GtkWidget *widget, GtkStyle *prev_style,
                                   gpointer user_data) {
...
/*
gboolean
slab_section_expose_event (GtkWidget * widget, GdkEventExpose * event, gpointer
data)
{
        gdk_draw_rectangle (widget->window,
widget->style->light_gc[GTK_STATE_SELECTED], TRUE, widget->allocation.x,
widget->allocation.y, widget->allocation.width + 40, widget->allocation.height);
...
void slab_section_set_selected(SlabSection *section, gboolean selected) {
  if (selected == section->selected) return;
          ...
          if(selected)
          {
                  section->expose_handler_id =
                          g_signal_connect (section, "expose-event",
                                            G_CALLBACK
     (slab_section_expose_event), NULL);
          }
          else
          {
                  g_signal_handler_disconnect(section,
     section->expose_handler_id);
          }
  */
...
GtkWidget *slab_section_new_with_markup(const gchar *title_markup,
                                        SlabStyle style) {
  SlabSection *section;
  ...
  switch (style) {
    case Style1:
      ...
      break;
    case Style2:
      gtk_widget_set_margin_top(GTK_WIDGET(section->childbox),
                                SLAB_TOP_PADDING);
      gtk_widget_set_margin_bottom(GTK_WIDGET(section->childbox),
                                   SLAB_BOTTOM_PADDING);
      gtk_widget_set_margin_start(GTK_WIDGET(section->childbox),
                                  SLAB_LEFT_PADDING);
  ...
  gtk_box_pack_start(GTK_BOX(section), GTK_WIDGET(section->childbox), TRUE,
                     TRUE, 0);
  ...
  g_signal_connect(section, "style-set", G_CALLBACK(slab_section_style_set),
                   NULL);
...
GtkWidget *slab_section_new(const gchar *title, SlabStyle style) {
  ...
  markup =
      g_strdup_printf("<span size=\"large\" weight=\"bold\">%s</span>", title);
