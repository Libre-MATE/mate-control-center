...
static void about_me_destroy(void) {
  if (me->dialog) g_object_unref(me->dialog);
  if (me->image) g_object_unref(me->image);
...
static void about_me_load_photo(MateAboutMe *me) {
  gchar *file = NULL;
  GError *error = NULL;
#if HAVE_ACCOUNTSSERVICE
  ...
  if (me->image != NULL) {
    ...
    e_image_chooser_set_from_file(E_IMAGE_CHOOSER(me->image_chooser),
                                  me->person);
    g_error_free(error);
  }
  g_free(file);
...
static void about_me_update_photo(MateAboutMe *me) {
  gchar *file;
  GError *error = NULL;
  ...
  if (me->image_changed && me->have_image) {
    ...
    e_image_chooser_get_image_data(E_IMAGE_CHOOSER(me->image_chooser),
                                   (char **)&data, &length);
    ...
    if (do_scale) {
      ...
      scaled = gdk_pixbuf_scale_simple(pixbuf, width * scale, height * scale,
                                       GDK_INTERP_BILINEAR);
      gdk_pixbuf_save_to_buffer(scaled, &scaled_data, &scaled_length, "png",
                                NULL, "compression", "9", NULL);
...
static void about_me_load_info(MateAboutMe *me) {
  set_fingerprint_label(me->enable_fingerprint_button,
                        me->disable_fingerprint_button);
}

static void about_me_update_preview(GtkFileChooser *chooser, MateAboutMe *me) {
  gchar *uri;
  ...
  if (uri) {
    ...
    if (!me->thumbs)
      me->thumbs = mate_desktop_thumbnail_factory_new(
    ...
    if (pixbuf != NULL) {
      ...
      gtk_image_set_from_icon_name(GTK_IMAGE(image), "dialog-question",
...
static void about_me_image_clicked_cb(GtkWidget *button, MateAboutMe *me) {
  ...
  chooser_dialog = GTK_FILE_CHOOSER(gtk_file_chooser_dialog_new(
      _("Select Image"),
      GTK_WINDOW(gtk_builder_get_object(dialog, "about-me-dialog")),
      GTK_FILE_CHOOSER_ACTION_OPEN, _("No Image"), GTK_RESPONSE_NO,
      "gtk-cancel", GTK_RESPONSE_CANCEL, "gtk-open", GTK_RESPONSE_ACCEPT,
      NULL));
...
static void about_me_image_changed_cb(GtkWidget *widget, MateAboutMe *me) {
  me->have_image = TRUE;
  me->image_changed = TRUE;
  about_me_update_photo(me);
}
...
static void about_me_icon_theme_changed(GtkWindow *window,
                                        GtkIconTheme *theme) {
  GtkIconInfo *icon;
...
static void about_me_button_clicked_cb(GtkDialog *dialog, gint response_id,
                                       MateAboutMe *me) {
...
static void about_me_passwd_clicked_cb(GtkWidget *button, MateAboutMe *me) {
  GtkBuilder *dialog;

  dialog = me->dialog;
  mate_about_me_password(
      GTK_WINDOW(gtk_builder_get_object(dialog, "about-me-dialog")));
}

static void about_me_fingerprint_button_clicked_cb(GtkWidget *button,
                                                   MateAboutMe *me) {
  fingerprint_button_clicked(me->dialog, me->enable_fingerprint_button,
                             me->disable_fingerprint_button);
}

#if HAVE_ACCOUNTSSERVICE
static void on_user_is_loaded_changed(ActUser *user, GParamSpec *pspec,
                                      MateAboutMe *me) {
  if (act_user_is_loaded(user)) {
    about_me_load_photo(me);
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(user), G_CALLBACK(on_user_is_loaded_changed), me);
  }
}
#endif

static gint about_me_setup_dialog(void) {
  GtkWidget *widget;
  ...
  if (gtk_builder_add_from_resource(
          dialog, "/org/mate/mcc/am/mate-about-me-dialog.ui", &error) == 0) {
  ...
  gtk_container_add(
      GTK_CONTAINER(gtk_builder_get_object(dialog, "button-image")),
      me->image_chooser);
  ...
  g_signal_connect_object(me->theme, "changed",
                          G_CALLBACK(about_me_icon_theme_changed), main_dialog,
                          G_CONNECT_SWAPPED);
  ...
  g_signal_connect(me->user, "notify::is-loaded",
                   G_CALLBACK(on_user_is_loaded_changed), me);
#endif
  /* Contact Tab */
  about_me_load_photo(me);
  ...
  widget = WID("fullname");
  str =
      g_strdup_printf("<b><span size=\"xx-large\">%s</span></b>", me->username);
  ...
  widget = WID("password");
  g_signal_connect(widget, "clicked", G_CALLBACK(about_me_passwd_clicked_cb),
                   me);

  widget = WID("button-image");
  g_signal_connect(widget, "clicked", G_CALLBACK(about_me_image_clicked_cb),
                   me);
