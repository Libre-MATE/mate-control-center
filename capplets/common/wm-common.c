...
static char *wm_common_get_window_manager_property(Atom atom) {
  ...
  val = NULL;
  result = XGetWindowProperty(GDK_DISPLAY_XDISPLAY(display), wm_window, atom, 0,
                              G_MAXLONG, False, utf8_string, &type, &format,
                              &nitems, &bytes_after, (guchar **)&val);
...
char *wm_common_get_current_window_manager(void) {
...
char **wm_common_get_current_keybindings(void) {
  ...
  if (keybindings) {
    ...
    for (p = results; *p; p++) g_strstrip(*p);
    g_free(keybindings);
  } else {
...
static void update_wm_window(void) {
  ...
  XGetWindowProperty(GDK_DISPLAY_XDISPLAY(display), GDK_ROOT_WINDOW(),
                     gdk_x11_get_xatom_by_name("_NET_SUPPORTING_WM_CHECK"), 0,
                     G_MAXLONG, False, XA_WINDOW, &type, &format, &nitems,
  ...
  gdk_x11_display_error_trap_push(display);
  XSelectInput(GDK_DISPLAY_XDISPLAY(display), *xwindow,
...
static GdkFilterReturn wm_window_event_filter(GdkXEvent *xev, GdkEvent *event,
                                              gpointer data) {
  WMCallbackData *ncb_data = (WMCallbackData *)data;
  XEvent *xevent = (XEvent *)xev;

  if ((xevent->type == DestroyNotify && wm_window != None &&
       xevent->xany.window == wm_window) ||
      (xevent->type == PropertyNotify &&
       xevent->xany.window == GDK_ROOT_WINDOW() &&
       xevent->xproperty.atom ==
           (gdk_x11_get_xatom_by_name("_NET_SUPPORTING_WM_CHECK"))) ||
      (xevent->type == PropertyNotify && wm_window != None &&
       xevent->xany.window == wm_window &&
...
void wm_common_register_window_manager_change(GFunc func, gpointer data) {
  ...
  XSelectInput(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
               GDK_ROOT_WINDOW(), PropertyChangeMask);
  XSync(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), False);
}

void wm_common_update_window() { update_wm_window(); }
