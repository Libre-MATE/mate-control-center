...
static void model_key_changed(GSettings *settings, gchar *key,
                              GtkBuilder *dialog) {
...
static void setup_model_entry(GtkBuilder *dialog) {
  gchar *value;
  ...
  g_signal_connect(xkb_kbd_settings, "changed::model",
                   G_CALLBACK(model_key_changed), dialog);
}

static void cleanup_xkb_tabs(GtkBuilder *dialog) {
...
static void reset_to_defaults(GtkWidget *button, GtkBuilder *dialog) {
...
static void chk_separate_group_per_window_toggled(GSettings *settings,
                                                  gchar *key,
                                                  GtkBuilder *dialog) {
...
static void chk_new_windows_inherit_layout_toggled(
    GtkWidget *chk_new_windows_inherit_layout, GtkBuilder *dialog) {
  xkb_save_default_group(gtk_toggle_button_get_active(
                             GTK_TOGGLE_BUTTON(chk_new_windows_inherit_layout))
                             ? -1
                             : 0);
}

void setup_xkb_tabs(GtkBuilder *dialog) {
  ...
  g_settings_bind(xkb_general_settings, "group-per-window",
                  WID("chk_separate_group_per_window"), "active",
                  G_SETTINGS_BIND_DEFAULT);

  g_signal_connect(xkb_general_settings, "changed::group-per-window",
                   G_CALLBACK(chk_separate_group_per_window_toggled), dialog);
  ...
  g_signal_connect(gtk_builder_get_object(dialog, "xkb_reset_to_defaults"),
                   "clicked", G_CALLBACK(reset_to_defaults), dialog);

  g_signal_connect(chk_new_windows_inherit_layout, "toggled",
                   (GCallback)chk_new_windows_inherit_layout_toggled, dialog);

  g_signal_connect_swapped(gtk_builder_get_object(dialog, "xkb_layout_options"),
                           "clicked", G_CALLBACK(xkb_options_popup_dialog),
                           dialog);

  g_signal_connect_swapped(gtk_builder_get_object(dialog, "xkb_model_pick"),
                           "clicked", G_CALLBACK(choose_model), dialog);
  ...
  g_signal_connect(gtk_builder_get_object(dialog, "keyboard_dialog"), "destroy",
                   G_CALLBACK(cleanup_xkb_tabs), dialog);

  enable_disable_restoring(dialog);
}

void enable_disable_restoring(GtkBuilder *dialog) {
...
static void xkb_save_gslist_as_strv(gchar *schema, gchar *key, GSList *list) {
  GSettings *settings;
  GArray *array;
  GSList *l;
...
void xkb_layouts_set_selected_list(GSList *list) {
  xkb_save_gslist_as_strv(XKB_KBD_SCHEMA, "layouts", list);
}

void xkb_options_set_selected_list(GSList *list) {
