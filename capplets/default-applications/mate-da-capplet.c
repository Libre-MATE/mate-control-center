...
/* For combo box */
enum { SURFACE_COL, TEXT_COL, ID_COL, ICONAME_COL, N_COLUMNS };

static void set_changed(GtkComboBox* combo, MateDACapplet* capplet, GList* list,
                        gint type) {
  guint index;
  GAppInfo* item;
  ...
  if (index < g_list_length(list)) {
    ...
    switch (type) {
      case DA_TYPE_WEB_BROWSER:
        ...
        g_app_info_set_as_default_for_type(item, "x-scheme-handler/https",
                                           NULL);
        g_app_info_set_as_default_for_type(item, "text/html", NULL);
        /* about:config is used by firefox and others */
        g_app_info_set_as_default_for_type(item, "x-scheme-handler/about",
                                           NULL);
        break;

      case DA_TYPE_EMAIL:
        g_app_info_set_as_default_for_type(item, "x-scheme-handler/mailto",
                                           NULL);
        g_app_info_set_as_default_for_type(item, "application/x-extension-eml",
                                           NULL);
      ...
      case DA_TYPE_WORD:
        g_app_info_set_as_default_for_type(
            item, "application/vnd.oasis.opendocument.text", NULL);
      ...
      case DA_TYPE_SPREADSHEET:
        g_app_info_set_as_default_for_type(
            item, "application/vnd.oasis.opendocument.spreadsheet", NULL);
        g_app_info_set_as_default_for_type(item, "application/vnd.ms-excel",
                                           NULL);
        g_app_info_set_as_default_for_type(
            item,
      ...
      case DA_TYPE_TERMINAL:
        g_settings_set_string(capplet->terminal_settings, TERMINAL_KEY,
                              g_app_info_get_executable(item));
        break;

      case DA_TYPE_VISUAL:
        g_settings_set_string(capplet->visual_settings, VISUAL_KEY,
                              g_app_info_get_executable(item));
        break;

      case DA_TYPE_MOBILITY:
        g_settings_set_string(capplet->mobility_settings, MOBILITY_KEY,
                              g_app_info_get_executable(item));
        break;

      case DA_TYPE_CALCULATOR:
        g_settings_set_string(capplet->calculator_settings, CALCULATOR_KEY,
                              g_app_info_get_executable(item));
        break;
      ...
      case DA_TYPE_MESSENGER:
        ...
        g_settings_set_string(capplet->messenger_settings, MESSENGER_KEY,
                              g_app_info_get_executable(item));
...
static void close_cb(GtkWidget* window, gint response, MateDACapplet* capplet) {
  if (response == GTK_RESPONSE_HELP) {
    capplet_help(GTK_WINDOW(window), "prefs-preferredapps");
  } else {
    set_changed(GTK_COMBO_BOX(capplet->web_combo_box), capplet,
                capplet->web_browsers, DA_TYPE_WEB_BROWSER);
    set_changed(GTK_COMBO_BOX(capplet->mail_combo_box), capplet,
                capplet->mail_readers, DA_TYPE_EMAIL);
    set_changed(GTK_COMBO_BOX(capplet->file_combo_box), capplet,
                capplet->file_managers, DA_TYPE_FILE);
    set_changed(GTK_COMBO_BOX(capplet->text_combo_box), capplet,
                capplet->text_editors, DA_TYPE_TEXT);
    set_changed(GTK_COMBO_BOX(capplet->media_combo_box), capplet,
                capplet->media_players, DA_TYPE_MEDIA);
    set_changed(GTK_COMBO_BOX(capplet->video_combo_box), capplet,
                capplet->video_players, DA_TYPE_VIDEO);
    set_changed(GTK_COMBO_BOX(capplet->term_combo_box), capplet,
                capplet->terminals, DA_TYPE_TERMINAL);
    set_changed(GTK_COMBO_BOX(capplet->visual_combo_box), capplet,
                capplet->visual_ats, DA_TYPE_VISUAL);
    set_changed(GTK_COMBO_BOX(capplet->mobility_combo_box), capplet,
                capplet->mobility_ats, DA_TYPE_MOBILITY);
    set_changed(GTK_COMBO_BOX(capplet->image_combo_box), capplet,
                capplet->image_viewers, DA_TYPE_IMAGE);
    set_changed(GTK_COMBO_BOX(capplet->document_combo_box), capplet,
                capplet->document_viewers, DA_TYPE_DOCUMENT);
    set_changed(GTK_COMBO_BOX(capplet->word_combo_box), capplet,
                capplet->word_editors, DA_TYPE_WORD);
    set_changed(GTK_COMBO_BOX(capplet->spreadsheet_combo_box), capplet,
                capplet->spreadsheet_editors, DA_TYPE_SPREADSHEET);
    set_changed(GTK_COMBO_BOX(capplet->calculator_combo_box), capplet,
                capplet->calculators, DA_TYPE_CALCULATOR);
    set_changed(GTK_COMBO_BOX(capplet->messenger_combo_box), capplet,
                capplet->messengers, DA_TYPE_MESSENGER);
...
static void terminal_combo_changed_cb(GtkComboBox* combo,
                                      MateDACapplet* capplet) {
  set_changed(combo, capplet, capplet->terminals, DA_TYPE_TERMINAL);
}

static void visual_combo_changed_cb(GtkComboBox* combo,
                                    MateDACapplet* capplet) {
  set_changed(combo, capplet, capplet->visual_ats, DA_TYPE_VISUAL);
}

static void mobility_combo_changed_cb(GtkComboBox* combo,
                                      MateDACapplet* capplet) {
...
static void document_combo_changed_cb(GtkComboBox* combo,
                                      MateDACapplet* capplet) {
  set_changed(combo, capplet, capplet->document_viewers, DA_TYPE_DOCUMENT);
}

static void word_combo_changed_cb(GtkComboBox* combo, MateDACapplet* capplet) {
  set_changed(combo, capplet, capplet->word_editors, DA_TYPE_WORD);
}

static void spreadsheet_combo_changed_cb(GtkComboBox* combo,
                                         MateDACapplet* capplet) {
  set_changed(combo, capplet, capplet->spreadsheet_editors,
              DA_TYPE_SPREADSHEET);
}

static void calculator_combo_changed_cb(GtkComboBox* combo,
                                        MateDACapplet* capplet) {
  set_changed(combo, capplet, capplet->calculators, DA_TYPE_CALCULATOR);
}

static void messenger_combo_changed_cb(GtkComboBox* combo,
                                       MateDACapplet* capplet) {
  set_changed(combo, capplet, capplet->messengers, DA_TYPE_MESSENGER);
}

static void refresh_combo_box_icons(GtkIconTheme* theme, GtkComboBox* combo_box,
                                    GList* app_list) {
  GtkTreeIter iter;
  GtkTreeModel* model;
  gboolean valid;
  ...
  while (valid) {
    ...
    surface =
        gtk_icon_theme_load_surface(theme, icon_name, 22, scale_factor, NULL,
                                    GTK_ICON_LOOKUP_FORCE_SIZE, NULL);

    gtk_list_store_set(GTK_LIST_STORE(model), &iter, SURFACE_COL, surface, -1);
...
static void theme_changed_cb(GtkIconTheme* theme, MateDACapplet* capplet) {
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->web_combo_box),
                          capplet->web_browsers);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->mail_combo_box),
                          capplet->mail_readers);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->media_combo_box),
                          capplet->media_players);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->video_combo_box),
                          capplet->video_players);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->term_combo_box),
                          capplet->terminals);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->visual_combo_box),
                          capplet->visual_ats);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->mobility_combo_box),
                          capplet->mobility_ats);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->file_combo_box),
                          capplet->file_managers);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->text_combo_box),
                          capplet->text_editors);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->document_combo_box),
                          capplet->document_viewers);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->word_combo_box),
                          capplet->word_editors);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->spreadsheet_combo_box),
                          capplet->spreadsheet_editors);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->calculator_combo_box),
                          capplet->calculators);
  refresh_combo_box_icons(theme, GTK_COMBO_BOX(capplet->messenger_combo_box),
                          capplet->messengers);
}

static void screen_changed_cb(GtkWidget* widget, GdkScreen* screen,
                              MateDACapplet* capplet) {
  GtkIconTheme* theme;
  ...
  if (capplet->icon_theme != NULL) {
    g_signal_handlers_disconnect_by_func(capplet->icon_theme, theme_changed_cb,
                                         capplet);
  }
...
static void fill_combo_box(GtkIconTheme* theme, GtkComboBox* combo_box,
                           GList* app_list, gchar* mime) {
  guint index = 0;
  GList* entry;
  GtkTreeModel* model;
  ...
  if (g_strcmp0(mime, "terminal") == 0) {
    ...
    gchar* default_terminal =
        g_settings_get_string(terminal_settings, TERMINAL_KEY);
    ...
    gchar* default_mobility =
        g_settings_get_string(mobility_settings, MOBILITY_KEY);
    for (entry = app_list; entry != NULL; entry = g_list_next(entry)) {
      GAppInfo* item = (GAppInfo*)entry->data;
    ...
    gchar* default_calculator =
        g_settings_get_string(calculator_settings, CALCULATOR_KEY);
    for (entry = app_list; entry != NULL; entry = g_list_next(entry)) {
      GAppInfo* item = (GAppInfo*)entry->data;
    ...
    gchar* default_messenger =
        g_settings_get_string(messenger_settings, MESSENGER_KEY);
    for (entry = app_list; entry != NULL; entry = g_list_next(entry)) {
      GAppInfo* item = (GAppInfo*)entry->data;
  ...
  model = GTK_TREE_MODEL(gtk_list_store_new(4, CAIRO_GOBJECT_TYPE_SURFACE,
  ...
  for (entry = app_list; entry != NULL; entry = g_list_next(entry)) {
    ...
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, SURFACE_COL, surface,
                       TEXT_COL, g_app_info_get_display_name(item), ID_COL,
                       g_app_info_get_id(item), ICONAME_COL, icon_name, -1);
...
static GList* fill_list_from_desktop_file(GList* app_list,
...
#define get_widget(name) GTK_WIDGET(gtk_builder_get_object(builder, name))
  ...
  capplet->builder = builder = gtk_builder_new_from_resource(
      "/org/mate/mcc/da/mate-default-applications-properties.ui");
  ...
  capplet->mobility_startup_checkbutton =
      get_widget("mobility_start_checkbutton");

  g_signal_connect(capplet->window, "screen-changed",
                   G_CALLBACK(screen_changed_cb), capplet);
  ...
  capplet->mail_readers =
      g_app_info_get_all_for_type("x-scheme-handler/mailto");
  ...
  capplet->spreadsheet_editors =
      g_app_info_get_all_for_type("application/vnd.ms-excel");
  ...
  for (const gchar* c = *sys_config_dirs; c; c = *++sys_config_dirs) {
    ...
    capplet->visual_ats =
        fill_list_from_desktop_file(capplet->visual_ats, path);
    g_free(path);
  }
  capplet->visual_ats = g_list_reverse(capplet->visual_ats);

  capplet->mobility_ats = NULL;
  capplet->mobility_ats = fill_list_from_desktop_file(
      capplet->mobility_ats, APPLICATIONSDIR "/dasher.desktop");
  capplet->mobility_ats = fill_list_from_desktop_file(
      capplet->mobility_ats, APPLICATIONSDIR "/gok.desktop");
  capplet->mobility_ats = fill_list_from_desktop_file(
      capplet->mobility_ats, APPLICATIONSDIR "/onboard.desktop");
  ...
  for (entry = all_apps; entry != NULL; entry = g_list_next(entry)) {
    ...
    if (g_desktop_app_info_get_categories(item) != NULL &&
        g_strrstr(g_desktop_app_info_get_categories(item),
                  "TerminalEmulator")) {
  ...
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->web_combo_box),
                 capplet->web_browsers, "x-scheme-handler/http");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->mail_combo_box),
                 capplet->mail_readers, "x-scheme-handler/mailto");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->term_combo_box),
                 capplet->terminals, "terminal");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->media_combo_box),
                 capplet->media_players, "audio/x-vorbis+ogg");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->video_combo_box),
                 capplet->video_players, "video/x-ogm+ogg");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->image_combo_box),
                 capplet->image_viewers, "image/png");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->text_combo_box),
                 capplet->text_editors, "text/plain");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->file_combo_box),
                 capplet->file_managers, "inode/directory");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->visual_combo_box),
                 capplet->visual_ats, "visual");
  fill_combo_box(capplet->icon_theme,
                 GTK_COMBO_BOX(capplet->mobility_combo_box),
                 capplet->mobility_ats, "mobility");
  fill_combo_box(capplet->icon_theme,
                 GTK_COMBO_BOX(capplet->document_combo_box),
                 capplet->document_viewers, "application/pdf");
  fill_combo_box(capplet->icon_theme, GTK_COMBO_BOX(capplet->word_combo_box),
                 capplet->word_editors,
                 "application/vnd.oasis.opendocument.text");
  fill_combo_box(capplet->icon_theme,
                 GTK_COMBO_BOX(capplet->spreadsheet_combo_box),
                 capplet->spreadsheet_editors,
                 "application/vnd.oasis.opendocument.spreadsheet");
  fill_combo_box(capplet->icon_theme,
                 GTK_COMBO_BOX(capplet->calculator_combo_box),
                 capplet->calculators, "calculator");
  fill_combo_box(capplet->icon_theme,
                 GTK_COMBO_BOX(capplet->messenger_combo_box),
                 capplet->messengers, "messenger");

  g_signal_connect(capplet->web_combo_box, "changed",
                   G_CALLBACK(web_combo_changed_cb), capplet);
  g_signal_connect(capplet->mail_combo_box, "changed",
                   G_CALLBACK(mail_combo_changed_cb), capplet);
  g_signal_connect(capplet->term_combo_box, "changed",
                   G_CALLBACK(terminal_combo_changed_cb), capplet);
  g_signal_connect(capplet->media_combo_box, "changed",
                   G_CALLBACK(media_combo_changed_cb), capplet);
  g_signal_connect(capplet->video_combo_box, "changed",
                   G_CALLBACK(video_combo_changed_cb), capplet);
  g_signal_connect(capplet->visual_combo_box, "changed",
                   G_CALLBACK(visual_combo_changed_cb), capplet);
  g_signal_connect(capplet->mobility_combo_box, "changed",
                   G_CALLBACK(mobility_combo_changed_cb), capplet);
  g_signal_connect(capplet->image_combo_box, "changed",
                   G_CALLBACK(image_combo_changed_cb), capplet);
  g_signal_connect(capplet->text_combo_box, "changed",
                   G_CALLBACK(text_combo_changed_cb), capplet);
  g_signal_connect(capplet->file_combo_box, "changed",
                   G_CALLBACK(file_combo_changed_cb), capplet);
  g_signal_connect(capplet->document_combo_box, "changed",
                   G_CALLBACK(document_combo_changed_cb), capplet);
  g_signal_connect(capplet->word_combo_box, "changed",
                   G_CALLBACK(word_combo_changed_cb), capplet);
  g_signal_connect(capplet->spreadsheet_combo_box, "changed",
                   G_CALLBACK(spreadsheet_combo_changed_cb), capplet);
  g_signal_connect(capplet->calculator_combo_box, "changed",
                   G_CALLBACK(calculator_combo_changed_cb), capplet);
  g_signal_connect(capplet->messenger_combo_box, "changed",
                   G_CALLBACK(messenger_combo_changed_cb), capplet);

  g_settings_bind(capplet->mobility_settings, MOBILITY_STARTUP_KEY,
                  capplet->mobility_startup_checkbutton, "active",
                  G_SETTINGS_BIND_DEFAULT);
  g_settings_bind(capplet->visual_startup_settings, VISUAL_STARTUP_KEY,
...
int main(int argc, char** argv) {
  ...
  GOptionEntry option_entries[] = {
      {"show-page", 'p', G_OPTION_FLAG_IN_MAIN, G_OPTION_ARG_STRING,
       &start_page,
  ...
  GOptionContext* context =
      g_option_context_new(_("- MATE Default Applications"));
