...
static void set_have_icons(AppearanceData *data, gboolean value) {
  ...
  for (name = menu_item_names; *name != NULL; name++) {
    GtkImageMenuItem *item =
        GTK_IMAGE_MENU_ITEM(appearance_capplet_get_widget(data, *name));
    GtkWidget *image;
...
static void menus_have_icons_cb(GSettings *settings, gchar *key,
                                AppearanceData *data) {
...
void ui_init(AppearanceData *data) {
  ...
  g_settings_bind(data->interface_settings, MENU_ICONS_KEY,
  ...
  set_have_icons(
      data, g_settings_get_boolean(data->interface_settings, MENU_ICONS_KEY));

  g_settings_bind(data->interface_settings, BUTTON_ICONS_KEY,
                  gtk_builder_get_object(data->ui, "button_icons_toggle"),
                  "active", G_SETTINGS_BIND_DEFAULT);

  g_settings_bind(
      data->filechooser_settings, FILECHOOSER_HIDDEN_ITEMS_KEY,
      gtk_builder_get_object(data->ui, "filechooser_hidden_items_toggle"),
      "active", G_SETTINGS_BIND_DEFAULT);

  g_settings_bind(
      data->filechooser_settings, FILECHOOSER_SIZE_COL_KEY,
      gtk_builder_get_object(data->ui, "filechooser_size_column_toggle"),
      "active", G_SETTINGS_BIND_DEFAULT);

#if GTK_CHECK_VERSION(3, 24, 12)
  g_settings_bind(
      data->filechooser_settings, FILECHOOSER_TYPE_COL_KEY,
      gtk_builder_get_object(data->ui, "filechooser_type_column_toggle"),
      "active", G_SETTINGS_BIND_DEFAULT);
#endif /* GTK_CHECK_VERSION(3, 24, 12) */

  g_settings_bind(data->filechooser_settings, FILECHOOSER_DIR_FIRST_KEY,
                  gtk_builder_get_object(
                      data->ui, "filechooser_sort_directories_first_toggle"),
                  "active", G_SETTINGS_BIND_DEFAULT);
}
