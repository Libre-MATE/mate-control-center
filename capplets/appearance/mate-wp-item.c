...
static void set_bg_properties(MateWPItem *item) {
  if (item->filename) mate_bg_set_filename(item->bg, item->filename);
...
void mate_wp_item_update(MateWPItem *item) {
  ...
  if (item->pcolor != NULL) gdk_rgba_free(item->pcolor);

  if (item->scolor != NULL) gdk_rgba_free(item->scolor);

  item->pcolor = gdk_rgba_copy(&color1);
  item->scolor = gdk_rgba_copy(&color2);
}

MateWPItem *mate_wp_item_new(const gchar *filename, GHashTable *wallpapers,
  ...
  if (item->fileinfo != NULL && item->fileinfo->mime_type != NULL &&
    ...
    if (g_utf8_validate(item->fileinfo->name, -1, NULL))
      ...
      item->name =
          g_filename_to_utf8(item->fileinfo->name, -1, NULL, NULL, NULL);
...
void mate_wp_item_free(MateWPItem *item) {
  ...
  if (item->bg) g_object_unref(item->bg);
...
static GdkPixbuf *add_slideshow_frame(GdkPixbuf *pixbuf) {
  ...
  gdk_pixbuf_composite(sheet, tmp, 6, 6, w, h, 6.0, 6.0, 1.0, 1.0,
                       GDK_INTERP_NEAREST, 255);
  gdk_pixbuf_composite(sheet, tmp, 3, 3, w, h, 3.0, 3.0, 1.0, 1.0,
                       GDK_INTERP_NEAREST, 255);
  gdk_pixbuf_composite(pixbuf, tmp, 0, 0, w, h, 0.0, 0.0, 1.0, 1.0,
                       GDK_INTERP_NEAREST, 255);
...
GdkPixbuf *mate_wp_item_get_frame_thumbnail(MateWPItem *item,
                                            ...
                                            int width, int height, gint frame) {
  GdkPixbuf *pixbuf = NULL;

  set_bg_properties(item);

  if (frame != -1)
    pixbuf = mate_bg_create_frame_thumbnail(
        item->bg, thumbs, gdk_screen_get_default(), width, height, frame);
  else
    pixbuf = mate_bg_create_thumbnail(item->bg, thumbs,
  ...
  mate_bg_get_image_size(item->bg, thumbs, width, height, &item->width,
                         &item->height);
...
void mate_wp_item_update_description(MateWPItem *item) {
  ...
  if (!strcmp(item->filename, "(none)")) {
    ...
    if (!item->artist || item->artist[0] == 0 ||
        !g_strcmp0(item->artist, "(none)"))
    ...
    if (mate_bg_has_multiple_sizes(item->bg))
      ...
      size = g_strdup_printf(_("%d %s by %d %s"), item->width,
    ...
    if (description && size) {
       ...
       * Artist: wallpaper author
       */
      item->description = g_markup_printf_escaped(
          _("<b>%s</b>\n"
            "%s\n"
            "Folder: %s\n"
            "Artist: %s"),
          item->name, _("Image missing"), dirname, artist);
    }
