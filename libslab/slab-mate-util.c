...
gboolean open_desktop_item_exec(MateDesktopItem *desktop_item) {
  ...
  if (!desktop_item) return FALSE;

  mate_desktop_item_launch(desktop_item, NULL,
...
gboolean open_desktop_item_help(MateDesktopItem *desktop_item) {
  ...
  if (doc_path) {
    ...
    error = NULL;
    if (!gtk_show_uri_on_window(NULL, help_uri, gtk_get_current_event_time(),
                                &error)) {
...
void copy_file(const gchar *src_uri, const gchar *dst_uri) {
  ...
  res = g_file_copy(src, dst, G_FILE_COPY_NONE, NULL, NULL, NULL, &error);

  if (!res) {
    g_warning("error copying [%s] to [%s]: %s.", src_uri, dst_uri,
              error->message);
    g_error_free(error);
  }
