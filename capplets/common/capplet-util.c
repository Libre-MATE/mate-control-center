...
static void capplet_error_dialog(GtkWindow *parent, char const *msg,
                                 GError *err) {
  if (err != NULL) {
    GtkWidget *dialog;
    ...
    dialog = gtk_message_dialog_new(
        ...
        GTK_BUTTONS_CLOSE, msg, err->message);
...
void capplet_help(GtkWindow *parent, char const *section) {
  ...
  if (!gtk_show_uri_on_window(parent, uri, gtk_get_current_event_time(),
                              &error)) {
    capplet_error_dialog(parent, _("There was an error displaying help: %s"),
                         error);
  }
...
static gboolean directory_delete_recursive(GFile *directory, GError **error) {
  ...
  if (success) success = g_file_delete(directory, NULL, error);
 ...
 **/
gboolean capplet_file_delete_recursive(GFile *file, GError **error) {
  GFileInfo *info;
  GFileType type;
