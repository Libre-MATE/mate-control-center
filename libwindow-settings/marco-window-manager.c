...
static void value_changed(GSettings *settings, gchar *key, void *data) {
...
GObject *window_manager_new(int expected_interface_version) {
  ...
  if (expected_interface_version != MATE_WINDOW_MANAGER_INTERFACE_VERSION) {
    g_warning(
        "Marco window manager module wasn't compiled with the current version "
...
static GList *add_themes_from_dir(GList *current_list, const char *path) {
  GDir *theme_dir;
  const gchar *entry;
  ...
  while ((entry = g_dir_read_name(theme_dir)) != NULL) {
    theme_file_path =
        g_build_filename(path, entry, "metacity-1/metacity-theme-2.xml", NULL);
    ...
    if (g_file_test(theme_file_path, G_FILE_TEST_EXISTS)) {
      ...
      theme_file_path = g_build_filename(
          path, entry, "metacity-1/metacity-theme-1.xml", NULL);

      if (g_file_test(theme_file_path, G_FILE_TEST_EXISTS)) {
        for (node = current_list; (node != NULL) && (!found);
             node = node->next) {
          found = (strcmp(node->data, entry) == 0);
        }
...
static GList *marco_get_theme_list(MateWindowManager *wm) {
  GList *themes = NULL;
...
static char *marco_get_user_theme_folder(MateWindowManager *wm) {
  return g_build_filename(g_get_home_dir(), ".themes", NULL);
}

static void marco_change_settings(MateWindowManager *wm,
  ...
  if (settings->flags & MATE_WM_SETTING_COMPOSITING_MANAGER)
    g_settings_set_boolean(meta_wm->p->settings, MARCO_COMPOSITING_MANAGER_KEY,
  ...
  if (settings->flags & MATE_WM_SETTING_MOUSE_FOCUS)
    g_settings_set_enum(
        meta_wm->p->settings, MARCO_FOCUS_KEY,
        settings->focus_follows_mouse ? FOCUS_MODE_SLOPPY : FOCUS_MODE_CLICK);

  if (settings->flags & MATE_WM_SETTING_AUTORAISE)
    g_settings_set_boolean(meta_wm->p->settings, MARCO_AUTORAISE_KEY,
                           settings->autoraise);

  if (settings->flags & MATE_WM_SETTING_AUTORAISE_DELAY)
    g_settings_set_int(meta_wm->p->settings, MARCO_AUTORAISE_DELAY_KEY,
                       settings->autoraise_delay);

  if (settings->flags & MATE_WM_SETTING_FONT) {
    g_settings_set_string(meta_wm->p->settings, MARCO_FONT_KEY, settings->font);
  }
  ...
  if (settings->flags & MATE_WM_SETTING_MOUSE_MOVE_MODIFIER) {
    ...
    g_settings_set_string(meta_wm->p->settings, MARCO_MOUSE_MODIFIER_KEY,
                          value);
    g_free(value);
  }

  if (settings->flags & MATE_WM_SETTING_THEME) {
    g_settings_set_string(meta_wm->p->settings, MARCO_THEME_KEY,
                          settings->theme);
  }

  if (settings->flags & MATE_WM_SETTING_DOUBLE_CLICK_ACTION) {
    g_settings_set_enum(meta_wm->p->settings, MARCO_DOUBLE_CLICK_TITLEBAR_KEY,
                        settings->double_click_action);
  }
}

static void marco_get_settings(MateWindowManager *wm,
  ...
  if (to_get & MATE_WM_SETTING_COMPOSITING_MANAGER) {
    settings->compositing_manager = g_settings_get_boolean(
        meta_wm->p->settings, MARCO_COMPOSITING_MANAGER_KEY);
    settings->flags |= MATE_WM_SETTING_COMPOSITING_MANAGER;
  }

  if (to_get & MATE_WM_SETTING_COMPOSITING_ALTTAB) {
    settings->compositing_fast_alt_tab = g_settings_get_boolean(
        meta_wm->p->settings, MARCO_COMPOSITING_FAST_ALT_TAB_KEY);
  ...
  if (to_get & MATE_WM_SETTING_MOUSE_FOCUS) {
    ...
    marco_focus_value =
        g_settings_get_enum(meta_wm->p->settings, MARCO_FOCUS_KEY);
    settings->focus_follows_mouse = FALSE;
    if (marco_focus_value == FOCUS_MODE_SLOPPY ||
        marco_focus_value == FOCUS_MODE_MOUSE)
  ...
  if (to_get & MATE_WM_SETTING_AUTORAISE) {
    settings->autoraise =
        (g_settings_get_boolean(meta_wm->p->settings, MARCO_AUTORAISE_KEY) !=
         FALSE);
  ...
  if (to_get & MATE_WM_SETTING_AUTORAISE_DELAY) {
    settings->autoraise_delay =
        g_settings_get_int(meta_wm->p->settings, MARCO_AUTORAISE_DELAY_KEY);
  ...
  if (to_get & MATE_WM_SETTING_FONT) {
    ...
    str = g_settings_get_string(meta_wm->p->settings, MARCO_FONT_KEY);

    if (str == NULL) str = g_strdup("Sans Bold 12");

    if (meta_wm->p->font && strcmp(meta_wm->p->font, str) == 0) {
      g_free(str);
    } else {
  ...
  if (to_get & MATE_WM_SETTING_MOUSE_MOVE_MODIFIER) {
    ...
    str = g_settings_get_string(meta_wm->p->settings, MARCO_MOUSE_MODIFIER_KEY);

    if (str == NULL) str = g_strdup("<Super>");
  ...
  if (to_get & MATE_WM_SETTING_THEME) {
    ...
    str = g_settings_get_string(meta_wm->p->settings, MARCO_THEME_KEY);

    if (str == NULL) str = g_strdup("Spidey");
  ...
  if (to_get & MATE_WM_SETTING_DOUBLE_CLICK_ACTION) {
    settings->double_click_action = g_settings_get_enum(
        meta_wm->p->settings, MARCO_DOUBLE_CLICK_TITLEBAR_KEY);

    settings->flags |= MATE_WM_SETTING_DOUBLE_CLICK_ACTION;
  }
}

static int marco_get_settings_mask(MateWindowManager *wm) {
  return MATE_WM_SETTING_MASK;
}

static void marco_get_double_click_actions(
    MateWindowManager *wm, const MateWMDoubleClickAction **actions_p,
    int *n_actions_p) {
...
static void marco_window_manager_init(MarcoWindowManager *marco_window_manager,
  ...
  g_signal_connect(marco_window_manager->p->settings, "changed",
                   G_CALLBACK(value_changed), marco_window_manager);
}

static void marco_window_manager_finalize(GObject *object) {
...
static void marco_window_manager_class_init(MarcoWindowManagerClass *class) {
...
GType marco_window_manager_get_type(void) {
  ...
  if (!marco_window_manager_type) {
    ...
    marco_window_manager_type = g_type_register_static(
        mate_window_manager_get_type(), "MarcoWindowManager",
