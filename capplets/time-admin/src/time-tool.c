...
void ta_refresh_date(TimeAdmin *ta, struct tm *LocalTime) {
  gtk_calendar_select_month(GTK_CALENDAR(ta->Calendar), LocalTime->tm_mon,
                            LocalTime->tm_year + 1900);
  gtk_calendar_select_day(GTK_CALENDAR(ta->Calendar), LocalTime->tm_mday);
  gtk_calendar_clear_marks(GTK_CALENDAR(ta->Calendar));
  gtk_calendar_mark_day(GTK_CALENDAR(ta->Calendar), LocalTime->tm_mday);
}

static void UpdateDate(TimeAdmin *ta) {
  if (ta->NtpState) {
...
gboolean GetNtpState(TimeAdmin *ta) {
  ...
  proxy = g_dbus_proxy_new_sync(
      ta->Connection, G_DBUS_PROXY_FLAGS_NONE, NULL,
  ...
  ret = g_dbus_proxy_call_sync(
      proxy, "Get", g_variant_new("(ss)", "org.freedesktop.timedate1", "NTP"),
...
const gchar *GetTimeZone(TimeAdmin *ta) {
  ...
  ret = g_dbus_proxy_call_sync(
      proxy, "Get",
      g_variant_new("(ss)", "org.freedesktop.timedate1", "Timezone"),
      G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
  if (ret == NULL) {
    goto EXIT;
  }
...
void SetTimeZone(GDBusProxy *proxy, const char *zone) {
  ...
  ret = g_dbus_proxy_call_sync(proxy, "SetTimezone",
...
static void ChangeSpinBttonState(TimeAdmin *ta) {
...
gboolean ChangeNtpSync(GtkSwitch *widget, gboolean state, gpointer data) {
  ...
  ret = g_dbus_proxy_call_sync(ta->proxy, "SetNTP",
...
static gboolean SetTime(GDBusProxy *proxy, gint64 TimeSec) {
  GError *error = NULL;
  GVariant *ret;

  ret = g_dbus_proxy_call_sync(
      proxy, "SetTime", g_variant_new("(xbb)", TimeSec * 1000 * 1000, 0, 0),
      G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
