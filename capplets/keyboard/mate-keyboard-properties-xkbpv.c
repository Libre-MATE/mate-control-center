 ...
 */
extern gboolean xkl_xkb_config_native_prepare(
    XklEngine *engine, const XklConfigRec *data,
...
GtkWidget *xkb_layout_preview_create_widget(GtkBuilder *chooserDialog) {
  GtkWidget *kbdraw = matekbd_keyboard_drawing_new();

  matekbd_keyboard_drawing_set_groups_levels(MATEKBD_KEYBOARD_DRAWING(kbdraw),
                                             pGroupsLevels);
  return kbdraw;
}

void xkb_layout_preview_update(GtkBuilder *chooser_dialog) {
#ifdef HAVE_X11_EXTENSIONS_XKB_H
...
void xkb_layout_preview_set_drawing_layout(GtkWidget *kbdraw, const gchar *id) {
#ifdef HAVE_X11_EXTENSIONS_XKB_H
  if (kbdraw != NULL) {
    if (id != NULL) {
      XklConfigRec *data;
      ...
      if (xkl_config_rec_get_from_server(data, engine)) {
        if ((p = data->layouts) != NULL) g_strfreev(data->layouts);

        if ((p = data->variants) != NULL) g_strfreev(data->variants);

        data->layouts = g_new0(char *, 2);
        data->variants = g_new0(char *, 2);
        if (matekbd_keyboard_config_split_items(id, &layout, &variant) &&
            variant != NULL) {
        ...
        if (xkl_xkb_config_native_prepare(engine, data, &component_names)) {
          matekbd_keyboard_drawing_set_keyboard(
              MATEKBD_KEYBOARD_DRAWING(kbdraw), &component_names);
      ...
      matekbd_keyboard_drawing_set_keyboard(MATEKBD_KEYBOARD_DRAWING(kbdraw),
                                            NULL);
  }
#endif
}
