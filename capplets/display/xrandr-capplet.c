...
struct App {
  ...
  /* These are used while we are waiting for the ApplyConfiguration method to be
   * executed over D-bus */
  GDBusConnection *connection;
  GDBusProxy *proxy;

  enum { APPLYING_VERSION_1, APPLYING_VERSION_2 } apply_configuration_state;
};

/* Response codes for custom buttons in the main dialog */
enum { RESPONSE_MAKE_DEFAULT = 1 };
...
static void apply_configuration_returned_cb(GObject *source_object,
                                            GAsyncResult *res, gpointer data);

static void error_message(App *app, const char *primary_text,
                          const char *secondary_text) {
  GtkWidget *dialog;

  dialog = gtk_message_dialog_new(
      (app && app->dialog) ? GTK_WINDOW(app->dialog) : NULL,
...
static gchar *idle_free(gchar *s) {
  g_idle_add(do_free, s);

  return s;
}

static void on_screen_changed(MateRRScreen *scr, gpointer data) {
  MateRRConfig *current;
  App *app = data;

  current = mate_rr_config_new_current(app->screen, NULL);

  if (app->current_configuration) g_object_unref(app->current_configuration);
...
static void on_viewport_changed(FooScrollArea *scroll_area,
...
static void layout_set_font(PangoLayout *layout, const char *font) {
...
static void clear_combo(GtkWidget *widget) {
...
static gboolean foreach (GtkTreeModel *model, GtkTreePath * path,
...
static void add_key(GtkWidget *widget, const char *text, guint width,
                    guint height, int rate, MateRRRotation rotation) {
  ForeachInfo info;
  GtkComboBox *box = GTK_COMBO_BOX(widget);
  GtkTreeModel *model = gtk_combo_box_get_model(box);
  GtkListStore *store = GTK_LIST_STORE(model);
  ...
  if (!info.found) {
    GtkTreeIter iter;
    gtk_list_store_insert_with_values(store, &iter, -1, 0, text, 1, width, 2,
                                      height, 3, rate, 4, width * height, 5,
                                      rotation, -1);
  }
}

static gboolean combo_select(GtkWidget *widget, const char *text) {
  GtkComboBox *box = GTK_COMBO_BOX(widget);
  GtkTreeModel *model = gtk_combo_box_get_model(box);
  ForeachInfo info;
...
static void rebuild_rotation_combo(App *app) {
  typedef struct {
  ...
  gtk_widget_set_sensitive(app->rotation_combo,
                           app->current_output && mate_rr_output_info_is_active(
                                                      app->current_output));

  if (!app->current_output) return;
  ...
  for (i = 0; i < G_N_ELEMENTS(rotations); ++i) {
     ...
     */
    if (mate_rr_config_applicable(app->current_configuration, app->screen,
                                  NULL)) {
      add_key(app->rotation_combo, _(info->name), 0, 0, 0, info->rotation);

      if (info->rotation == current) selection = _(info->name);
    }
  }
...
static char *make_rate_string(int hz) {
  return g_strdup_printf(_("%d Hz"), hz);
}

static void rebuild_rate_combo(App *app) {
  MateRRMode **modes;
  int best;
  int i;
  ...
  if (!app->current_output || !(modes = get_current_modes(app))) return;
  ...
  for (i = 0; modes[i] != NULL; ++i) {
    ...
    if ((width == (guint)output_width) && (height == (guint)output_height)) {
      add_key(app->refresh_combo, idle_free(make_rate_string(rate)), 0, 0, rate,
              -1);
  ...
  if (!combo_select(
          app->refresh_combo,
          idle_free(make_rate_string(
              mate_rr_output_info_get_refresh_rate(app->current_output)))))
    combo_select(app->refresh_combo, idle_free(make_rate_string(best)));
}

static int count_active_outputs(App *app) {
  int i, count = 0;
  MateRROutputInfo **outputs =
      mate_rr_config_get_outputs(app->current_configuration);

  for (i = 0; outputs[i] != NULL; ++i) {
    if (mate_rr_output_info_is_active(outputs[i])) count++;
  }

  return count;
}

/* FIXME: this function is copied from
 * mate-settings-daemon/plugins/xrandr/gsd-xrandr-manager.c. Do we need to put
 * this function in mate-desktop for public use?
 */
static gboolean get_clone_size(MateRRScreen *screen, guint *width,
                               guint *height) {
...
static gboolean output_info_supports_mode(App *app, MateRROutputInfo *info,
                                          guint width, guint height) {
  MateRROutput *output;
  MateRRMode **modes;
  guint i;

  if (!mate_rr_output_info_is_connected(info)) return FALSE;

  output = mate_rr_screen_get_output_by_name(
      app->screen, mate_rr_output_info_get_name(info));
...
/* Computes whether "Mirror Screens" (clone mode) is supported based on these
 * criteria:
 *
 ...
 */
static gboolean mirror_screens_is_supported(App *app) {
  guint clone_width;
  guint clone_height;
  ...
  if (have_clone_size) {
    ...
    for (i = 0; outputs[i] != NULL; i++) {
      ...
      if (mate_rr_output_info_is_connected(outputs[i]) &&
          output_info_supports_mode(app, outputs[i], clone_width,
                                    clone_height)) {
        num_outputs_with_clone_size++;
      }
    }

    if (num_outputs_with_clone_size >= 2) mirror_is_supported = TRUE;
  }

  return mirror_is_supported;
}

static void on_scale_btn_active_changed_cb(GtkWidget *widget, App *app) {
  gint scale;
...
static void rebuild_scale_window(App *app) {
  GtkRadioButton *group = NULL;
  gint32 scale;
  int i;
  const char *button_label[] = {_("auto detect"), _("100%"), _("200%"), NULL};

  if (app->scale_bbox != NULL) return;
  ...
  for (i = 0; button_label[i] != NULL; i++) {
    GtkWidget *scale_btn;
    scale_btn =
        gtk_radio_button_new_with_label_from_widget(group, button_label[i]);
    if (!group) group = GTK_RADIO_BUTTON(scale_btn);
    if (i == scale)
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(scale_btn), TRUE);

    g_object_set_data(G_OBJECT(scale_btn), "scale", GINT_TO_POINTER(i));

    g_signal_connect(scale_btn, "toggled",
                     G_CALLBACK(on_scale_btn_active_changed_cb), app);
    gtk_container_add(GTK_CONTAINER(app->scale_bbox), scale_btn);
  }
  gtk_widget_show_all(app->scale_bbox);
}

static void rebuild_mirror_screens(App *app) {
  gboolean mirror_is_active;
  gboolean mirror_is_supported;

  g_signal_handlers_block_by_func(app->clone_checkbox,
                                  G_CALLBACK(on_clone_changed), app);

  mirror_is_active = app->current_configuration &&
                     mate_rr_config_get_clone(app->current_configuration);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(app->clone_checkbox),
                               mirror_is_active);
  gtk_widget_set_sensitive(app->clone_checkbox, mirror_is_supported);

  g_signal_handlers_unblock_by_func(app->clone_checkbox,
                                    G_CALLBACK(on_clone_changed), app);
}

static void rebuild_current_monitor_label(App *app) {
  char *str, *tmp;
  GdkRGBA color;
  gboolean use_color;
  ...
  if (app->current_output) {
    if (mate_rr_config_get_clone(app->current_configuration))
      ...
      tmp = g_strdup_printf(
          _("Monitor: %s"),
          mate_rr_output_info_get_display_name(app->current_output));

    str = g_strdup_printf("<b>%s</b>", tmp);
    mate_rr_labeler_get_rgba_for_output(app->labeler, app->current_output,
                                        &color);
    use_color = TRUE;
    g_free(tmp);
  } else {
  ...
  if (use_color) {
    ...
    gtk_widget_override_background_color(
        app->current_monitor_event_box,
        gtk_widget_get_state_flags(app->current_monitor_event_box), &color);
    ...
    gtk_widget_override_color(
        app->current_monitor_label,
        gtk_widget_get_state_flags(app->current_monitor_label), &black);
  }

  gtk_event_box_set_visible_window(
      GTK_EVENT_BOX(app->current_monitor_event_box), use_color);
}

static void rebuild_on_off_radios(App *app) {
  gboolean sensitive;
  gboolean on_active;
  gboolean off_active;

  g_signal_handlers_block_by_func(app->monitor_on_radio,
                                  G_CALLBACK(monitor_on_off_toggled_cb), app);
  g_signal_handlers_block_by_func(app->monitor_off_radio,
                                  G_CALLBACK(monitor_on_off_toggled_cb), app);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(app->monitor_on_radio),
                               on_active);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(app->monitor_off_radio),
                               off_active);

  g_signal_handlers_unblock_by_func(app->monitor_on_radio,
                                    G_CALLBACK(monitor_on_off_toggled_cb), app);
  g_signal_handlers_unblock_by_func(app->monitor_off_radio,
                                    G_CALLBACK(monitor_on_off_toggled_cb), app);
}

static char *make_resolution_string(guint width, guint height) {
  return g_strdup_printf(_("%u x %u"), width, height);
}

static void find_best_mode(MateRRMode **modes, guint *out_width,
                           guint *out_height) {
  guint i;
...
static void rebuild_resolution_combo(App *app) {
  guint i;
  MateRRMode **modes;
  const char *current;
  ...
  if (!(modes = get_current_modes(app)) || !app->current_output ||
      !mate_rr_output_info_is_active(app->current_output)) {
  ...
  for (i = 0; modes[i] != NULL; ++i) {
    ...
    add_key(app->resolution_combo,
            idle_free(make_resolution_string(width, height)), width, height, 0,
            -1);
  }

  current = idle_free(
      make_resolution_string((guint)output_width, (guint)output_height));
...
static void rebuild_gui(App *app) {
  ...
  gtk_widget_set_sensitive(
      app->primary_button,
      app->current_output &&
          !mate_rr_output_info_get_primary(app->current_output));

  app->ignore_gui_changes = FALSE;
}

static gboolean get_mode(GtkWidget *widget, int *width, int *height, int *freq,
                         MateRRRotation *rot) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  GtkComboBox *box = GTK_COMBO_BOX(widget);
  ...
  if (!gtk_combo_box_get_active_iter(box, &iter)) return FALSE;

  if (!width) width = &dummy;

  if (!height) height = &dummy;

  if (!freq) freq = &dummy;

  if (!rot) rot = (MateRRRotation *)&dummy;

  model = gtk_combo_box_get_model(box);
  gtk_tree_model_get(model, &iter, 1, width, 2, height, 3, freq, 5, rot, -1);
...
static void select_resolution_for_current_output(App *app) {
  MateRRMode **modes;
  int width, height;
  guint best_w, best_h;
  int x, y;
  ...
  find_best_mode(modes, &best_w, &best_h);
  mate_rr_output_info_set_geometry(app->current_output, x, y, (int)best_w,
                                   (int)best_w);
}
...
static void monitor_on_off_toggled_cb(GtkToggleButton *toggle, gpointer data) {
  ...
  if (is_on)
    select_resolution_for_current_output(
        app); /* The refresh rate will be picked in rebuild_rate_combo() */

  rebuild_gui(app);
  foo_scroll_area_invalidate(FOO_SCROLL_AREA(app->area));
}

static void realign_outputs_after_resolution_change(
    App *app, MateRROutputInfo *output_that_changed, int old_width,
    int old_height) {
  ...
  mate_rr_output_info_get_geometry(output_that_changed, &x, &y, &width,
                                   &height);
  ...
  for (i = 0; outputs[i] != NULL; i++) {
    ...
    if (outputs[i] == output_that_changed ||
        mate_rr_output_info_is_connected(outputs[i]))
      continue;

    mate_rr_output_info_get_geometry(outputs[i], &output_x, &output_y,
                                     &output_width, &output_height);
    ...
    mate_rr_output_info_set_geometry(outputs[i], output_x, output_y,
                                     output_width, output_height);
  }
}
...
static void on_resolution_changed(GtkComboBox *box, gpointer data) {
  ...
  mate_rr_output_info_get_geometry(app->current_output, &x, &y, &old_width,
                                   &old_height);
  ...
  realign_outputs_after_resolution_change(app, app->current_output, old_width,
                                          old_height);
...
static void lay_out_outputs_horizontally(App *app) {
  int i;
  int x;
  MateRROutputInfo **outputs;
  ...
  if (mate_rr_config_get_clone(app->current_configuration)) {
    ...
    for (i = 0; outputs[i]; i++) {
      ...
      if (output_info_supports_mode(app, outputs[i], width, height)) {
        ...
        mate_rr_output_info_set_geometry(outputs[i], x, y, (int)width,
                                         (int)height);
      }
    }
  } else {
...
static GList *list_connected_outputs(App *app, int *total_w, int *total_h) {
  guint i;
  int dummy;
  GList *result = NULL;
  MateRROutputInfo **outputs;

  if (!total_w) total_w = &dummy;
  if (!total_h) total_h = &dummy;
...
static guint get_n_connected(App *app) {
...
static double compute_scale(App *app) {
...
static void add_edge(MateRROutputInfo *output, int x1, int y1, int x2, int y2,
                     GArray *edges) {
  Edge e;
...
static void list_edges_for_output(MateRROutputInfo *output, GArray *edges) {
  int x, y, w, h;
...
static void list_edges(MateRRConfig *config, GArray *edges) {
  int i;
...
static gboolean overlap(int s1, int e1, int s2, int e2) {
  return (!(e1 < s2 || s1 >= e2));
}

static gboolean horizontal_overlap(Edge *snapper, Edge *snappee) {
  if (snapper->y1 != snapper->y2 || snappee->y1 != snappee->y2) return FALSE;

  return overlap(snapper->x1, snapper->x2, snappee->x1, snappee->x2);
}

static gboolean vertical_overlap(Edge *snapper, Edge *snappee) {
  if (snapper->x1 != snapper->x2 || snappee->x1 != snappee->x2) return FALSE;

  return overlap(snapper->y1, snapper->y2, snappee->y1, snappee->y2);
}

static void add_snap(GArray *snaps, Snap snap) {
  if (ABS(snap.dx) <= 200 || ABS(snap.dy) <= 200)
    g_array_append_val(snaps, snap);
}

static void add_edge_snaps(Edge *snapper, Edge *snappee, GArray *snaps) {
  Snap snap;
...
static void list_snaps(MateRROutputInfo *output, GArray *edges, GArray *snaps) {
  guint i;
  ...
  for (i = 0; i < edges->len; ++i) {
    ...
    if (output_edge->output == output) {
      ...
      for (j = 0; j < edges->len; ++j) {
        ...
        if (edge->output != output) add_edge_snaps(output_edge, edge, snaps);
      }
    }
  }
}
...
static gboolean corner_on_edge(int x, int y, Edge *e) {
  ...
  if (y == e->y1 && y == e->y2 && x >= e->x1 && x <= e->x2) return TRUE;

  return FALSE;
}

static gboolean edges_align(Edge *e1, Edge *e2) {
  if (corner_on_edge(e1->x1, e1->y1, e2)) return TRUE;

  if (corner_on_edge(e2->x1, e2->y1, e1)) return TRUE;

  return FALSE;
}

static gboolean output_is_aligned(MateRROutputInfo *output, GArray *edges) {
...
static void get_output_rect(MateRROutputInfo *output, GdkRectangle *rect) {
  mate_rr_output_info_get_geometry(output, &rect->x, &rect->y, &rect->width,
                                   &rect->height);
  get_geometry(output, &rect->width, &rect->height); /* accounts for rotation */
}

static gboolean output_overlaps(MateRROutputInfo *output,
                                MateRRConfig *config) {
  int i;
  ...
  for (i = 0; outputs[i]; ++i) {
    if (outputs[i] != output && mate_rr_output_info_is_connected(outputs[i])) {
      ...
      if (gdk_rectangle_intersect(&output_rect, &other_rect, NULL)) return TRUE;
    }
  }
...
static gboolean mate_rr_config_is_aligned(MateRRConfig *config, GArray *edges) {
  int i;
  ...
  for (i = 0; outputs[i]; ++i) {
    if (mate_rr_output_info_is_connected(outputs[i])) {
      if (!output_is_aligned(outputs[i], edges)) return FALSE;

      if (output_overlaps(outputs[i], config)) return FALSE;
    }
  }
...
static gboolean is_corner_snap(const Snap *s) {
  return s->dx != 0 && s->dy != 0;
}

static int compare_snaps(gconstpointer v1, gconstpointer v2) {
  const Snap *s1 = v1;
  const Snap *s2 = v2;
 ...
 */
static void set_cursor(GtkWidget *widget, GdkCursorType type) {
  GdkCursor *cursor;
  GdkWindow *window;
...
static void set_monitors_tooltip(App *app, gboolean is_dragging) {
  const char *text;
  ...
  if (is_dragging)
    ...
    text =
        _("Select a monitor to change its properties; drag it to rearrange its "
          "placement.");

  gtk_widget_set_tooltip_text(app->area, text);
}

static void on_output_event(FooScrollArea *area, FooScrollAreaEvent *event,
                            gpointer data) {
  MateRROutputInfo *output = data;
  App *app = g_object_get_data(G_OBJECT(area), "app");

  /* If the mouse is inside the outputs, set the cursor to "you can move me".
   * See on_canvas_event() for where we reset the cursor to the default if it
   * exits the outputs' area.
   */
  if (!mate_rr_config_get_clone(app->current_configuration) &&
      get_n_connected(app) > 1)
  ...
  if (event->type == FOO_BUTTON_PRESS) {
    ...
    if (!mate_rr_config_get_clone(app->current_configuration) &&
        get_n_connected(app) > 1) {
      int output_x, output_y;
      mate_rr_output_info_get_geometry(output, &output_x, &output_y, NULL,
                                       NULL);
    ...
    if (foo_scroll_area_is_grabbed(area)) {
      ...
      for (i = 0; i < snaps->len; ++i) {
        ...
        mate_rr_output_info_set_geometry(output, new_x + snap->dx,
                                         new_y + snap->dy, width, height);
        ...
        if (mate_rr_config_is_aligned(app->current_configuration, new_edges)) {
          ...
          mate_rr_output_info_set_geometry(output, info->output_x,
                                           info->output_y, width, height);
        }
      }
...
static void on_canvas_event(FooScrollArea *area, FooScrollAreaEvent *event,
                            gpointer data) {
...
static PangoLayout *get_display_name(App *app, MateRROutputInfo *output) {
  char *text;
  PangoLayout *layout;
...
static void paint_background(FooScrollArea *area, cairo_t *cr) {
  GdkRectangle viewport;
  GtkWidget *widget;
  ...
  gtk_style_context_get(widget_style, gtk_style_context_get_state(widget_style),
                        GTK_STYLE_PROPERTY_BACKGROUND_COLOR, &base_color, NULL);
  gtk_style_context_restore(widget_style);
  gdk_cairo_set_source_rgba(cr, base_color);
  gdk_rgba_free(base_color);

  cairo_rectangle(cr, viewport.x, viewport.y, viewport.width, viewport.height);
  ...
  mate_desktop_gtk_style_get_dark_color(
      widget_style, gtk_style_context_get_state(widget_style), &dark_color);
  gtk_style_context_restore(widget_style);
...
static void paint_output(App *app, cairo_t *cr, guint i) {
  int w, h;
  double scale = compute_scale(app);
  double x, y;
  int output_x, output_y;
  ...
  rotation = mate_rr_output_info_get_rotation(output);
  if (rotation & MATE_RR_REFLECT_X) cairo_scale(cr, -1, 1);

  if (rotation & MATE_RR_REFLECT_Y) cairo_scale(cr, 1, -1);
  ...
  foo_scroll_area_add_input_from_fill(FOO_SCROLL_AREA(app->area), cr,
                                      on_output_event, output);
  cairo_fill(cr);
  ...
  cairo_rectangle(cr, x + 0.5, y + 0.5, w * scale + 0.5 - 1,
                  h * scale + 0.5 - 1);
  ...
  available_w = w * scale + 0.5 - 6; /* Same as the inner rectangle's width,
                                        minus 1 pixel of padding on each side */
  if (available_w < ink_extent.width)
    factor = available_w / ink_extent.width;
  else
    factor = 1.0;

  cairo_move_to(cr, x + ((w * scale + 0.5) - factor * log_extent.width) / 2,
...
static void on_area_paint(FooScrollArea *area, cairo_t *cr, gpointer data) {
  App *app = data;
  ...
  if (!app->current_configuration) return;
  ...
  for (list = connected_outputs; list != NULL; list = list->next) {
    ...
    if (mate_rr_config_get_clone(app->current_configuration)) break;
  }
}

static void make_text_combo(GtkWidget *widget, int sort_column) {
  GtkComboBox *box = GTK_COMBO_BOX(widget);
  GtkListStore *store = gtk_list_store_new(6, G_TYPE_STRING, /* Text */
                                           G_TYPE_INT,       /* Width */
                                           G_TYPE_INT,       /* Height */
...
static void compute_virtual_size_for_configuration(MateRRConfig *config,
                                                   int *ret_width,
                                                   int *ret_height) {
  int i;
  int width, height;
...
static void check_required_virtual_size(App *app) {
  int req_width, req_height;
  int min_width, max_width;
  int min_height, max_height;

  compute_virtual_size_for_configuration(app->current_configuration, &req_width,
                                         &req_height);
...
static void begin_version2_apply_configuration(App *app,
  ...
  app->proxy = g_dbus_proxy_new_sync(
      app->connection, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.mate.SettingsDaemon",
      "/org/mate/SettingsDaemon/XRANDR", "org.mate.SettingsDaemon.XRANDR_2",
      NULL, &error);
  if (app->proxy == NULL) {
    ...
    g_dbus_proxy_call(app->proxy, "ApplyConfiguration",
...
static void begin_version1_apply_configuration(App *app) {
  GError *error = NULL;

  app->proxy = g_dbus_proxy_new_sync(
      app->connection, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.mate.SettingsDaemon",
      "/org/mate/SettingsDaemon/XRANDR", "org.mate.SettingsDaemon.XRANDR", NULL,
      &error);
  if (app->proxy == NULL) {
    g_warning("Failed to get dbus connection: %s", error->message);
    g_error_free(error);
    ...
    g_dbus_proxy_call(app->proxy, "ApplyConfiguration", g_variant_new("()"),
                      G_DBUS_CALL_FLAGS_NONE, -1, NULL,
                      (GAsyncReadyCallback)apply_configuration_returned_cb,
                      app);
  }
...
static void ensure_current_configuration_is_saved(void) {
  ...
  rr_screen =
      mate_rr_screen_new(gdk_screen_get_default(), NULL); /* NULL-GError */
  if (!rr_screen) return;
...
static void apply_configuration_returned_cb(GObject *source_object,
  ...
  if (variant == NULL) {
    if (app->apply_configuration_state == APPLYING_VERSION_2 &&
      ...
      /* We don't pop up an error message; mate-settings-daemon already does
       * that in case the selected RANDR configuration could not be applied.
       */
      g_error_free(error);
    }
  }
...
static gboolean sanitize_and_save_configuration(App *app) {
  GError *error;
  ...
  if (!mate_rr_config_save(app->current_configuration, &error)) {
    error_message(app, _("Could not save the monitor configuration"),
                  error->message);
    g_error_free(error);
    return FALSE;
  }
...
static void apply(App *app) {
  GError *error = NULL;

  if (!sanitize_and_save_configuration(app)) return;
  ...
  begin_version2_apply_configuration(app, gtk_widget_get_window(app->dialog),
                                     app->apply_button_clicked_timestamp);
}

static void on_detect_displays(GtkWidget *widget, gpointer data) {
  App *app = data;
  GError *error;
...
static void set_primary(GtkWidget *widget, gpointer data) {
  App *app = data;
  int i;
  MateRROutputInfo **outputs;
  ...
  outputs = mate_rr_config_get_outputs(app->current_configuration);
  for (i = 0; outputs[i] != NULL; i++) {
    mate_rr_output_info_set_primary(outputs[i],
                                    outputs[i] == app->current_output);
  }
...
static void on_show_icon_toggled(GtkWidget *widget, gpointer data) {
...
static MateRROutputInfo *get_nearest_output(MateRRConfig *configuration, int x,
                                            int y) {
  int i;
  int nearest_index;
  int nearest_dist;
 ...
 */
static MateRROutputInfo *get_output_for_window(MateRRConfig *configuration,
                                               GdkWindow *window) {
  ...
  for (i = 0; outputs[i] != NULL; i++) {
    ...
    mate_rr_output_info_get_geometry(outputs[i], &output_rect.x, &output_rect.y,
                                     &output_rect.width, &output_rect.height);

    if (mate_rr_output_info_is_connected(outputs[i]) &&
        gdk_rectangle_intersect(&win_rect, &output_rect, &intersection)) {
      int area;
  ...
  if (largest_index != -1)
    ...
    return get_nearest_output(configuration, win_rect.x + win_rect.width / 2,
...
static void select_current_output_from_dialog_position(App *app) {
  if (gtk_widget_get_realized(app->dialog))
    app->current_output = get_output_for_window(
        app->current_configuration, gtk_widget_get_window(app->dialog));
  else
    app->current_output = NULL;
 ...
 */
static gboolean dialog_map_event_cb(GtkWidget *widget, GdkEventAny *event,
                                    gpointer data) {
  App *app = data;
...
static void apply_button_clicked_cb(GtkButton *button, gpointer data) {
  App *app = data;
...
static void success_dialog_for_make_default(App *app) {
  GtkWidget *dialog;

  dialog = gtk_message_dialog_new(
      GTK_WINDOW(app->dialog), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO,
      GTK_BUTTONS_OK, _("The monitor configuration has been saved"));
...
static void error_dialog_for_make_default(App *app, const char *error_text) {
  error_message(app, _("Could not set the default configuration for monitors"),
                error_text);
}

static void make_default(App *app) {
  char *command_line;
  ...
  dest_filename =
      g_settings_get_string(app->settings, DEFAULT_CONFIGURATION_FILE_KEY);
  if (!dest_filename) return; /* FIXME: present an error? */
  ...
  command_line = g_strdup_printf(
      "pkexec %s/mate-display-properties-install-systemwide %s %s", SBINDIR,
      source_filename, dest_basename);

  error = NULL;
  if (!g_spawn_command_line_sync(command_line, NULL, &std_error, &exit_status,
                                 &error)) {
...
static GtkWidget *_gtk_builder_get_widget(GtkBuilder *builder,
                                          const gchar *name) {
  return GTK_WIDGET(gtk_builder_get_object(builder, name));
}

static void run_application(App *app) {
  GtkBuilder *builder;
  GtkWidget *align;
  GError *error = NULL;

  builder =
      gtk_builder_new_from_resource("/org/mate/mcc/display/display-capplet.ui");
  ...
  app->current_monitor_event_box =
      _gtk_builder_get_widget(builder, "current_monitor_event_box");
  app->current_monitor_label =
      _gtk_builder_get_widget(builder, "current_monitor_label");

  app->monitor_on_radio = _gtk_builder_get_widget(builder, "monitor_on_radio");
  app->monitor_off_radio =
      _gtk_builder_get_widget(builder, "monitor_off_radio");
  g_signal_connect(app->monitor_on_radio, "toggled",
                   G_CALLBACK(monitor_on_off_toggled_cb), app);
  g_signal_connect(app->monitor_off_radio, "toggled",
                   G_CALLBACK(monitor_on_off_toggled_cb), app);

  app->resolution_combo = _gtk_builder_get_widget(builder, "resolution_combo");
  ...
  g_signal_connect(app->refresh_combo, "changed", G_CALLBACK(on_rate_changed),
                   app);
  ...
  g_signal_connect(app->clone_checkbox, "toggled", G_CALLBACK(on_clone_changed),
                   app);
  ...
  g_signal_connect(app->primary_button, "clicked", G_CALLBACK(set_primary),
                   app);

  app->show_icon_checkbox =
      _gtk_builder_get_widget(builder, "show_notification_icon");

  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(app->show_icon_checkbox),
      g_settings_get_boolean(app->settings, SHOW_ICON_KEY));

  g_signal_connect(app->show_icon_checkbox, "toggled",
                   G_CALLBACK(on_show_icon_toggled), app);
  ...
  g_signal_connect(app->area, "paint", G_CALLBACK(on_area_paint), app);
