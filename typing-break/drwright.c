...
static void activity_detected_cb(DrwMonitor *monitor, DrWright *drwright);
...
static void break_window_done_cb(GtkWidget *window, DrWright *dr);
static void break_window_postpone_cb(GtkWidget *window, DrWright *dr);
static void break_window_destroy_cb(GtkWidget *window, DrWright *dr);
static void popup_break_cb(GtkAction *action, DrWright *dr);
static void popup_preferences_cb(GtkAction *action, DrWright *dr);
...
static const GtkActionEntry actions[] = {
    {"Preferences", "preferences-desktop", N_("_Preferences"), NULL, NULL,
     G_CALLBACK(popup_preferences_cb)},
    {"About", "help-about", N_("_About"), NULL, NULL,
     G_CALLBACK(popup_about_cb)},
    {"TakeABreak", NULL, N_("_Take a Break"), NULL, NULL,
     G_CALLBACK(popup_break_cb)}};

extern gboolean debug;

static void setup_debug_values(DrWright *dr) {
  dr->type_time = 5;
  dr->warn_time = 4;
  dr->break_time = 10;
}
...
static void update_app_indicator(DrWright *dr) {
  ...
  if (!dr->enabled) {
    app_indicator_set_status(dr->indicator, APP_INDICATOR_STATUS_PASSIVE);
    return;
  }
...
static void set_status_icon(GtkStatusIcon *icon, cairo_surface_t *surface) {
  GdkPixbuf *pixbuf;
...
static void update_icon(DrWright *dr) {
  GdkPixbuf *pixbuf;
  ...
  tmp_pixbuf =
      gdk_pixbuf_get_from_surface(dr->neutral_bar, 0, 0, width, height);
  ...
  gdk_pixbuf_composite(pixbuf, tmp_pixbuf, 0, offset, width, height - offset, 0,
                       0, 1.0, 1.0, GDK_INTERP_BILINEAR, 255);

  if (set_pixbuf) {
    gtk_status_icon_set_from_pixbuf(dr->icon, tmp_pixbuf);
  }
...
static gboolean blink_timeout_cb(DrWright *dr) {
  gfloat r;
  gint timeout;

  r = (dr->type_time - drw_timer_elapsed(dr->timer) - dr->save_last_time) /
      dr->warn_time;
  ...
  if (timeout) {
    dr->blink_timeout_id =
        g_timeout_add(timeout, (GSourceFunc)blink_timeout_cb, dr);
  } else {
...
static void start_blinking(DrWright *dr) {
#ifndef HAVE_APP_INDICATOR
...
static void stop_blinking(DrWright *dr) {
#ifndef HAVE_APP_INDICATOR
  if (dr->blink_timeout_id) {
...
static gboolean grab_keyboard_on_window(GdkWindow *window,
  ...
  status = gdk_seat_grab(seat, window, GDK_SEAT_CAPABILITY_KEYBOARD, TRUE, NULL,
                         NULL, NULL, NULL);
...
static gboolean break_window_map_event_cb(GtkWidget *widget, GdkEvent *event,
                                          DrWright *dr) {
  grab_keyboard_on_window(gtk_widget_get_window(dr->break_window),
                          gtk_get_current_event_time());

  return FALSE;
}

static gboolean maybe_change_state(DrWright *dr) {
  gint elapsed_time;
  ...
  switch (dr->state) {
    ...
    case STATE_BREAK_SETUP:
      ...
      g_signal_connect(dr->break_window, "map_event",
                       G_CALLBACK(break_window_map_event_cb), dr);

      g_signal_connect(dr->break_window, "done",
                       G_CALLBACK(break_window_done_cb), dr);

      g_signal_connect(dr->break_window, "postpone",
                       G_CALLBACK(break_window_postpone_cb), dr);

      g_signal_connect(dr->break_window, "destroy",
                       G_CALLBACK(break_window_destroy_cb), dr);
...
static gboolean update_status(DrWright *dr) {
  gint min;
  gchar *str;
#ifdef HAVE_APP_INDICATOR
  GtkWidget *item;
  ...
  if (!dr->enabled) {
    ...
    app_indicator_set_status(dr->indicator, APP_INDICATOR_STATUS_PASSIVE);
#else
    gtk_status_icon_set_tooltip_text(dr->icon, _("Disabled"));
#endif /* HAVE_APP_INDICATOR */
    return TRUE;
  }
  ...
  if (min >= 1) {
    ...
    str = g_strdup_printf(ngettext("%d minute until the next break",
                                   "%d minutes until the next break", min),
                          min);
#endif /* HAVE_APP_INDICATOR */
  } else {
#ifdef HAVE_APP_INDICATOR
...
static gint get_time_left(DrWright *dr) {
  gint elapsed_time;

  elapsed_time = drw_timer_elapsed(dr->timer);

  return floor(0.5 +
               (dr->type_time - elapsed_time - dr->save_last_time) / 60.0);
}

static void activity_detected_cb(DrwMonitor *monitor, DrWright *dr) {
  drw_timer_start(dr->idle_timer);
}

static void gsettings_notify_cb(GSettings *settings, gchar *key,
                                gpointer user_data) {
...
static void popup_preferences_cb(GtkAction *action, DrWright *dr) {
  ...
  if (!mate_gdk_spawn_command_line_on_screen(
          screen, "mate-keyboard-properties --typing-break", &error)) {
    GtkWidget *error_dialog;

    error_dialog = gtk_message_dialog_new(
        NULL, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
        _("Unable to bring up the typing break properties dialog with the "
          "following error: %s"),
        error->message);
...
static void popup_about_cb(GtkAction *action, DrWright *dr) {
  gint i;
  gchar *authors[] = {N_("Written by Richard Hult <richard@imendio.com>"),
                      N_("Eye candy added by Anders Carlsson"), NULL};

  for (i = 0; authors[i]; i++) authors[i] = _(authors[i]);

  gtk_show_about_dialog(NULL, "authors", authors, "comments",
                        _("A computer break reminder."), "logo-icon-name",
                        "mate-typing-monitor", "translator-credits",
                        _("translator-credits"), "version", VERSION, NULL);
}

#ifndef HAVE_APP_INDICATOR
static void popup_menu_cb(GtkWidget *widget, guint button, guint activate_time,
                          DrWright *dr) {
  GtkWidget *menu;
  ...
  gtk_menu_popup(GTK_MENU(menu), NULL, NULL, gtk_status_icon_position_menu,
                 dr->icon, 0, gtk_get_current_event_time());
}
#endif /* HAVE_APP_INDICATOR */
...
static void break_window_postpone_cb(GtkWidget *window, DrWright *dr) {
  ...
  if (elapsed_time + dr->save_last_time >= dr->type_time) {
    ...
    gfloat postpone_time =
        (((float)elapsed_time) / dr->break_time) * sqrt(dr->type_time);
    postpone_time *= postpone_time;
    dr->save_last_time =
        dr->type_time - MAX(dr->warn_time, (gint)postpone_time);
  }
...
static void init_app_indicator(DrWright *dr) {
  ...
  dr->indicator = app_indicator_new_with_path(
      "typing-break-indicator", TYPING_MONITOR_ACTIVE_ICON,
      APP_INDICATOR_CATEGORY_APPLICATION_STATUS, IMAGEDIR);
  if (dr->enabled) {
    app_indicator_set_status(dr->indicator, APP_INDICATOR_STATUS_ACTIVE);
  } else {
    app_indicator_set_status(dr->indicator, APP_INDICATOR_STATUS_PASSIVE);
  }
...
static void init_tray_icon(DrWright *dr) {
  ...
  pixbuf = gdk_pixbuf_get_from_surface(
      dr->neutral_bar, 0, 0, cairo_image_surface_get_width(dr->neutral_bar),
  ...
  g_signal_connect(dr->icon, "popup_menu", G_CALLBACK(popup_menu_cb), dr);
}
#endif /* HAVE_APP_INDICATOR */
...
DrWright *drwright_new(void) {
  ...
  g_signal_connect(dr->monitor, "activity", G_CALLBACK(activity_detected_cb),
                   dr);
  ...
  dr->green_bar =
      cairo_image_surface_create_from_png(IMAGEDIR "/bar-green.png");
  dr->disabled_bar =
      cairo_image_surface_create_from_png(IMAGEDIR "/bar-disabled.png");

  init_tray_icon(dr);
#endif /* HAVE_APP_INDICATOR */

  g_timeout_add_seconds(12, (GSourceFunc)update_status, dr);

  g_timeout_add_seconds(1, (GSourceFunc)maybe_change_state, dr);
