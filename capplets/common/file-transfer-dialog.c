...
enum { CANCEL, DONE, LAST_SIGNAL };

guint file_transfer_dialog_signals[LAST_SIGNAL] = {
    0,
};
...
G_DEFINE_TYPE_WITH_PRIVATE(FileTransferDialog, file_transfer_dialog,
                           GTK_TYPE_DIALOG)

static void file_transfer_dialog_update_num_files(FileTransferDialog *dlg) {
  gchar *str;

  if (dlg->priv->total <= 1) return;

  str = g_strdup_printf(_("Copying file: %u of %u"), dlg->priv->nth,
                        dlg->priv->total);
  gtk_progress_bar_set_text(GTK_PROGRESS_BAR(dlg->priv->progress), str);
  g_free(str);
}

static void file_transfer_dialog_response(GtkDialog *dlg, gint response_id) {
  FileTransferDialog *dialog = FILE_TRANSFER_DIALOG(dlg);

  g_cancellable_cancel(dialog->priv->cancellable);
}

static void file_transfer_dialog_finalize(GObject *object) {
...
static void file_transfer_dialog_set_prop(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
  FileTransferDialog *dlg = FILE_TRANSFER_DIALOG(object);
  GFile *file;
  ...
  switch (prop_id) {
    case PROP_FROM_URI:
      ...
      escaped =
          g_uri_escape_string(base, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
    ...
    case PROP_FRACTION_COMPLETE:
      gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(dlg->priv->progress),
    ...
    case PROP_PARENT:
      ...
      if (parent) {
        ...
        gtk_window_set_title(GTK_WINDOW(dlg), _("Copying files"));
      break;
    default:
...
static void file_transfer_dialog_get_prop(GObject *object, guint prop_id,
...
static void file_transfer_dialog_class_init(FileTransferDialogClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_pointer("parent", _("Parent Window"),
                           _("Parent window of the dialog"),
                           G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_FROM_URI,
      g_param_spec_string("from_uri", _("From URI"),
                          _("URI currently transferring from"), NULL,
                          G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_TO_URI,
      g_param_spec_string("to_uri", _("To URI"),
                          _("URI currently transferring to"), NULL,
                          G_PARAM_WRITABLE));

  g_object_class_install_property(
      object_class, PROP_FRACTION_COMPLETE,
      g_param_spec_double("fraction_complete", _("Fraction completed"),
                          _("Fraction of transfer currently completed"), 0, 1,
                          0, G_PARAM_WRITABLE));

  g_object_class_install_property(
      object_class, PROP_NTH_URI,
      g_param_spec_uint("nth_uri", _("Current URI index"),
                        _("Current URI index - starts from 1"), 1, INT_MAX, 1,
                        G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_TOTAL_URIS,
      g_param_spec_uint("total_uris", _("Total URIs"),
                        _("Total number of URIs"), 1, INT_MAX, 1,
                        G_PARAM_READWRITE));

  file_transfer_dialog_signals[CANCEL] = g_signal_new(
      "cancel", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_FIRST, 0, NULL,
      NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  file_transfer_dialog_signals[DONE] = g_signal_new(
      "done", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST, 0, NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void file_transfer_dialog_init(FileTransferDialog *dlg) {
  GtkWidget *vbox;
  GtkWidget *hbox;
  ...
  gtk_box_pack_start(GTK_BOX(progress_vbox), dlg->priv->progress, FALSE, FALSE,
                     0);
...
GtkWidget *file_transfer_dialog_new(void) {
  return GTK_WIDGET(g_object_new(file_transfer_dialog_get_type(), NULL));
}

GtkWidget *file_transfer_dialog_new_with_parent(GtkWindow *parent) {
  return GTK_WIDGET(
      g_object_new(file_transfer_dialog_get_type(), "parent", parent, NULL));
}

static gboolean file_transfer_job_update(gpointer user_data) {
  ...
  g_object_set(data->dialog, "from_uri", data->source, "to_uri", data->target,
               "nth_uri", data->current_file, "fraction_complete", fraction,
               NULL);
  return FALSE;
}

static void file_transfer_job_progress(goffset current_bytes,
                                       goffset total_bytes,
                                       gpointer user_data) {
  FileTransferData *data = user_data;
...
static void file_transfer_job_destroy(FileTransferJob *job) {
...
static gboolean file_transfer_dialog_done(FileTransferDialog *dialog) {
  g_signal_emit(dialog, file_transfer_dialog_signals[DONE], 0, NULL);
  return FALSE;
}

static gboolean file_transfer_dialog_cancel(FileTransferDialog *dialog) {
  g_signal_emit(dialog, file_transfer_dialog_signals[CANCEL], 0, NULL);
  return FALSE;
}

static gboolean file_transfer_dialog_overwrite(gpointer user_data) {
  FileTransferData *data = user_data;
  GtkDialog *dialog;
  ...
  if (dialog != NULL) {
    ...
    dialog = GTK_DIALOG(gtk_message_dialog_new(
        GTK_WINDOW(data->dialog), GTK_DIALOG_MODAL, GTK_MESSAGE_QUESTION,
        GTK_BUTTONS_NONE,
...
static gboolean file_transfer_job_schedule(GIOSchedulerJob *io_job,
                                           GCancellable *cancellable,
                                           FileTransferJob *job) {
  GFile *source, *target;
  gboolean success;
  ...
  g_io_scheduler_job_send_to_mainloop(io_job, file_transfer_job_update, &data,
                                      NULL);
  ...
  do {
    ...
    error = NULL;
    success =
        g_file_copy(source, target, copy_flags, job->dialog->priv->cancellable,
                    file_transfer_job_progress, &data, &error);
    ...
    if (error != NULL) {
      if (error->domain == G_IO_ERROR && error->code == G_IO_ERROR_EXISTS) {
        ...
        g_io_scheduler_job_send_to_mainloop(
            io_job, file_transfer_dialog_overwrite, &data, NULL);
  ...
  if (success) {
    if (job->source_files == NULL) {
      g_io_scheduler_job_send_to_mainloop_async(
          io_job, (GSourceFunc)file_transfer_dialog_done,
          g_object_ref(job->dialog), g_object_unref);
      return FALSE;
    }
  } else /* error on copy or cancelled */
  {
    g_io_scheduler_job_send_to_mainloop_async(
        io_job, (GSourceFunc)file_transfer_dialog_cancel,
        g_object_ref(job->dialog), g_object_unref);
    return FALSE;
  }
...
void file_transfer_dialog_copy_async(FileTransferDialog *dlg,
  ...
  for (l = g_list_last(source_files); l; l = l->prev, ++n) {
    job->source_files =
        g_slist_prepend(job->source_files, g_object_ref(l->data));
  }
  for (l = g_list_last(target_files); l; l = l->prev) {
    job->target_files =
        g_slist_prepend(job->target_files, g_object_ref(l->data));
  }

  g_object_set(dlg, "total_uris", n, NULL);

  g_io_scheduler_push_job((GIOSchedulerJobFunc)file_transfer_job_schedule, job,
                          (GDestroyNotify)file_transfer_job_destroy, priority,
