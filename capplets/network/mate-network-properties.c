 ...
 * GSettings schemas is
 * http://git.gnome.org/browse/gsettings-desktop-schemas/tree/
...
static GtkTreeModel *create_listmodel(void) {
  GtkListStore *store;
...
static GtkTreeModel *populate_listmodel(GtkListStore *store, GSList *list) {
  GtkTreeIter iter;
  GSList *pointer;
...
static GtkWidget *config_treeview(GtkTreeView *tree, GtkTreeModel *model) {
  GtkCellRenderer *renderer;

  renderer = gtk_cell_renderer_text_new();
  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tree), -1, "Hosts",
...
static void read_ignore_hosts_from_gsettings(void) {
  gchar **array;
  ignore_hosts = NULL;
  gint i;
...
static void save_ignore_hosts_to_gsettings(void) {
  GArray *array;
  GSList *l;
  ...
  g_settings_set_strv(proxy_settings, IGNORE_HOSTS_KEY,
                      (const gchar **)array->data);
  g_array_free(array, TRUE);
}

static void cb_add_url(GtkButton *button, gpointer data) {
  GtkBuilder *builder = GTK_BUILDER(data);

  const gchar *entry_text = gtk_entry_get_text(
      GTK_ENTRY(gtk_builder_get_object(builder, "entry_url")));
  ...
  gtk_entry_set_text(GTK_ENTRY(gtk_builder_get_object(builder, "entry_url")),
                     "");

  save_ignore_hosts_to_gsettings();
}

static void cb_remove_url(GtkButton *button, gpointer data) {
  GtkBuilder *builder = GTK_BUILDER(data);
...
static void cb_dialog_response(GtkDialog *dialog, gint response_id) {
  if (response_id == GTK_RESPONSE_HELP)
    capplet_help(GTK_WINDOW(dialog), "goscustdesk-50");
...
static void cb_details_dialog_response(GtkDialog *dialog, gint response_id) {
  if (response_id == GTK_RESPONSE_HELP)
    capplet_help(GTK_WINDOW(dialog), "goscustdesk-50");
  else {
...
static void cb_use_auth_toggled(GtkToggleButton *toggle, GtkWidget *table) {
  gtk_widget_set_sensitive(table, gtk_toggle_button_get_active(toggle));
}

static void cb_http_details_button_clicked(GtkWidget *button,
                                           GtkWidget *parent) {
  GtkBuilder *builder;
  ...
  if (gtk_builder_add_objects_from_resource(
          builder, "/org/mate/mcc/network/mate-network-properties.ui",
  ...
  g_signal_connect(widget, "response", G_CALLBACK(cb_details_dialog_response),
                   NULL);
...
static void proxy_mode_radiobutton_clicked_cb(GtkWidget *widget,
                                              GtkBuilder *builder) {
  GSList *mode_group;
  int mode;
  int old_mode;
...
static void connect_sensitivity_signals(GtkBuilder *builder,
                                        GSList *mode_group) {
  for (; mode_group != NULL; mode_group = mode_group->next) {
    g_signal_connect(mode_group->data, "clicked",
                     G_CALLBACK(proxy_mode_radiobutton_clicked_cb), builder);
  }
}

static void cb_ignore_hosts_gsettings_changed(GSettings *settings, gchar *key,
                                              gpointer user_data) {
  g_slist_free_full(ignore_hosts, g_free);
...
static void setup_dialog(GtkBuilder *builder) {
  GSList *mode_group;
  ...
  mode_group = gtk_radio_button_get_group(
      GTK_RADIO_BUTTON(gtk_builder_get_object(builder, "none_radiobutton")));
  ...
  g_settings_bind(http_proxy_settings, HTTP_PROXY_HOST_KEY,
                  ...
                  G_SETTINGS_BIND_DEFAULT);
  g_signal_connect(gtk_builder_get_object(builder, "details_button"), "clicked",
  ...
  g_settings_bind(https_proxy_settings, SECURE_PROXY_PORT_KEY,
                  gtk_builder_get_object(builder, "secure_port_spinbutton"),
                  "value", G_SETTINGS_BIND_DEFAULT);
  ...
  g_settings_bind(ftp_proxy_settings, FTP_PROXY_PORT_KEY,
                  gtk_builder_get_object(builder, "ftp_port_spinbutton"),
                  "value", G_SETTINGS_BIND_DEFAULT);
  ...
  g_settings_bind(socks_proxy_settings, SOCKS_PROXY_PORT_KEY,
                  gtk_builder_get_object(builder, "socks_port_spinbutton"),
                  "value", G_SETTINGS_BIND_DEFAULT);
  ...
  g_signal_connect(gtk_builder_get_object(builder, "button_add_url"), "clicked",
                   G_CALLBACK(cb_add_url), builder);
  g_signal_connect(gtk_builder_get_object(builder, "entry_url"), "activate",
                   G_CALLBACK(cb_add_url), builder);
...
int main(int argc, char **argv) {
  GtkBuilder *builder;
  GError *error = NULL;
  gchar *builder_widgets[] = {
      "network_dialog", "adjustment1",       "adjustment2", "adjustment3",
      "adjustment4",    "delete_button_img", NULL};
  GtkWidget *widget;
  ...
  if (gtk_builder_add_objects_from_resource(
          builder, "/org/mate/mcc/network/mate-network-properties.ui",
          builder_widgets, &error) == 0) {
    g_warning("Could not load main dialog: %s", error->message);
    g_error_free(error);
    g_object_unref(builder);
    return (EXIT_FAILURE);
  }
