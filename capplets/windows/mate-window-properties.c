...
enum { FOCUS_MODE_CLICK, FOCUS_MODE_SLOPPY, FOCUS_MODE_MOUSE };
...
static void update_sensitivity(void) {
  ...
  gtk_widget_set_sensitive(
      focus_mode_mouse_checkbutton,
      g_settings_get_enum(marco_settings, MARCO_FOCUS_KEY) != FOCUS_MODE_CLICK);
  gtk_widget_set_sensitive(
      autoraise_delay_hbox,
      g_settings_get_enum(marco_settings, MARCO_FOCUS_KEY) != FOCUS_MODE_CLICK);
  gtk_widget_set_sensitive(
      autoraise_delay_spinbutton,
      g_settings_get_enum(marco_settings, MARCO_FOCUS_KEY) !=
              FOCUS_MODE_CLICK &&
          g_settings_get_boolean(marco_settings, MARCO_AUTORAISE_KEY));
}

static void marco_settings_changed_callback(GSettings *settings,
                                            const gchar *key,
                                            gpointer user_data) {
  update_sensitivity();
}

static void mouse_focus_toggled_callback(GtkWidget *button, void *data) {
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(focus_mode_checkbutton))) {
    g_settings_set_enum(marco_settings, MARCO_FOCUS_KEY,
                        gtk_toggle_button_get_active(
                            GTK_TOGGLE_BUTTON(focus_mode_mouse_checkbutton))
                            ? FOCUS_MODE_MOUSE
                            : FOCUS_MODE_SLOPPY);
  } else {
...
static void mouse_focus_changed_callback(GSettings *settings, const gchar *key,
                                         gpointer user_data) {
  if (g_settings_get_enum(settings, key) == FOCUS_MODE_MOUSE) {
    ...
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(focus_mode_mouse_checkbutton), TRUE);
  } else if (g_settings_get_enum(settings, key) == FOCUS_MODE_SLOPPY) {
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(focus_mode_checkbutton),
                                 TRUE);
    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(focus_mode_mouse_checkbutton), FALSE);
  } else {
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(focus_mode_checkbutton),
                                 FALSE);
    gtk_toggle_button_set_active(
...
static void autoraise_delay_spinbutton_value_callback(GtkWidget *spinbutton,
                                                      void *data) {
  g_settings_set_int(
      marco_settings, MARCO_AUTORAISE_DELAY_KEY,
      gtk_spin_button_get_value(GTK_SPIN_BUTTON(spinbutton)) * 1000);
}

static void double_click_titlebar_changed_callback(GtkWidget *optionmenu,
                                                   void *data) {
...
static void titlebar_layout_changed_callback(GtkWidget *optionmenu) {
  gint value = gtk_combo_box_get_active(GTK_COMBO_BOX(optionmenu));

  if (value < MARCO_BUTTON_LAYOUT_COUNT) {
    g_settings_set_string(marco_settings, MARCO_BUTTON_LAYOUT_KEY,
                          button_layout[value]);
    g_settings_set_string(interface_settings, GTK_BUTTON_LAYOUT_KEY,
                          button_layout[value]);
  }
  /* Custom Layout */
  else if ((value == MARCO_BUTTON_LAYOUT_COUNT) &&
...
static void set_titlebar_button_layout(void) {
  gchar *str;
  ...
  if (!found) {
    ...
    gtk_combo_box_text_append_text(
        GTK_COMBO_BOX_TEXT(titlebar_layout_optionmenu), _("Custom"));
...
static void alt_click_radio_toggled_callback(GtkWidget *radio, void *data) {
...
static void set_alt_click_value(void) {
  ...
  mouse_move_modifier =
      g_settings_get_string(marco_settings, MARCO_MOUSE_MODIFIER_KEY);
  ...
  if (mouse_move_modifier != NULL) {
    for (i = 0; i < n_mouse_modifiers; i++) {
      ...
      if (strcmp(value, mouse_move_modifier) == 0) {
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(mouse_modifiers[i].radio), TRUE);
        match_found = TRUE;
        break;
      }
      g_free(value);
...
static void wm_unsupported(void) {
  ...
  gtk_message_dialog_set_markup(GTK_MESSAGE_DIALOG(no_tool_dialog),
                                _("The current window manager is unsupported"));
...
static void wm_changed_callback(GdkScreen *screen, void *data) {
  const char *current_wm;

  current_wm = gdk_x11_screen_get_window_manager_name(screen);

  gtk_widget_set_sensitive(dialog_win,
                           g_strcmp0(current_wm, WM_COMMON_MARCO) == 0);
}

static void response_cb(GtkWidget *dialog_win, int response_id, void *data) {
...
int main(int argc, char **argv) {
  ...
  builder = gtk_builder_new_from_resource(
      "/org/mate/mcc/windows/window-properties.ui");

  gtk_builder_add_callback_symbols(
      builder, "on_dialog_win_response", G_CALLBACK(response_cb),
      "on_double_click_titlebar_optionmenu_changed",
      G_CALLBACK(double_click_titlebar_changed_callback),
      "on_autoraise_delay_spinbutton_value_changed",
      G_CALLBACK(autoraise_delay_spinbutton_value_callback),
      "on_titlebar_layout_optionmenu_changed",
      G_CALLBACK(titlebar_layout_changed_callback),
      "on_focus_mode_checkbutton_toggled",
      G_CALLBACK(mouse_focus_toggled_callback),
      "on_focus_mode_mouse_checkbutton_toggled",
...
#define GET_WIDGET(x) GTK_WIDGET(gtk_builder_get_object(builder, x))
  ...
  compositing_fast_alt_tab_checkbutton =
      GET_WIDGET("compositing_fast_alt_tab_checkbutton");
  double_click_titlebar_optionmenu =
      GET_WIDGET("double_click_titlebar_optionmenu");
...
#undef GET_WIDGET
  ...
  gtk_combo_box_set_active(
      GTK_COMBO_BOX(double_click_titlebar_optionmenu),
  ...
  /* Behaviour */
  g_settings_bind(marco_settings, MARCO_SHOW_TAB_BORDER_KEY,
                  show_tab_border_checkbutton, "active",
                  G_SETTINGS_BIND_DEFAULT);

  g_settings_bind(marco_settings, MARCO_COMPOSITING_FAST_ALT_TAB_KEY,
                  compositing_fast_alt_tab_checkbutton, "active",
                  G_SETTINGS_BIND_DEFAULT | G_SETTINGS_BIND_INVERT_BOOLEAN);

  /* Placement */
  g_settings_bind(marco_settings, MARCO_CENTER_NEW_WINDOWS_KEY,
                  center_new_windows_checkbutton, "active",
                  G_SETTINGS_BIND_DEFAULT);

  g_settings_bind(marco_settings, MARCO_ALLOW_TILING_KEY,
                  enable_tiling_checkbutton, "active", G_SETTINGS_BIND_DEFAULT);

  g_settings_bind(marco_settings, MARCO_ALLOW_TOP_TILING_KEY,
                  allow_top_tiling_checkbutton, "active",
                  G_SETTINGS_BIND_DEFAULT);

  /* Compositing Manager */
  g_settings_bind(marco_settings, MARCO_COMPOSITING_MANAGER_KEY,
                  compositing_checkbutton, "active", G_SETTINGS_BIND_DEFAULT);

  /* Initialize the checkbox state appropriately */
  mouse_focus_changed_callback(marco_settings, MARCO_FOCUS_KEY, NULL);

  g_settings_bind(marco_settings, MARCO_AUTORAISE_KEY, autoraise_checkbutton,
...
static void fill_radio(GtkRadioButton *group, MouseClickModifier *modifier) {
  modifier->radio =
      gtk_radio_button_new_with_mnemonic_from_widget(group, modifier->name);
...
static void reload_mouse_modifiers(void) {
  ...
  keymap = XGetKeyboardMapping(
      gdk_x11_display_get_xdisplay(gdk_display_get_default()), min_keycode,
      max_keycode - min_keycode, &keysyms_per_keycode);

  modmap = XGetModifierMapping(
  ...
  if ((1 << mod_meta) != Mod1Mask) have_meta = TRUE;
  if (mod_super != 0 && mod_super != mod_meta) have_super = TRUE;
  ...
  while (i < n_mouse_modifiers) {
    ...
    if (mouse_modifiers[i].radio) gtk_widget_destroy(mouse_modifiers[i].radio);
    ++i;
  }
  ...
  if (have_super) ++n_mouse_modifiers;
  if (have_hyper) ++n_mouse_modifiers;
  if (have_meta) ++n_mouse_modifiers;
