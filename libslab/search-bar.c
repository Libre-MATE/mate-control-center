...
static void nld_search_bar_class_init(NldSearchBarClass *nld_search_bar_class) {
  ...
  signals[SEARCH] =
      g_signal_new("search", G_TYPE_FROM_CLASS(nld_search_bar_class),
                   ...
                   G_STRUCT_OFFSET(NldSearchBarClass, search), NULL, NULL,
                   nld_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
}

static void nld_search_bar_init(NldSearchBar *search_bar) {
  ...
  g_signal_connect(entry, "activate", G_CALLBACK(emit_search_callback),
                   search_bar);

  priv->search_timeout = -1;
}

static void nld_search_bar_finalize(GObject *object) {
  NldSearchBarPrivate *priv =
      nld_search_bar_get_instance_private(NLD_SEARCH_BAR(object));

  if (priv->timeout_id) g_source_remove(priv->timeout_id);

  G_OBJECT_CLASS(nld_search_bar_parent_class)->finalize(object);
}

static gboolean nld_search_bar_focus(GtkWidget *widget, GtkDirectionType dir) {
  NldSearchBarPrivate *priv =
      nld_search_bar_get_instance_private(NLD_SEARCH_BAR(widget));
...
static void nld_search_bar_grab_focus(GtkWidget *widget) {
  NldSearchBarPrivate *priv =
      nld_search_bar_get_instance_private(NLD_SEARCH_BAR(widget));
...
static gboolean search_timeout(gpointer search_bar) {
  NldSearchBarPrivate *priv = nld_search_bar_get_instance_private(search_bar);
...
static void entry_changed(GtkWidget *entry, gpointer search_bar) {
  NldSearchBarPrivate *priv = nld_search_bar_get_instance_private(search_bar);

  if (priv->search_timeout == 0)
    emit_search(search_bar);
  else if (priv->search_timeout > 0) {
    if (priv->timeout_id != 0) g_source_remove(priv->timeout_id);
    priv->timeout_id =
...
void nld_search_bar_set_search_timeout(NldSearchBar *search_bar,
                                       int search_timeout) {
  NldSearchBarPrivate *priv = nld_search_bar_get_instance_private(search_bar);

  if (priv->search_timeout != -1 && search_timeout == -1)
    g_signal_handlers_disconnect_by_func(priv->entry, entry_changed,
                                         search_bar);
  else if (search_timeout != -1) {
    g_signal_connect(priv->entry, "changed", G_CALLBACK(entry_changed),
                     search_bar);
  }
...
void nld_search_bar_set_text(NldSearchBar *search_bar, const char *text,
                             gboolean activate) {
  NldSearchBarPrivate *priv = nld_search_bar_get_instance_private(search_bar);

  gtk_entry_set_text(priv->entry, text);
  if (activate) emit_search(search_bar);
}
