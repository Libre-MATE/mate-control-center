...
static void LocationChanged(TimezoneMap *map, TzLocation *location,
                            TimeAdmin *ta);

enum { CITY_COL_CITY_HUMAN_READABLE, CITY_COL_ZONE, CITY_NUM_COLS };
...
static void load_backward_tz(TzDB *tz_db) {
  ...
  while (fgets(buf, 128, fp)) {
    ...
    if (g_ascii_strncasecmp(buf, "Link\t", 5) != 0) continue;

    items = g_strsplit(buf, "\t", -1);
    real = NULL;
    alias = NULL;
    for (j = 1; items[j] != NULL; j++) {
      if (items[j][0] == '\0') continue;
      if (real == NULL) {
        real = items[j];
        continue;
      }
...
static GtkWidget *GetTimeZoneMap(TimeAdmin *ta) {
  ...
  g_signal_connect(map, "location-changed", G_CALLBACK(LocationChanged), ta);
...
static char *translated_city_name(TzLocation *loc) {
  ...
  split_translated = g_regex_split_simple(
      "[\\x{2044}\\x{2215}\\x{29f8}\\x{ff0f}/]", zone_translated, 0, 0);
  ...
  name = g_strdup_printf(C_("timezone loc", "%s, %s"),
                         split_translated[length - 1], country);

  return name;
}

static void update_timezone(TimezoneMap *map) {
  ...
  tz_desc = g_strdup_printf(
      "%s (%s)", g_date_time_get_timezone_abbreviation(date), utc_label);
  time_label = g_date_time_format(date, _("%R"));

  bubble_text = g_strdup_printf(
      "<b>%s</b>\n"
      "<small>%s</small>\n"
      "<b>%s</b>",
      tz_desc, city_country, time_label);
...
static void get_initial_timezone(TimeAdmin *ta) {
  ...
  if (timezone == NULL ||
    ...
    g_warning("Timezone '%s' is unhandled,setting %s as default",
              timezone ? timezone : "(null)", DEFAULT_TZ);
    timezone_map_set_timezone(TIMEZONEMAP(ta->map), DEFAULT_TZ);
  }
  update_timezone(TIMEZONEMAP(ta->map));
}

static void LoadCities(TzLocation *loc, GtkListStore *CityStore) {
  g_autofree gchar *human_readable = NULL;

  human_readable = translated_city_name(loc);
  gtk_list_store_insert_with_values(
      CityStore, NULL, 0, CITY_COL_CITY_HUMAN_READABLE, human_readable,
      CITY_COL_ZONE, loc->zone, -1);
}
...
static void CreateCityList(TimeAdmin *ta) {
  ...
  ta->CityListStore =
      gtk_list_store_new(CITY_NUM_COLS, G_TYPE_STRING, G_TYPE_STRING);
...
static gboolean CityChanged(GtkEntryCompletion *completion, GtkTreeModel *model,
                            GtkTreeIter *iter, TimeAdmin *self) {
  GtkWidget *entry;
  g_autofree gchar *zone = NULL;

  gtk_tree_model_get(model, iter, CITY_COL_ZONE, &zone, -1);
...
static void ChoooseTimezoneDone(GtkWidget *widget, TimeAdmin *ta) {
  TimezoneMap *map;
...
static void ChoooseTimezoneClose(GtkWidget *widget, TimeAdmin *ta) {
  gtk_widget_hide_on_delete(GTK_WIDGET(ta->dialog));
}
...
void SetupTimezoneDialog(TimeAdmin *ta) {
  ...
  ta->dialog =
      gtk_dialog_new_with_buttons(_("Time Zone Selection"), NULL,
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(ta->TZclose),
                              "text-button");
  gtk_widget_set_can_default(ta->TZclose, TRUE);
  gtk_dialog_add_action_widget(GTK_DIALOG(ta->dialog), ta->TZclose,
                               GTK_RESPONSE_CANCEL);
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(ta->TZconfire),
                              "text-button");
  gtk_widget_set_can_default(ta->TZconfire, TRUE);
  gtk_dialog_add_action_widget(GTK_DIALOG(ta->dialog), ta->TZconfire,
                               GTK_RESPONSE_OK);

  g_signal_connect(ta->TZconfire, "clicked", G_CALLBACK(ChoooseTimezoneDone),
                   ta);

  g_signal_connect(ta->TZclose, "clicked", G_CALLBACK(ChoooseTimezoneClose),
                   ta);
  ...
  g_signal_connect(gtk_entry_get_completion(GTK_ENTRY(ta->TimezoneEntry)),
                   "match-selected", G_CALLBACK(CityChanged), ta);

  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(ta->dialog))), Vbox, TRUE,
      TRUE, 8);
}
...
struct {
  ...
  const char *dest;
    ...
    {"Asia/Istanbul",
     "Europe/Istanbul"}, /* Istanbul is in both Europe and Asia */
...
static gboolean compare_timezones(const char *a, const char *b) {
  if (g_str_equal(a, b)) return TRUE;
  if (strchr(b, '/') == NULL) {
    g_autofree gchar *prefixed = NULL;

    prefixed = g_strdup_printf("/%s", b);
    if (g_str_has_suffix(a, prefixed)) return TRUE;
  }
...
char *tz_info_get_clean_name(TzDB *tz_db, const char *tz) {
  ...
  if (!replaced) timezone = tz;

  ret = g_hash_table_lookup(tz_db->backward, timezone);
  if (ret == NULL) return g_strdup(timezone);
  return g_strdup(ret);
}
...
void RunTimeZoneDialog(GtkButton *button, gpointer data) {
  TimeAdmin *ta = (TimeAdmin *)data;

  gtk_widget_show_all(GTK_WIDGET(ta->dialog));
}

static void tz_location_free(TzLocation *loc, gpointer data) {
  g_free(loc->country);
  g_free(loc->zone);
  g_free(loc->comment);
  g_free(loc);
...
GPtrArray *tz_get_locations(TzDB *db) { return db->locations; }
