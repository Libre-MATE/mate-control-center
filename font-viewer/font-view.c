...
#define FONT_VIEW_APPLICATION(obj)                               \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), FONT_VIEW_TYPE_APPLICATION, \
                              FontViewApplication))
...
static const gchar *app_menu =
    ...
    "	     <attribute name=\"label\" translatable=\"yes\">About Font "
    "Viewer</attribute>"
    "      </item>"
    "      <item>"
...
static void strip_whitespace(gchar **original) {
  GString *reassembled;
  gchar **split;
  const gchar *str;
  gint idx, n_stripped;
  ...
  for (idx = 0; split[idx] != NULL; idx++) {
    ...
    if (n_stripped++ > 0) g_string_append(reassembled, "\n");
...
static void strip_version(gchar **original) {
  gchar *ptr, *stripped;
...
static void add_row(GtkWidget *grid, const gchar *name, const gchar *value,
                    gboolean multiline) {
  ...
  gtk_grid_attach_next_to(GTK_GRID(grid), label, name_w, GTK_POS_RIGHT, 1, 1);
}

static void populate_grid(FontViewApplication *self, GtkWidget *grid,
                          FT_Face face) {
  gchar *s;
  GFileInfo *info;
  ...
  if (face->style_name) add_row(grid, _("Style"), face->style_name, FALSE);
  ...
  if (FT_IS_SFNT(face)) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (FT_Get_Sfnt_Name(face, i, &sname) != 0) continue;
      ...
      switch (sname.name_id) {
        ...
        case TT_NAME_ID_VERSION_STRING:
          g_free(version);
          version = g_convert((gchar *)sname.string, sname.string_len, "UTF-8",
...
static void install_button_refresh_appearance(FontViewApplication *self,
                                              GError *error) {
  FT_Face face;
  ...
  if (error != NULL) {
    ...
    if (font_view_model_get_iter_for_face(FONT_VIEW_MODEL(self->model), face,
                                          NULL)) {
...
static void font_install_finished_cb(GObject *source_object, GAsyncResult *res,
                                     gpointer user_data) {
...
static void font_model_config_changed_cb(FontViewModel *model,
                                         gpointer user_data) {
  FontViewApplication *self = user_data;

  if (self->font_file != NULL) install_button_refresh_appearance(self, NULL);
}

static void install_button_clicked_cb(GtkButton *button, gpointer user_data) {
  FontViewApplication *self = user_data;
  gchar *dest_filename;
  GError *err = NULL;
  FcConfig *config;
  ...
  g_file_copy_async(self->font_file, dest_file, G_FILE_COPY_NONE, 0, NULL, NULL,
                    NULL, font_install_finished_cb, self);

  g_object_unref(dest_file);
  g_object_unref(dest_location);
}

static void back_button_clicked_cb(GtkButton *button, gpointer user_data) {
  FontViewApplication *self = user_data;
  font_view_application_do_overview(self);
}

static void font_view_show_font_error(FontViewApplication *self,
                                      const gchar *message) {
  GtkWidget *dialog;

  dialog = gtk_message_dialog_new(
      GTK_WINDOW(self->main_window), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
      GTK_BUTTONS_CLOSE, _("This font could not be displayed."));
...
static void font_widget_error_cb(SushiFontWidget *font_widget,
...
static void font_widget_loaded_cb(SushiFontWidget *font_widget,
                                  gpointer user_data) {
  FontViewApplication *self = user_data;
  ...
  gd_main_toolbar_set_labels(GD_MAIN_TOOLBAR(self->toolbar), face->family_name,
                             face->style_name);

  install_button_refresh_appearance(self, NULL);
}

static void info_button_clicked_cb(GtkButton *button, gpointer user_data) {
  FontViewApplication *self = user_data;
  ...
  g_object_set(grid, "margin-top", 6, "margin-start", 16, "margin-end", 16,
  ...
  dialog = gtk_dialog_new_with_buttons(
      _("Info"), GTK_WINDOW(self->main_window),
      GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, "gtk-close",
      GTK_RESPONSE_CLOSE, NULL);
  gtk_container_add(
      GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), grid);
  g_signal_connect(dialog, "response", G_CALLBACK(gtk_widget_destroy), NULL);
  gtk_widget_show_all(dialog);
}

static gboolean font_visible_func(GtkTreeModel *model, GtkTreeIter *iter,
                                  gpointer data) {
...
static void font_view_ensure_model(FontViewApplication *self) {
  ...
  gtk_tree_model_filter_set_visible_func(
      GTK_TREE_MODEL_FILTER(self->filter_model), font_visible_func, self, NULL);
}

static void font_view_application_do_open(FontViewApplication *self,
                                          GFile *file, gint face_index) {
  gchar *uri;
  ...
  /* add install button */
  self->install_button = gd_main_toolbar_add_button(
      GD_MAIN_TOOLBAR(self->toolbar), NULL, _("Install"), FALSE);
  g_signal_connect(self->install_button, "clicked",
                   G_CALLBACK(install_button_clicked_cb), self);

  self->back_button = gd_main_toolbar_add_button(
      GD_MAIN_TOOLBAR(self->toolbar), "go-previous-symbolic", _("Back"), TRUE);
  ...
  if (self->font_widget == NULL) {
    ...
    gtk_widget_override_background_color(self->font_widget,
...
static gboolean icon_view_release_cb(GtkWidget *widget, GdkEventButton *event,
                                     gpointer user_data) {
  FontViewApplication *self = user_data;
  GtkTreePath *path;
  GtkTreeIter iter;
  ...
  path =
      gtk_icon_view_get_path_at_pos(GTK_ICON_VIEW(widget), event->x, event->y);

  if (path != NULL &&
      gtk_tree_model_get_iter(self->filter_model, &filter_iter, path)) {
    gtk_tree_model_filter_convert_iter_to_child_iter(
        GTK_TREE_MODEL_FILTER(self->filter_model), &iter, &filter_iter);
    gtk_tree_model_get(self->model, &iter, COLUMN_PATH, &font_path,
                       COLUMN_FACE_INDEX, &face_index, -1);
...
static void font_view_application_do_overview(FontViewApplication *self) {
  ...
  gd_main_toolbar_set_labels(GD_MAIN_TOOLBAR(self->toolbar), _("All Fonts"),
                             NULL);
  ...
  if (self->icon_view == NULL) {
    ...
    self->icon_view = icon_view =
        gtk_icon_view_new_with_model(self->filter_model);
    g_object_set(icon_view, "column-spacing", VIEW_COLUMN_SPACING, "margin",
                 VIEW_MARGIN, NULL);
    ...
    g_object_set(cell, "xalign", 0.5, "yalign", 0.5, NULL);

    gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(icon_view), cell, FALSE);
    gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(icon_view), cell, "pixbuf",
                                  COLUMN_ICON);

    cell = gtk_cell_renderer_text_new();
    g_object_set(cell, "alignment", PANGO_ALIGN_CENTER, "xalign", 0.5,
                 "wrap-mode", PANGO_WRAP_WORD_CHAR, "wrap-width",
                 VIEW_ITEM_WRAP_WIDTH, NULL);
    gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(icon_view), cell, FALSE);
    gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(icon_view), cell, "text",
                                  COLUMN_NAME);
...
static void query_info_ready_cb(GObject *object, GAsyncResult *res,
                                gpointer user_data) {
  FontViewApplication *self = user_data;
  GFileInfo *info;
  GError *error = NULL;
...
static void font_view_application_open(GApplication *application, GFile **files,
...
static void action_about(GSimpleAction *action, GVariant *parameter,
                         gpointer user_data) {
  FontViewApplication *self = user_data;
  const gchar *authors[] = {"Mate Developer", "Cosimo Cecchi",
                            "James Henstridge", NULL};

  gtk_show_about_dialog(
      GTK_WINDOW(self->main_window), "version", VERSION, "authors", authors,
      "program-name", _("Font Viewer"), "comments",
      _("View fonts on your system"), "logo-icon-name", FONT_VIEW_ICON_NAME,
      "translator-credits", _("translator-credits"), "license-type",
      GTK_LICENSE_GPL_2_0, "wrap-license", TRUE, NULL);
}
...
static void search_text_changed(GtkEntry *entry, FontViewApplication *self) {
  gtk_tree_model_filter_refilter(GTK_TREE_MODEL_FILTER(self->filter_model));
}

static void font_view_application_startup(GApplication *application) {
  FontViewApplication *self = FONT_VIEW_APPLICATION(application);
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(self->toolbar),
                              "menubar");
  self->search_button = gd_main_toolbar_add_toggle(
      GD_MAIN_TOOLBAR(self->toolbar), NULL, _("Search"), FALSE);
...
static void font_view_application_activate(GApplication *application) {
  FontViewApplication *self = FONT_VIEW_APPLICATION(application);
...
static void font_view_application_quit_mainloop(GApplication *application) {
  G_APPLICATION_CLASS(font_view_application_parent_class)
...
static void font_view_application_dispose(GObject *obj) {
...
static void font_view_application_init(FontViewApplication *self) {
  /* do nothing */
}

static void font_view_application_class_init(FontViewApplicationClass *klass) {
...
static GApplication *font_view_application_new(void) {
  if (!FcInit()) g_critical("Can't initialize fontconfig library");

  return g_object_new(FONT_VIEW_TYPE_APPLICATION, "application-id",
                      "org.mate.font-viewer", "flags",
