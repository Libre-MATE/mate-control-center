...
enum { PROP_0, PROP_WIDTH, PROP_HEIGHT, NUM_PROPERTIES };
...
static gboolean image_drag_motion_cb(GtkWidget *widget, GdkDragContext *context,
                                     gint x, gint y, guint time,
                                     EImageChooser *chooser);
static gboolean image_drag_drop_cb(GtkWidget *widget, GdkDragContext *context,
                                   gint x, gint y, guint time,
                                   EImageChooser *chooser);
...
enum DndTargetType { DND_TARGET_TYPE_URI_LIST };
#define URI_LIST_TYPE "text/uri-list"

static GtkTargetEntry image_drag_types[] = {
    {URI_LIST_TYPE, 0, DND_TARGET_TYPE_URI_LIST},
};
static const int num_image_drag_types =
    sizeof(image_drag_types) / sizeof(image_drag_types[0]);

GtkWidget *e_image_chooser_new(void) {
  return g_object_new(E_TYPE_IMAGE_CHOOSER, "orientation",
                      GTK_ORIENTATION_VERTICAL, NULL);
}

GtkWidget *e_image_chooser_new_with_size(int width, int height) {
  return g_object_new(E_TYPE_IMAGE_CHOOSER, "width", width, "height", height,
                      "orientation", GTK_ORIENTATION_VERTICAL, NULL);
}

static void e_image_chooser_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
...
static void e_image_chooser_get_property(GObject *object, guint prop_id,
...
static void e_image_chooser_class_init(EImageChooserClass *klass) {
  ...
  image_chooser_signals[CHANGED] = g_signal_new(
      "changed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(EImageChooserClass, changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  properties[PROP_WIDTH] =
      g_param_spec_int("width", "Chooser width", "Chooser width to show image",
                       0, G_MAXINT, 32, G_PARAM_READWRITE);
  properties[PROP_HEIGHT] = g_param_spec_int("height", "Chooser height",
                                             "Chooser height to show image", 0,
                                             G_MAXINT, 32, G_PARAM_READWRITE);
...
static void e_image_chooser_init(EImageChooser *chooser) {
  ...
  gtk_drag_dest_set(priv->image, 0, image_drag_types, num_image_drag_types,
                    GDK_ACTION_COPY);
  g_signal_connect(priv->image, "drag_motion", G_CALLBACK(image_drag_motion_cb),
                   chooser);
  g_signal_connect(priv->image, "drag_drop", G_CALLBACK(image_drag_drop_cb),
                   chooser);
  g_signal_connect(priv->image, "drag_data_received",
...
static gboolean set_image_from_data(EImageChooser *chooser, char *data,
                                    int length) {
  gboolean rv = FALSE;
  ...
  if (pixbuf) {
    ...
    if (priv->scaleable) {
      ...
      scaled = gdk_pixbuf_scale_simple(pixbuf, priv->width, priv->height,
                                       GDK_INTERP_BILINEAR);
  ...
  if (!strcmp(target_type, URI_LIST_TYPE)) {
    ...
    if (istream != NULL) {
      ...
      if (info != NULL) {
        ...
        success =
            g_input_stream_read_all(istream, buf, size, &size, NULL, NULL);
...
gboolean e_image_chooser_set_from_file(EImageChooser *chooser,
  ...
  if (!set_image_from_data(chooser, data, data_length)) g_free(data);
