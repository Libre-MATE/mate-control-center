...
/* Signals */
enum { DONE, POSTPONE, LAST_SIGNAL };
...
static void postpone_clicked_cb(GtkWidget *button, GtkWidget *window);
static gboolean label_draw_event_cb(GtkLabel *label, cairo_t *cr,
                                    gpointer user_data);
static void label_size_allocate_cb(GtkLabel *label, GdkRectangle *allocation,
                                   gpointer user_data);
...
static void drw_break_window_class_init(DrwBreakWindowClass *klass) {
  ...
  signals[POSTPONE] =
      g_signal_new("postpone", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0,
                   NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[DONE] =
      g_signal_new("done", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL,
...
static void drw_break_window_init(DrwBreakWindow *window) {
  ...
  gdk_monitor_get_geometry(gdk_display_get_monitor(display, root_monitor),
                           &monitor);
  ...
  right_padding = WidthOfScreen(gdk_x11_screen_get_xscreen(screen)) / scale -
                  monitor.width - monitor.x;
  bottom_padding = HeightOfScreen(gdk_x11_screen_get_xscreen(screen)) / scale -
  ...
  if (allow_postpone) {
    ...
    priv->postpone_sensitize_id =
        g_timeout_add_seconds(5, (GSourceFunc)postpone_sensitize_cb, window);

    g_signal_connect(priv->postpone_button, "clicked",
                     G_CALLBACK(postpone_clicked_cb), window);

    gtk_box_pack_end(GTK_BOX(button_box), priv->postpone_button, FALSE, TRUE,
                     0);
  ...
  priv->image =
      gtk_image_new_from_icon_name("process-stop", GTK_ICON_SIZE_DIALOG);
  ...
  g_signal_connect(priv->break_label, "draw", G_CALLBACK(label_draw_event_cb),
                   NULL);

  g_signal_connect_after(priv->break_label, "size-allocate",
                         G_CALLBACK(label_size_allocate_cb), NULL);

  str = g_strdup_printf(
      "<span size=\"xx-large\" foreground=\"white\"><b>%s</b></span>",
      _("Take a break!"));
  ...
  g_signal_connect(priv->clock_label, "draw", G_CALLBACK(label_draw_event_cb),
                   NULL);

  g_signal_connect_after(priv->clock_label, "size-allocate",
                         G_CALLBACK(label_size_allocate_cb), NULL);
  ...
  priv->clock_timeout_id =
      g_timeout_add(1000, (GSourceFunc)clock_timeout_cb, window);
#ifdef HAVE_CANBERRA_GTK
  ca_context_play(ca_gtk_context_get(), 0, CA_PROP_EVENT_ID,
                  "desktop-screen-lock", NULL);
#endif
}
...
GtkWidget *drw_break_window_new(void) {
  ...
  object = g_object_new(DRW_TYPE_BREAK_WINDOW, "type", GTK_WINDOW_POPUP,
                        "skip-taskbar-hint", TRUE, "skip-pager-hint", TRUE,
                        "focus-on-map", TRUE, NULL);
...
static gboolean clock_timeout_cb(DrwBreakWindow *window) {
  ...
  txt = g_strdup_printf(
      "<span size=\"25000\" foreground=\"white\"><b>%d:%02d</b></span>",
      minutes, seconds);
...
static void postpone_entry_activate_cb(GtkWidget *entry,
...
static gboolean grab_on_window(GdkWindow *window, guint32 activate_time) {
  GdkDisplay *display;
  GdkSeat *seat;
  ...
  return (gdk_seat_grab(seat, window, GDK_SEAT_CAPABILITY_ALL, TRUE, NULL, NULL,
                        NULL, NULL) == GDK_GRAB_SUCCESS);
}

static gboolean postpone_cancel_cb(DrwBreakWindow *window) {
  DrwBreakWindowPrivate *priv;
...
static gboolean postpone_entry_key_press_event_cb(GtkEntry *entry,
                                                  GdkEventKey *event,
                                                  DrwBreakWindow *window) {
  DrwBreakWindowPrivate *priv;
  ...
  priv->postpone_timeout_id = g_timeout_add_seconds(
      POSTPONE_CANCEL, (GSourceFunc)postpone_cancel_cb, window);
...
static void postpone_clicked_cb(GtkWidget *button, GtkWidget *window) {
  ...
  priv->postpone_timeout_id = g_timeout_add_seconds(
      POSTPONE_CANCEL, (GSourceFunc)postpone_cancel_cb, bw);

  grab_on_window(gtk_widget_get_window(priv->postpone_entry),
                 gtk_get_current_event_time());

  gtk_widget_grab_focus(priv->postpone_entry);

  g_signal_connect(priv->postpone_entry, "activate",
                   G_CALLBACK(postpone_entry_activate_cb), bw);

  g_signal_connect(priv->postpone_entry, "key_press_event",
                   G_CALLBACK(postpone_entry_key_press_event_cb), bw);
}

static void get_layout_location(GtkLabel *label, gint *xp, gint *yp) {
  GtkWidget *widget;
...
static gboolean label_draw_event_cb(GtkLabel *label, cairo_t *cr,
  ...
  gtk_paint_layout(gtk_widget_get_style(widget), cr,
                   gtk_widget_get_state(widget), FALSE, widget, "label", x, y,
