...
static GFileType get_file_type(GFile *file) {
...
static void add_theme_to_hash_by_name(GHashTable *hash_table, gpointer data) {
  ...
  if (list == NULL) {
    ...
    if (!added) list = g_list_append(list, info);
  }
  g_hash_table_insert(hash_table, g_strdup(info->name), list);
}

static void remove_theme_from_hash_by_name(GHashTable *hash_table,
                                           gpointer data) {
  MateThemeCommonInfo *info = data;
  GList *list;
...
static MateThemeCommonInfo *get_theme_from_hash_by_name(GHashTable *hash_table,
                                                        const gchar *name,
                                                        gint priority) {
  GList *list;
  ...
  while (list) {
    ...
    if (info->priority == priority) return info;
...
static gint theme_compare(MateThemeCommonInfo *a, MateThemeCommonInfo *b) {
  gint cmp;
  ...
  switch (a->type) {
    ...
    case MATE_THEME_TYPE_ICON:
      cmp = mate_theme_icon_info_compare((MateThemeIconInfo *)a,
                                         (MateThemeIconInfo *)b);
      break;
    case MATE_THEME_TYPE_CURSOR:
      cmp = mate_theme_cursor_info_compare((MateThemeCursorInfo *)a,
...
static void theme_free(MateThemeCommonInfo *info) {
  switch (info->type) {
    case MATE_THEME_TYPE_METATHEME:
...
MateThemeMetaInfo *mate_theme_read_meta_theme(GFile *meta_theme_uri) {
  ...
  if (meta_theme_ditem == NULL) return NULL;
  ...
  if (str != NULL) meta_theme_info->notification_theme_name = g_strdup(str);
...
static MateThemeIconInfo *read_icon_theme(GFile *icon_theme_uri) {
  ...
  if (icon_theme_ditem == NULL) return NULL;

  name =
      mate_desktop_item_get_localestring(icon_theme_ditem, "Icon Theme/Name");
  if (!name) {
    name = mate_desktop_item_get_localestring(icon_theme_ditem,
                                              MATE_DESKTOP_ITEM_NAME);
    if (!name) goto read_icon_theme_out;
  }

  /* If index.theme has no Directories entry, it is only a cursor theme */
  directories =
      mate_desktop_item_get_string(icon_theme_ditem, "Icon Theme/Directories");
  if (directories == NULL) goto read_icon_theme_out;
  ...
  icon_theme_info->hidden =
      mate_desktop_item_get_boolean(icon_theme_ditem, "Icon Theme/Hidden");
...
static void add_default_cursor_theme(void) {
...
static GdkPixbuf *gdk_pixbuf_from_xcursor_image(XcursorImage *cursor) {
  GdkPixbuf *pixbuf;
#define BUF_SIZE sizeof(guint32) * cursor->width * cursor->height
  ...
  pixbuf = gdk_pixbuf_new_from_data(
      (const guchar *)buf, GDK_COLORSPACE_RGB, TRUE, 8, cursor->width,
      cursor->height, cursor->width * 4, (GdkPixbufDestroyNotify)g_free, NULL);

  if (!pixbuf) g_free(buf);

  return pixbuf;
}

static MateThemeCursorInfo *read_cursor_theme(GFile *cursor_theme_uri) {
  ...
  if (get_file_type(cursors_uri) == G_FILE_TYPE_DIRECTORY) {
    ...
    if (sizes->len == 0) {
      ...
      cursor_theme_ditem =
          mate_desktop_item_new_from_file(cursor_theme_file, 0, NULL);
      ...
      if (cursor_theme_ditem != NULL) {
        ...
        readable_name =
            mate_desktop_item_get_string(cursor_theme_ditem, "Icon Theme/Name");
        if (readable_name)
          cursor_theme_info->readable_name = g_strdup(readable_name);
        else
          cursor_theme_info->readable_name = g_strdup(name);

        cursor_theme_info->hidden = mate_desktop_item_get_boolean(
            cursor_theme_ditem, "Icon Theme/Hidden");
...
static void handle_change_signal(gpointer data, MateThemeChangeType change_type,
  ...
  for (list = callbacks; list; list = list->next) {
    ...
    (*callback_data->func)(theme, change_type, element_type,
                           callback_data->data);
  }
  ...
  if (type == MATE_THEME_TYPE_REGULAR) {
    g_print("theme \"%s\" has a theme of type %s (priority %d) has been %s\n",
            theme->name, element_str, theme->priority, type_str);
  } else if (type_str != NULL) {
    g_print("%s theme \"%s\" (priority %d) has been %s\n", type_str,
            theme->name, theme->priority, type_str);
  }
#endif
}

/* index_uri should point to the gtkrc file that was modified */
static void update_theme_index(GFile *index_uri, MateThemeElement key_element,
                               gint priority) {
  ...
  if (theme_info == NULL) {
    if (theme_exists) {
      ...
      g_hash_table_insert(theme_hash_by_uri, g_strdup(common_theme_dir),
                          theme_info);
    ...
    if (!theme_info->has_marco && !theme_info->has_keybinding &&
        !theme_info->has_gtk) {
...
static void update_gtk2_index(GFile *gtk2_index_uri, gint priority) {
  update_theme_index(gtk2_index_uri, MATE_THEME_GTK_2, priority);
}

static void update_keybinding_index(GFile *keybinding_index_uri,
                                    gint priority) {
  update_theme_index(keybinding_index_uri, MATE_THEME_GTK_2_KEYBINDING,
                     priority);
}

static void update_marco_index(GFile *marco_index_uri, gint priority) {
  update_theme_index(marco_index_uri, MATE_THEME_MARCO, priority);
}

static void update_common_theme_dir_index(GFile *theme_index_uri,
                                          MateThemeType type, gint priority) {
  gboolean theme_exists;
  ...
  if (type != MATE_THEME_TYPE_CURSOR) {
    ...
    if (get_file_type(theme_index_uri) == G_FILE_TYPE_REGULAR) {
      ...
      if (type == MATE_THEME_TYPE_ICON)
        ...
        theme_info =
            (MateThemeCommonInfo *)mate_theme_read_meta_theme(theme_index_uri);
    } else {
      theme_info = NULL;
    }

  }
  /* cursor themes don't necessarily have an index file, so try those in any
     case */
  else {
  ...
  old_theme_info =
      (MateThemeCommonInfo *)g_hash_table_lookup(hash_by_uri, common_theme_dir);
  ...
  if (old_theme_info == NULL) {
    ...
    if (theme_exists) {
      if (theme_compare(theme_info, old_theme_info) != 0) {
        ...
        g_hash_table_insert(hash_by_uri, g_strdup(common_theme_dir),
                            theme_info);
        add_theme_to_hash_by_name(hash_by_name, theme_info);
...
static void update_meta_theme_index(GFile *meta_theme_index_uri,
                                    gint priority) {
  update_common_theme_dir_index(meta_theme_index_uri, MATE_THEME_TYPE_METATHEME,
                                priority);
}

static void update_icon_theme_index(GFile *icon_theme_index_uri,
                                    gint priority) {
  update_common_theme_dir_index(icon_theme_index_uri, MATE_THEME_TYPE_ICON,
                                priority);
}

static void update_cursor_theme_index(GFile *cursor_theme_index_uri,
                                      gint priority) {
  update_common_theme_dir_index(cursor_theme_index_uri, MATE_THEME_TYPE_CURSOR,
                                priority);
}

static void gtk2_dir_changed(GFileMonitor *monitor, GFile *file,
...
static void keybinding_dir_changed(GFileMonitor *monitor, GFile *file,
                                   GFile *other_file,
...
static void marco_dir_changed(GFileMonitor *monitor, GFile *file,
                              GFile *other_file, GFileMonitorEvent event_type,
                              CommonThemeDirMonitorData *monitor_data) {
  gchar *affected_file;
...
static void common_theme_dir_changed(GFileMonitor *monitor, GFile *file,
                                     GFile *other_file,
                                     GFileMonitorEvent event_type,
                                     CommonThemeDirMonitorData *monitor_data) {
  gchar *affected_file;
...
static void common_icon_theme_dir_changed(
    GFileMonitor *monitor, GFile *file, GFile *other_file,
    GFileMonitorEvent event_type, CommonIconThemeDirMonitorData *monitor_data) {
  gchar *affected_file;
...
static gboolean add_common_theme_dir_monitor(
    GFile *theme_dir_uri, CommonThemeDirMonitorData *monitor_data,
    GError **error) {
  GFile *uri, *subdir;
  ...
  g_signal_connect(monitor, "changed", (GCallback)common_theme_dir_changed,
                   monitor_data);
  ...
  if (monitor != NULL) {
    g_signal_connect(monitor, "changed", (GCallback)gtk2_dir_changed,
                     monitor_data);
  }
  ...
  monitor = g_file_monitor_directory(subdir, G_FILE_MONITOR_NONE, NULL, NULL);
  if (monitor != NULL) {
    g_signal_connect(monitor, "changed", (GCallback)keybinding_dir_changed,
                     monitor_data);
  }
  ...
  monitor = g_file_monitor_directory(subdir, G_FILE_MONITOR_NONE, NULL, NULL);
  if (monitor != NULL) {
    g_signal_connect(monitor, "changed", (GCallback)marco_dir_changed,
                     monitor_data);
  }
...
static gboolean add_common_icon_theme_dir_monitor(
    GFile *theme_dir_uri, CommonIconThemeDirMonitorData *monitor_data,
    GError **error) {
  GFile *index_uri;
  GFileMonitor *monitor;
  ...
  g_signal_connect(monitor, "changed", (GCallback)common_icon_theme_dir_changed,
                   monitor_data);
...
static void remove_common_theme_dir_monitor(
    CommonThemeDirMonitorData *monitor_data) {
...
static void remove_common_icon_theme_dir_monitor(
    CommonIconThemeDirMonitorData *monitor_data) {
  g_file_monitor_cancel(monitor_data->common_icon_theme_dir_handle);
}

static void top_theme_dir_changed(GFileMonitor *monitor, GFile *file,
                                  GFile *other_file,
                                  GFileMonitorEvent event_type,
                                  CallbackTuple *tuple) {
  GHashTable *handle_hash;
...
static void top_icon_theme_dir_changed(GFileMonitor *monitor, GFile *file,
                                       GFile *other_file,
                                       GFileMonitorEvent event_type,
                                       CallbackTuple *tuple) {
  GHashTable *handle_hash;
 ...
 * lib.
 */
static gboolean real_add_top_theme_dir_monitor(GFile *uri, gint priority,
                                               gboolean icon_theme,
                                               GError **error) {
  GFileInfo *file_info;
  GFileMonitor *monitor;
  ...
  /* Check the URI */
  if (get_file_type(uri) != G_FILE_TYPE_DIRECTORY) return FALSE;
  ...
  tuple->handle_hash = g_hash_table_new_full(
      g_str_hash, g_str_equal, (GDestroyNotify)g_free, (GDestroyNotify)g_free);
  tuple->priority = priority;

  /* Monitor the top directory */
  if ((monitor = g_file_monitor_directory(uri, G_FILE_MONITOR_NONE, NULL,
                                          NULL)) != NULL) {
    g_signal_connect(monitor, "changed",
                     (GCallback)(icon_theme ? top_icon_theme_dir_changed
                                            : top_theme_dir_changed),
                     tuple);
  } else {
  ...
  enumerator = g_file_enumerate_children(
      uri, G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_STANDARD_NAME,
      G_FILE_QUERY_INFO_NONE, NULL, NULL);
...
static gboolean add_top_theme_dir_monitor(GFile *uri, gint priority,
                                          GError **error) {
  return real_add_top_theme_dir_monitor(uri, priority, FALSE, error);
}

static gboolean add_top_icon_theme_dir_monitor(GFile *uri, gint priority,
                                               GError **error) {
...
MateThemeInfo *mate_theme_info_find(const gchar *theme_name) {
  return (MateThemeInfo *)get_theme_from_hash_by_name(theme_hash_by_name,
                                                      theme_name, -1);
}
...
static void mate_theme_info_find_by_type_helper(
    gpointer key, GList *list, struct MateThemeInfoHashData *hash_data) {
...
GList *mate_theme_info_find_by_type(guint elements) {
  ...
  g_hash_table_foreach(theme_hash_by_name,
                       (GHFunc)mate_theme_info_find_by_type_helper, &data);

  return data.list;
}

static void mate_theme_info_find_all_helper(const gchar *key, GList *list,
                                            GList **themes) {
...
MateThemeIconInfo *mate_theme_icon_info_find(const gchar *icon_theme_name) {
  ...
  return (MateThemeIconInfo *)get_theme_from_hash_by_name(
      icon_theme_hash_by_name, icon_theme_name, -1);
}
...
GList *mate_theme_icon_info_find_all(void) {
  ...
  g_hash_table_foreach(icon_theme_hash_by_name,
                       (GHFunc)mate_theme_info_find_all_helper, &list);

  return list;
}

gint mate_theme_icon_info_compare(MateThemeIconInfo *a, MateThemeIconInfo *b) {
  gint cmp;
...
void mate_theme_cursor_info_free(MateThemeCursorInfo *cursor_theme_info) {
...
MateThemeCursorInfo *mate_theme_cursor_info_find(
    const gchar *cursor_theme_name) {
  g_return_val_if_fail(cursor_theme_name != NULL, NULL);

  return (MateThemeCursorInfo *)get_theme_from_hash_by_name(
      cursor_theme_hash_by_name, cursor_theme_name, -1);
}
...
gboolean mate_theme_meta_info_validate(const MateThemeMetaInfo *info,
  ...
  if (!theme || !theme->has_gtk) {
    g_set_error(error, MATE_THEME_ERROR,
                MATE_THEME_ERROR_GTK_THEME_NOT_AVAILABLE,
                _("This theme will not look as intended because the required "
                  "GTK+ theme '%s' is not installed."),
                info->gtk_theme_name);
    return FALSE;
  }
  ...
  if (!theme || !theme->has_marco) {
    g_set_error(error, MATE_THEME_ERROR,
                ...
                _("This theme will not look as intended because the required "
                  "window manager theme '%s' is not installed."),
...
MateThemeMetaInfo *mate_theme_meta_info_find(const char *meta_theme_name) {
  ...
  return (MateThemeMetaInfo *)get_theme_from_hash_by_name(
      meta_theme_hash_by_name, meta_theme_name, -1);
}
...
gint mate_theme_meta_info_compare(MateThemeMetaInfo *a, MateThemeMetaInfo *b) {
  gint cmp;
...
gboolean mate_theme_color_scheme_parse(const gchar *scheme, GdkRGBA *colors) {
  gchar **color_scheme_strings, **color_scheme_pair, *current_string;
  gint i;

  if (!scheme || !strcmp(scheme, "")) return FALSE;
  ...
  /* loop through the name:color pairs, and save the color if we recognise the
   * name */
  i = 0;
...
gboolean mate_theme_color_scheme_equal(const gchar *s1, const gchar *s2) {
  ...
  for (i = 0; i < NUM_SYMBOLIC_COLORS; ++i) {
    if (!gdk_rgba_equal(&c1[i], &c2[i])) return FALSE;
  }

  return TRUE;
}

void mate_theme_init() {
  ...
  meta_theme_hash_by_uri =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  meta_theme_hash_by_name =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  icon_theme_hash_by_uri =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  icon_theme_hash_by_name =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  cursor_theme_hash_by_uri =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  cursor_theme_hash_by_name =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  theme_hash_by_uri =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  theme_hash_by_name =
  ...
  /* Add all the toplevel theme dirs following the XDG Base Directory
   * Specification */
  ...
  gtk_icon_theme_get_search_path(gtk_icon_theme_get_default(), &search_path,
                                 &n);
  for (i = 0; i < n; ++i) {
  ...
  if (!mate_theme_cursor_info_find("default")) add_default_cursor_theme();
