...
static void app_resizer_size_allocate(GtkWidget *resizer,
                                      GtkAllocation *allocation);
static gboolean app_resizer_paint_window(GtkWidget *widget, cairo_t *cr,
                                         AppShellData *app_data);

G_DEFINE_TYPE(AppResizer, app_resizer, GTK_TYPE_LAYOUT);

static void app_resizer_class_init(AppResizerClass *klass) {
...
static void app_resizer_init(AppResizer *window) {
...
static void resize_table(AppResizer *widget, GtkGrid *table, gint columns) {
  remove_container_entries(GTK_CONTAINER(table));
  widget->column = columns;
}

static void relayout_table(AppResizer *widget, GtkGrid *table,
                           GList *element_list) {
  gint row = 0, col = 0;
  do {
...
void app_resizer_layout_table_default(AppResizer *widget, GtkGrid *table,
                                      GList *element_list) {
  resize_table(widget, table, widget->cur_num_cols);
  relayout_table(widget, table, element_list);
}

static void relayout_tables(AppResizer *widget, gint num_cols) {
  GtkGrid *table;
  ...
  for (table_list = widget->cached_tables_list; table_list != NULL;
    ...
    launcher_list = g_list_reverse(
        launcher_list); /* Fixme - ugly hack because table stores prepend */
...
static gint calculate_num_cols(AppResizer *resizer, gint avail_width) {
  if (resizer->table_elements_homogeneous) {
    ...
    num_cols = (avail_width + resizer->cached_table_spacing) /
               (resizer->cached_element_width + resizer->cached_table_spacing);
    return num_cols;
  } else
    g_assert_not_reached(); /* Fixme - implement... */
}

static gint relayout_tables_if_needed(AppResizer *widget, gint avail_width,
                                      gint current_num_cols) {
...
static void app_resizer_size_allocate(GtkWidget *widget,
                                      GtkAllocation *allocation) {
  ...
  if (first_time) {
    /* we are letting the first show be the "natural" size of the child widget
     * so do nothing. */
    if (GTK_WIDGET_CLASS(app_resizer_parent_class)->size_allocate)
      (*GTK_WIDGET_CLASS(app_resizer_parent_class)->size_allocate)(widget,
                                                                   allocation);
  ...
  if (!resizer->cached_tables_list) /* if everthing is currently filtered out -
                                       just return */
  {
    ...
    /* We want the message to center itself and only scroll if it's bigger than
     * the available real size. */
  ...
  gtk_widget_get_preferred_size(GTK_WIDGET(resizer->cached_tables_list->data),
                                &other_requisiton, NULL);

  new_num_cols = relayout_tables_if_needed(
      APP_RESIZER(resizer), allocation->width, resizer->cur_num_cols);
...
GtkWidget *app_resizer_new(GtkBox *child, gint initial_num_columns,
  ...
  g_signal_connect(widget, "draw", G_CALLBACK(app_resizer_paint_window),
                   app_data);
...
static gboolean app_resizer_paint_window(GtkWidget *widget, cairo_t *cr,
  ...
  cairo_rectangle(cr, widget_allocation.x, widget_allocation.y,
  ...
  if (app_data->selected_group) {
    ...
    gdk_cairo_set_source_color(cr,
                               gtk_widget_get_style(selected_widget)->light);
    cairo_set_line_width(cr, 1);

    cairo_rectangle(
        cr, selected_widget_allocation.x, selected_widget_allocation.y,
        selected_widget_allocation.width, selected_widget_allocation.height);
    cairo_stroke_preserve(cr);
    cairo_fill(cr);
  }
