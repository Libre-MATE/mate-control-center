 ...
 *
 * mate-display-properties-install-systemwide - Install a RANDR profile for the
 * whole system
 *
...
static void usage(const char *program_name) {
  g_print(
      _("Usage: %s SOURCE_FILE DEST_NAME\n"
        "\n"
        ...
        "\n"
        "DEST_NAME - relative name for the installed file.  This will get put "
        "in\n"
        "            the systemwide directory for RANDR configurations,\n"
        "            so the result will typically be %s/DEST_NAME\n"),
      program_name, SYSTEM_RANDR_PATH);
}

static gboolean is_basename(const char *filename) {
  if (*filename == '\0') return FALSE; /* no empty strings, please */

  for (; *filename; filename++)
    if (G_IS_DIR_SEPARATOR(*filename)) return FALSE;

  return TRUE;
}

static gboolean copy_file(int source_fd, int dest_fd) {
  char buf[1024];
  ssize_t num_read;
  ssize_t num_written;
...
int main(int argc, char **argv) {
  ...
  if (!copy_file(source_fd, dest_fd)) {
    /* If something went wrong, remove the destination file to avoid leaving
     * trash around */
    unlink(template);
    return EXIT_FAILURE;
  }
