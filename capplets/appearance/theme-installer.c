...
enum { TARGZ, TARBZ, TARXZ, DIRECTORY };

static gboolean cleanup_tmp_dir(GIOSchedulerJob *job, GCancellable *cancellable,
                                const gchar *tmp_dir) {
  GFile *directory;
...
static int file_theme_type(const gchar *dir) {
  gchar *filename = NULL;
  gboolean exists;
...
static void missing_utility_message_dialog(GtkWindow *parent,
                                           const gchar *utility) {
  GtkWidget *dialog =
      gtk_message_dialog_new(parent, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
                             GTK_BUTTONS_OK, _("Cannot install theme"));
 ...
 */
static gboolean process_local_theme_tgz_tbz(GtkWindow *parent,
                                            const gchar *util,
                                            const gchar *tmp_dir,
                                            const gchar *archive) {
  gboolean rc;
  ...
  rc = (g_spawn_command_line_sync(command, NULL, NULL, &status, NULL) &&
        status == 0);
  g_free(command);
...
static gboolean process_local_theme_archive(GtkWindow *parent, gint filetype,
                                            const gchar *tmp_dir,
                                            const gchar *archive) {
  if (filetype == TARGZ)
...
static void invalid_theme_dialog(GtkWindow *parent, const gchar *filename,
  ...
  const gchar *engine =
      _("\"%s\" does not appear to be a valid theme. It may be a theme engine "
        "which you need to compile.");

  dialog = gtk_message_dialog_new(parent, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
                                  GTK_BUTTONS_OK, "%s", primary);
  if (maybe_theme_engine)
    gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(dialog), engine,
                                             filename);
  else
...
static gboolean mate_theme_install_real(GtkWindow *parent, const gchar *tmp_dir,
  ...
  switch (theme_type) {
    ...
    case THEME_ICON_CURSOR:
      target_dir = g_build_path(G_DIR_SEPARATOR_S, g_get_home_dir(), ".icons",
                                theme_name, NULL);
      break;
    case THEME_MATE:
    case THEME_MARCO:
    case THEME_GTK:
      target_dir = g_build_path(G_DIR_SEPARATOR_S, g_get_home_dir(), ".themes",
                                theme_name, NULL);
      break;
    case THEME_ENGINE:
  ...
  if (theme_type == THEME_MATE) {
    ...
    path = g_build_path(G_DIR_SEPARATOR_S, tmp_dir, "icons", NULL);
    if (g_file_test(path, G_FILE_TEST_IS_DIR) &&
      ...
      new_path = g_build_path(G_DIR_SEPARATOR_S, g_get_home_dir(), ".icons",
                              theme_name, NULL);
      new_file = g_file_new_for_path(new_path);

      if (!g_file_move(src_file, new_file, G_FILE_COPY_NONE, NULL, NULL, NULL,
                       &error)) {
        g_warning("Error while moving from `%s' to `%s': %s", path, new_path,
                  error->message);
        g_error_free(error);
        error = NULL;
      }
  ...
  if (g_file_test(target_dir, G_FILE_TEST_EXISTS)) {
    ...
    dialog = gtk_message_dialog_new(parent, GTK_DIALOG_MODAL, GTK_MESSAGE_INFO,
                                    GTK_BUTTONS_NONE, "%s", str);
    g_free(str);

    gtk_message_dialog_format_secondary_text(
        GTK_MESSAGE_DIALOG(dialog), _("Do you want to install it again?"));

    gtk_dialog_add_button(GTK_DIALOG(dialog), _("Cancel"), GTK_RESPONSE_CLOSE);

    apply_button = gtk_button_new_with_label(_("Install"));
    gtk_button_set_image(
        GTK_BUTTON(apply_button),
        gtk_image_new_from_icon_name("gtk-apply", GTK_ICON_SIZE_BUTTON));
    gtk_dialog_add_action_widget(GTK_DIALOG(dialog), apply_button,
                                 GTK_RESPONSE_APPLY);
  ...
  if (!g_file_move(theme_source_dir, theme_dest_dir, G_FILE_COPY_OVERWRITE,
                   NULL, NULL, NULL, &error)) {
    gchar *str;

    str =
        g_strdup_printf(_("Installation for theme \"%s\" failed."), theme_name);
    dialog = gtk_message_dialog_new(parent, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR,
    ...
    if (theme_type == THEME_ICON || theme_type == THEME_ICON_CURSOR) {
      ...
      /* update icon cache - shouldn't really matter if this fails */
      update_icon_cache =
          g_strdup_printf("gtk-update-icon-cache %s", target_dir);
      g_spawn_command_line_async(update_icon_cache, NULL);
    ...
    if (ask_user) {
      ...
      if (theme_type == THEME_GTK || theme_type == THEME_MARCO ||
          theme_type == THEME_ICON || theme_type == THEME_CURSOR ||
          theme_type == THEME_ICON_CURSOR) {
        /* TODO: currently cannot apply "mate themes" */
        gchar *str;

        str = g_strdup_printf(_("The theme \"%s\" has been installed."),
                              theme_name);
        dialog = gtk_message_dialog_new_with_markup(
            parent, GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_NONE,
        ...
        gtk_dialog_add_button(GTK_DIALOG(dialog), _("Keep Current Theme"),
                              GTK_RESPONSE_CLOSE);
        ...
        dialog = gtk_message_dialog_new(
            ...
            _("MATE Theme %s correctly installed"), theme_name);
        gtk_dialog_run(GTK_DIALOG(dialog));
      }
      gtk_widget_destroy(dialog);
    }
...
static void process_local_theme(GtkWindow *parent, const char *path) {
  GtkWidget *dialog;
  gint filetype;
  ...
  if (filetype == DIRECTORY) {
    ...
    mate_theme_install_real(parent, path, name, TRUE);
    g_free(name);
  } else {
    ...
    tmp_dir = g_strdup_printf("%s/.themes/.theme-%u", g_get_home_dir(),
                              g_random_int());
    ...
    if (!process_local_theme_archive(parent, filetype, tmp_dir, path) ||
      ...
      g_io_scheduler_push_job((GIOSchedulerJobFunc)cleanup_tmp_dir,
                              g_strdup(tmp_dir), g_free, G_PRIORITY_DEFAULT,
                              NULL);
      g_free(tmp_dir);
      return;
    }
    ...
    for (name = g_dir_read_name(dir); name && n_themes <= 1;
      ...
      if (g_file_test(theme_dir, G_FILE_TEST_IS_DIR)) ++n_themes;
    ...
    ok = TRUE;
    for (name = g_dir_read_name(dir); name && ok; name = g_dir_read_name(dir)) {
      gchar *theme_dir;

      theme_dir = g_build_filename(tmp_dir, name, NULL);

      if (g_file_test(theme_dir, G_FILE_TEST_IS_DIR))
        ok = mate_theme_install_real(parent, theme_dir, name, n_themes == 1);
    ...
    g_io_scheduler_push_job((GIOSchedulerJobFunc)cleanup_tmp_dir, tmp_dir,
                            g_free, G_PRIORITY_DEFAULT, NULL);
  }
}
...
static void transfer_done_cb(GtkWidget *dialog, TransferData *tdata) {
...
void mate_theme_install(GFile *file, GtkWindow *parent) {
  ...
  if (g_str_has_suffix(base, ".tar.gz") || g_str_has_suffix(base, ".tgz") ||
  ...
  g_signal_connect(dialog, "cancel", (GCallback)transfer_cancel_cb, path);
  g_signal_connect(dialog, "done", (GCallback)transfer_done_cb, tdata);

  target = g_list_append(NULL, g_file_new_for_path(path));
  file_transfer_dialog_copy_async(FILE_TRANSFER_DIALOG(dialog), src, target,
...
void mate_theme_installer_run(GtkWindow *parent) {
  ...
  if (running_theme_install) return;

  running_theme_install = TRUE;

  dialog = gtk_file_chooser_dialog_new(
      _("Select Theme"), parent, GTK_FILE_CHOOSER_ACTION_OPEN, "gtk-cancel",
      GTK_RESPONSE_REJECT, "gtk-open", GTK_RESPONSE_ACCEPT, NULL);
