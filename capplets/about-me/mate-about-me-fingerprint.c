...
enum { STATE_NONE, STATE_CLAIMED, STATE_ENROLLING };
...
static void create_manager(void) {
  ...
  manager = g_dbus_proxy_new_for_bus_sync(
      ...
      "net.reactivated.Fprint.Manager", NULL, &error);
  if (manager == NULL) {
...
static GDBusProxy *get_first_device(void) {
  GDBusProxy *device = NULL;
  GVariant *ret;
  char *device_str;
  GError *error = NULL;

  ret = g_dbus_proxy_call_sync(manager, "GetDefaultDevice", g_variant_new("()"),
  ...
  device = g_dbus_proxy_new_for_bus_sync(
      G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, NULL,
      "net.reactivated.Fprint", device_str, "net.reactivated.Fprint.Device",
      NULL, &error);
  if (device == NULL) {
    g_warning("Unable to contact Fprint Manager daemon: %s\n", error->message);
    g_error_free(error);
...
static const char *get_reason_for_error(const char *dbus_error) {
...
static GtkWidget *get_error_dialog(const char *title, const char *dbus_error,
                                   GtkWindow *parent) {
  ...
  error_dialog = gtk_message_dialog_new(
      parent, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s", title);
  reason = get_reason_for_error(dbus_error);
  gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(error_dialog),
                                           "%s",
                                           reason ? _(reason) : _(dbus_error));
  ...
  gtk_window_set_position(GTK_WINDOW(error_dialog),
...
void set_fingerprint_label(GtkWidget *enable, GtkWidget *disable) {
  ...
  ret = g_dbus_proxy_call_sync(device, "ListEnrolledFingers",
                               g_variant_new("(s)", ""), G_DBUS_CALL_FLAGS_NONE,
                               -1, NULL, &error);
  if (ret == NULL) {
    gchar *error_name = g_dbus_error_get_remote_error(error);
    if (!g_str_equal(error_name,
...
static void delete_fingerprints(void) {
  GVariant *ret;
  GDBusProxy *device;
  GError *error = NULL;
  ...
  ret = g_dbus_proxy_call_sync(device, "DeleteEnrolledFingers",
                               g_variant_new("(s)", ""), G_DBUS_CALL_FLAGS_NONE,
                               -1, NULL, &error);
  if (ret == NULL) {
...
static void delete_fingerprints_question(GtkBuilder *dialog, GtkWidget *enable,
                                         GtkWidget *disable) {
  GtkWidget *question;
  GtkWidget *button;
  ...
  gtk_dialog_add_button(GTK_DIALOG(question), "gtk-cancel",
                        GTK_RESPONSE_CANCEL);
  ...
  gtk_message_dialog_format_secondary_text(
      GTK_MESSAGE_DIALOG(question),
      _("Do you want to delete your registered fingerprints so fingerprint "
        "login is disabled?"));
...
static void enroll_data_destroy(EnrollData *data) {
  GVariant *ret;
  switch (data->state) {
    case STATE_ENROLLING:
      ret = g_dbus_proxy_call_sync(data->device, "EnrollStop",
      ...
      /* fall-through */
    case STATE_CLAIMED:
      ret = g_dbus_proxy_call_sync(data->device, "Release", g_variant_new("()"),
...
static const char *selected_finger(GtkBuilder *dialog) {
  ...
  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(gtk_builder_get_object(dialog, "radiobutton1")))) {
    gtk_widget_set_sensitive(finger_combobox, FALSE);
    return "right-index-finger";
  }

  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(gtk_builder_get_object(dialog, "radiobutton2")))) {
    gtk_widget_set_sensitive(finger_combobox, FALSE);
...
static void finger_radio_button_toggled(GtkToggleButton *button,
                                        EnrollData *data) {
  char *msg;

  data->finger = selected_finger(data->dialog);

  msg = g_strdup_printf(TR(finger_str_to_msg(data->finger, data->is_swipe)),
                        data->name);
  gtk_label_set_text(
      GTK_LABEL(gtk_builder_get_object(data->dialog, "enroll-label")), msg);
  g_free(msg);
}

static void finger_combobox_changed(GtkComboBox *combobox, EnrollData *data) {
...
static void assistant_cancelled(GtkAssistant *ass, EnrollData *data) {
...
static void on_signal(GDBusProxy *proxy G_GNUC_UNUSED,
  ...
  if (g_str_equal(result, "enroll-completed") ||
      g_str_equal(result, "enroll-stage-passed")) {
    char *name, *path;
    ...
    gtk_image_set_from_file(GTK_IMAGE(gtk_builder_get_object(dialog, name)),
                            path);
    g_free(name);
    g_free(path);
  }
  if (g_str_equal(result, "enroll-completed")) {
    gtk_label_set_text(
        GTK_LABEL(gtk_builder_get_object(dialog, "status-label")), _("Done!"));
    gtk_assistant_set_page_complete(GTK_ASSISTANT(data->ass), WID("page2"),
                                    TRUE);
  }
  ...
  if (done) {
    GVariant *ret;
    ret =
        g_dbus_proxy_call_sync(data->device, "EnrollStop", g_variant_new("()"),
                               G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL);
    if (ret != NULL) {
      g_variant_unref(ret);
    }
    ...
    if (g_str_equal(result, "enroll-completed") == FALSE) {
      ...
      ret = g_dbus_proxy_call_sync(data->device, "EnrollStart",
  ...
  msg = g_strdup_printf(TR(enroll_result_str_to_msg(result, data->is_swipe)),
                        data->name);
  gtk_label_set_text(GTK_LABEL(gtk_builder_get_object(dialog, "status-label")),
                     msg);
  g_free(msg);
  g_free(result);
}

static void assistant_prepare(GtkAssistant *ass, GtkWidget *page,
                              EnrollData *data) {
  const char *name;
  ...
  if (g_str_equal(name, "enroll")) {
    ...
    ret =
        g_dbus_proxy_call_sync(data->device, "Claim", g_variant_new("(s)", ""),
                               G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    if (error != NULL) {
      /* translators:
    ...
    ret = g_dbus_proxy_call_sync(
        p, "Get",
        g_variant_new("(ss)", "net.reactivated.Fprint.Device",
                      "num-enroll-stages"),
        G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    g_object_unref(p);
    if (ret == NULL) {
    out:
       ...
       * "Could you not access "Digital Persona U.are.U 4000/4000B" device */
      msg = g_strdup_printf(_("Could not access '%s' device"), data->name);
      d = get_error_dialog(msg, "net.reactivated.Fprint.Error.Internal",
                           GTK_WINDOW(data->ass));
      gtk_dialog_run(GTK_DIALOG(d));
      gtk_widget_destroy(d);
      g_free(msg);
    ...
    for (i = 1; i <= data->num_enroll_stages; i++) {
      ...
      gtk_image_set_from_file(
          GTK_IMAGE(gtk_builder_get_object(dialog, image_name)), path);
      g_free(image_name);
    }
    g_free(path);

    g_signal_connect(data->device, "g-signal", G_CALLBACK(on_signal), data);
    ret = g_dbus_proxy_call_sync(data->device, "EnrollStart",
                                 g_variant_new("(s)", data->finger),
                                 G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    if (ret == NULL) {
    ...
    if (data->state == STATE_ENROLLING) {
      value = g_dbus_proxy_call_sync(data->device, "EnrollStop",
                                     g_variant_new("()"),
                                     G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL);
      if (value != NULL) {
        g_variant_unref(value);
    ...
    if (data->state == STATE_CLAIMED) {
      value =
          g_dbus_proxy_call_sync(data->device, "Release", g_variant_new("()"),
                                 G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL);
      if (value != NULL) {
        g_variant_unref(value);
      }
...
static void enroll_fingerprints(GtkWindow *parent, GtkWidget *enable,
                                GtkWidget *disable) {
  GDBusProxy *device;
  GtkBuilder *dialog;
  EnrollData *data;
  ...
  if (manager == NULL) {
    create_manager();
    if (manager != NULL) device = get_first_device();
  } else {
    device = get_first_device();
  }
  ...
  if (manager == NULL || device == NULL) {
    ...
    d = get_error_dialog(
        _("Could not access any fingerprint readers"),
        _("Please contact your system administrator for help."), parent);
    gtk_dialog_run(GTK_DIALOG(d));
    gtk_widget_destroy(d);
    return;
  }
  ...
  GDBusProxy *p;
  p = g_dbus_proxy_new_sync(
      g_dbus_proxy_get_connection(device), G_DBUS_PROXY_FLAGS_NONE, NULL,
      g_dbus_proxy_get_name(device), g_dbus_proxy_get_object_path(device),
      "org.freedesktop.DBus.Properties", NULL, &error);
  if (p == NULL) {
    g_warning("Unable to contact Fprint Device daemon: %s\n", error->message);
    g_error_free(error);
  ...
  ret = g_dbus_proxy_call_sync(
      p, "GetAll", g_variant_new("(s)", "net.reactivated.Fprint.Device"),
      G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL);
  if (ret != NULL) {
    GVariantIter *iter;
    const gchar *key;
    ...
    while (g_variant_iter_loop(iter, "{sv}", &key, &value)) {
      if (g_str_equal(key, "name") &&
          g_variant_is_of_type(value, G_VARIANT_TYPE("(s)"))) {
        data->name = g_strdup(g_variant_get_string(value, NULL));
      } else if (g_str_equal(key, "scan-type") &&
                 g_variant_is_of_type(value, G_VARIANT_TYPE("(s)"))) {
  ...
  if (gtk_builder_add_from_resource(
          dialog, "/org/mate/mcc/am/mate-about-me-fingerprint.ui", &error) ==
      0) {
  ...
  g_signal_connect(ass, "cancel", G_CALLBACK(assistant_cancelled), data);
  g_signal_connect(ass, "close", G_CALLBACK(assistant_cancelled), data);
  g_signal_connect(ass, "prepare", G_CALLBACK(assistant_prepare), data);

  /* Page 1 */
  gtk_combo_box_set_active(
      GTK_COMBO_BOX(gtk_builder_get_object(dialog, "finger_combobox")), 0);
   ...
   * using the 'Digital Persona U.are.U 4000/4000B' device." */
  msg = g_strdup_printf(_("To enable fingerprint login, you need to save one "
                          "of your fingerprints, using the '%s' device."),
                        data->name);
  gtk_label_set_text(GTK_LABEL(gtk_builder_get_object(dialog, "intro-label")),
                     msg);
  g_free(msg);
  ...
  if (data->is_swipe != FALSE)
    gtk_assistant_set_page_title(GTK_ASSISTANT(ass), WID("page2"),
                                 _("Swipe finger on reader"));
  else
    gtk_assistant_set_page_title(GTK_ASSISTANT(ass), WID("page2"),
                                 _("Place finger on reader"));
...
void fingerprint_button_clicked(GtkBuilder *dialog, GtkWidget *enable,
  ...
  if (is_disable != FALSE) {
    ...
    enroll_fingerprints(
        GTK_WINDOW(gtk_builder_get_object(dialog, "about-me-dialog")), enable,
        disable);
  }
}
