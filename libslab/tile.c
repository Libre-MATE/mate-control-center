...
static void tile_action_triggered_event_marshal(GClosure *, GValue *, guint,
                                                const GValue *, gpointer,
                                                gpointer);

typedef void (*marshal_func_VOID__POINTER_POINTER)(gpointer, gpointer, gpointer,
                                                   gpointer);

enum { TILE_ACTIVATED_SIGNAL, TILE_ACTION_TRIGGERED_SIGNAL, LAST_SIGNAL };

static guint tile_signals[LAST_SIGNAL] = {0};

enum { PROP_0, PROP_TILE_URI, PROP_TILE_CONTEXT_MENU, PROP_TILE_ACTIONS };

static void tile_class_init(TileClass *this_class) {
  ...
  g_object_class_install_property(
      g_obj_class, PROP_TILE_URI,
      g_param_spec_string("tile-uri", "tile-uri", "the uri of the tile", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      g_obj_class, PROP_TILE_CONTEXT_MENU,
      g_param_spec_object("context-menu", "context-menu",
                          "the context menu for the tile", GTK_TYPE_MENU,
                          G_PARAM_READWRITE));

  tile_signals[TILE_ACTIVATED_SIGNAL] = g_signal_new(
      "tile-activated", G_TYPE_FROM_CLASS(this_class),
      G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(TileClass, tile_activated), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  tile_signals[TILE_ACTION_TRIGGERED_SIGNAL] =
      g_signal_new("tile-action-triggered", G_TYPE_FROM_CLASS(this_class),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(TileClass, tile_action_triggered), NULL,
                   NULL, tile_action_triggered_event_marshal, G_TYPE_NONE, 2,
                   G_TYPE_POINTER, G_TYPE_POINTER);
}

static GObject *tile_constructor(GType type, guint n_param,
                                 GObjectConstructParam *param) {
  GObject *g_obj;
  TilePrivate *priv;

  g_obj =
      (*G_OBJECT_CLASS(tile_parent_class)->constructor)(type, n_param, param);
...
static void tile_finalize(GObject *g_object) {
...
static void tile_dispose(GObject *g_object) {
  Tile *tile = TILE(g_object);
...
static void tile_get_property(GObject *g_obj, guint prop_id, GValue *value,
                              GParamSpec *param_spec) {
  if (!IS_TILE(g_obj)) return;
...
static void tile_set_property(GObject *g_obj, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_TILE_CONTEXT_MENU:
      ...
      if (menu == tile->context_menu) break;

      if (tile->context_menu) gtk_menu_detach(tile->context_menu);
...
static void tile_setup(Tile *tile) {
...
static gboolean tile_focus_in(GtkWidget *widget, GdkEventFocus *event) {
  gtk_widget_set_state_flags(widget, TILE_STATE_FOCUSED, TRUE);

  return FALSE;
}

static gboolean tile_focus_out(GtkWidget *widget, GdkEventFocus *event) {
...
static gboolean tile_draw(GtkWidget *widget, cairo_t *cr) {
...
static gboolean tile_button_release(GtkWidget *widget, GdkEventButton *event) {
  ...
  switch (event->button) {
    case 1:
      ...
      g_signal_emit_by_name(GTK_BUTTON(widget), "button-release-event", &event,
                            &handled);
    ...
    case 3:
      if (GTK_IS_MENU(tile->context_menu))
        gtk_menu_popup_at_widget(GTK_MENU(tile->context_menu), widget,
...
static gboolean tile_key_release(GtkWidget *widget, GdkEventKey *event) {
  TileEvent *tile_event;
...
static gboolean tile_popup_menu(GtkWidget *widget) {
  Tile *tile = TILE(widget);
...
static void tile_drag_begin(GtkWidget *widget, GdkDragContext *context) {
  TilePrivate *priv;
...
static void tile_drag_data_get(GtkWidget *widget, GdkDragContext *context,
...
static void tile_tile_action_triggered(Tile *tile, TileEvent *event,
                                       TileAction *action) {
  if (action && action->func) (*action->func)(tile, event, action);
}
...
void tile_trigger_action_with_time(Tile *tile, TileAction *action,
  ...
  g_signal_emit(tile, tile_signals[TILE_ACTION_TRIGGERED_SIGNAL], 0, event,
                action);
  g_free(event);
}

static void tile_action_triggered_event_marshal(GClosure *closure,
                                                GValue *retval, guint n_param,
                                                const GValue *param,
  ...
  callback(data_0, g_value_peek_pointer(param + 1),
