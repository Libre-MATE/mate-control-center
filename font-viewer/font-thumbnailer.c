...
static const gchar *get_ft_error(FT_Error error) {
...
int main(int argc, char **argv) {
  ...
  if (thumbstr_utf8 != NULL) default_thumbstr = FALSE;
  ...
  if (fragment) face_index = strtol(fragment + 1, NULL, 0);
  ...
  face =
      sushi_new_ft_face_from_uri(library, uri, face_index, &contents, &gerror);
  if (gerror) {
  ...
  surface =
      cairo_image_surface_create(CAIRO_FORMAT_ARGB32, thumb_size, thumb_size);
  ...
  if ((text_extents.width) > (thumb_size - 2 * PADDING_HORIZONTAL)) {
    scale_x =
        (gdouble)(thumb_size - 2 * PADDING_HORIZONTAL) / (text_extents.width);
  } else {
    scale_x = 1.0;
  }
  ...
  if ((text_extents.height) > (thumb_size - 2 * PADDING_VERTICAL)) {
    scale_y =
        (gdouble)(thumb_size - 2 * PADDING_VERTICAL) / (text_extents.height);
  } else {
    scale_y = 1.0;
  }
  ...
  cairo_translate(cr,
                  PADDING_HORIZONTAL - text_extents.x_bearing +
                      (thumb_size - scale * text_extents.width) / 2.0,
                  PADDING_VERTICAL - text_extents.y_bearing +
                      (thumb_size - scale * text_extents.height) / 2.0);
