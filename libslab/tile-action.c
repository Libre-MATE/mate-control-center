...
static void tile_action_init(TileAction *this) {
  this->tile = NULL;
  this->func = 0;
  this->menu_item = NULL;
  this->flags = 0;
...
static void tile_action_finalize(GObject *g_object) {
  TileAction *action = TILE_ACTION(g_object);
  if (action->menu_item) gtk_widget_destroy(GTK_WIDGET(action->menu_item));
...
void tile_action_set_menu_item_label(TileAction *this, const gchar *markup) {
  GtkWidget *label;
...
GtkMenuItem *tile_action_get_menu_item(TileAction *this) {
  return this->menu_item;
}

static void tile_action_menu_item_activate_cb(GtkMenuItem *menu_item,
                                              gpointer user_data) {
