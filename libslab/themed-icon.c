...
static void themed_icon_set_property(GObject *, guint, const GValue *,
                                     GParamSpec *);

static void themed_icon_show(GtkWidget *);
static void themed_icon_style_updated(GtkWidget *);

enum { PROP_0, PROP_ICON_ID, PROP_ICON_SIZE };
...
static void themed_icon_class_init(ThemedIconClass *themed_icon_class) {
  ...
  g_object_class_install_property(
      g_obj_class, PROP_ICON_ID,
      g_param_spec_string("icon-id", "icon-id", "the identifier of the icon",
                          NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      g_obj_class, PROP_ICON_SIZE,
      g_param_spec_enum("icon-size", "icon-size", "the size of the icon",
                        GTK_TYPE_ICON_SIZE, GTK_ICON_SIZE_BUTTON,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void themed_icon_init(ThemedIcon *icon) {
...
GtkWidget *themed_icon_new(const gchar *id, GtkIconSize size) {
  GtkWidget *icon = GTK_WIDGET(
      g_object_new(THEMED_ICON_TYPE, "icon-id", id, "icon-size", size, NULL));

  return icon;
}

static void themed_icon_finalize(GObject *object) {
  ThemedIcon *icon = THEMED_ICON(object);
  if (icon->id) g_free(icon->id);
  (*G_OBJECT_CLASS(themed_icon_parent_class)->finalize)(object);
}

static void themed_icon_get_property(GObject *g_obj, guint prop_id,
...
static void themed_icon_set_property(GObject *g_obj, guint prop_id,
                                     const GValue *value,
  ...
  switch (prop_id) {
    case PROP_ICON_ID:
      ...
      /*			gtk_image_load_by_id (GTK_IMAGE (icon),
       * icon->size, icon->id); */
...
static void themed_icon_show(GtkWidget *widget) {
...
static void themed_icon_style_updated(GtkWidget *widget) {
  ThemedIcon *icon = THEMED_ICON(widget);
