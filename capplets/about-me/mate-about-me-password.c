...
/* Buffer size for backend output (64 is too small to store message from
 * libpwquality)*/
#define BUFSIZE 128
 ...
 * Prototypes {{
 */
static void stop_passwd(PasswordDialog *pdialog);

static void free_passwd_resources(PasswordDialog *pdialog);

static gboolean io_watch_stdout(GIOChannel *source, GIOCondition condition,
                                PasswordDialog *pdialog);

static void passdlg_set_auth_state(PasswordDialog *pdialog, gboolean state);

static void passdlg_set_status(PasswordDialog *pdialog, gchar *msg);

static void passdlg_set_busy(PasswordDialog *pdialog, gboolean busy);

static void passdlg_clear(PasswordDialog *pdialog);

static guint passdlg_refresh_password_state(PasswordDialog *pdialog);
...
/* Child watcher */
static void child_watch_cb(GPid pid, gint status, PasswordDialog *pdialog) {
  if (WIFEXITED(status)) {
...
static gboolean spawn_passwd(PasswordDialog *pdialog, GError **error) {
  gchar *argv[2];
  gchar *envp[1];
  ...
  if (g_io_channel_set_encoding(pdialog->backend_stdin, NULL, error) !=
          G_IO_STATUS_NORMAL ||
      g_io_channel_set_encoding(pdialog->backend_stdout, NULL, error) !=
          G_IO_STATUS_NORMAL ||
      g_io_channel_set_encoding(pdialog->backend_stderr, NULL, error) !=
          G_IO_STATUS_NORMAL ||
      g_io_channel_set_flags(pdialog->backend_stdin, G_IO_FLAG_NONBLOCK,
                             error) != G_IO_STATUS_NORMAL ||
      g_io_channel_set_flags(pdialog->backend_stdout, G_IO_FLAG_NONBLOCK,
                             error) != G_IO_STATUS_NORMAL ||
      g_io_channel_set_flags(pdialog->backend_stderr, G_IO_FLAG_NONBLOCK,
                             error) != G_IO_STATUS_NORMAL) {
    /* Clean up */
    stop_passwd(pdialog);
    return FALSE;
  ...
  pdialog->backend_stdout_watch_id =
      g_io_add_watch(pdialog->backend_stdout, G_IO_IN | G_IO_PRI,
                     (GIOFunc)io_watch_stdout, pdialog);
  pdialog->backend_stderr_watch_id =
      g_io_add_watch(pdialog->backend_stderr, G_IO_IN | G_IO_PRI,
                     (GIOFunc)io_watch_stdout, pdialog);

  /* Add child watcher */
  pdialog->backend_child_watch_id = g_child_watch_add(
      pdialog->backend_pid, (GChildWatchFunc)child_watch_cb, pdialog);
...
static void free_passwd_resources(PasswordDialog *pdialog) {
  ...
  if (pdialog->backend_stdin != NULL) {
    if (g_io_channel_shutdown(pdialog->backend_stdin, TRUE, &error) !=
        G_IO_STATUS_NORMAL) {
      g_warning(_("Could not shutdown backend_stdin IO channel: %s"),
                error->message);
      g_error_free(error);
      error = NULL;
    }
  ...
  if (pdialog->backend_stdout != NULL) {
    if (g_io_channel_shutdown(pdialog->backend_stdout, TRUE, &error) !=
        G_IO_STATUS_NORMAL) {
      g_warning(_("Could not shutdown backend_stdout IO channel: %s"),
                error->message);
      g_error_free(error);
      error = NULL;
    }
  ...
  if (pdialog->backend_stderr != NULL) {
    if (g_io_channel_shutdown(pdialog->backend_stderr, TRUE, &error) !=
        G_IO_STATUS_NORMAL) {
      g_warning(_("Could not shutdown backend_stderr IO channel: %s"),
                error->message);
      g_error_free(error);
      error = NULL;
    }
...
static void authenticated_user(PasswordDialog *pdialog, gboolean retry) {
...
/*
 * IO watcher for stdout, called whenever there is data to read from the
 * backend. This is where most of the actual IO handling happens.
 */
static gboolean io_watch_stdout(GIOChannel *source, GIOCondition condition,
                                PasswordDialog *pdialog) {
  ...
  if (g_io_channel_read_chars(source, buf, BUFSIZE, &bytes_read, &error) !=
      G_IO_STATUS_NORMAL) {
    if (error) {
  ...
  switch (pdialog->backend_state) {
    case PASSWD_STATE_AUTH:
      ...
      if (is_string_complete(str->str, "assword: ", "failure", "wrong", "error",
                             NULL)) {
        ...
        if (g_strrstr(str->str, "New password: ") != NULL) {
          ...
          if (pdialog->authenticated) {
            ...
            passdlg_set_status(
                pdialog, _("Your password has been changed since you initially "
    ...
    case PASSWD_STATE_RETYPE:
      ...
      if (is_string_complete(
              str->str, "successfully", "short", "longer", "palindrome",
              "dictionary", "simpl", /* catches both simple and simplistic */
              "similar", "different", "case", "wrapped", "recovered", "recent",
              "unchanged", "match", "1 numeric or special", "failure",
              "rotated", "error", "BAD PASSWORD", NULL)) {
        ...
        if (g_strrstr(str->str, "successfully") != NULL) {
          ...
          if (g_strrstr(str->str, "recovered") != NULL) {
            ...
            msg =
                g_strdup(_("The new password must contain numeric or special "
                           "character(s)."));
            ...
            msg =
                g_strdup(_("The new password has already been used recently."));
          } else if (g_strrstr(str->str, "failure") != NULL) {
            /* Authentication failure */
            msg =
                g_strdup(_("Your password has been changed since you initially "
                           "authenticated! Please re-authenticate."));

            passdlg_set_auth_state(pdialog, FALSE);
          } else if (g_strrstr(str->str, "BAD PASSWORD") != NULL) {
            /* Actual error description from libpwquality is located in the
             * first string */
...
static void authenticate(PasswordDialog *pdialog) {
  gchar *s;
...
static void update_password(PasswordDialog *pdialog) {
  gchar *s;
...
static void passdlg_error_dialog(GtkWindow *parent, const gchar *title,
...
static void passdlg_set_auth_state(PasswordDialog *pdialog, gboolean state) {
  ...
  if (state) {
    ...
    /* Set open lock image */
    gtk_image_set_from_icon_name(GTK_IMAGE(pdialog->dialog_image),
                                 "changes-allow", GTK_ICON_SIZE_DIALOG);
  } else {
    /* Not authenticated state */
    ...
    gtk_image_set_from_icon_name(GTK_IMAGE(pdialog->dialog_image),
                                 "changes-prevent", GTK_ICON_SIZE_DIALOG);
  }
}
...
static gboolean passdlg_spawn_passwd(PasswordDialog *pdialog) {
  GError *error = NULL;
  gchar *details;
  ...
  if (!spawn_passwd(pdialog, &error)) {
    GtkWidget *parent =
        GTK_WIDGET(gtk_builder_get_object(pdialog->ui, "change-password"));

    /* translators: Unable to launch <program>: <error message> */
    details = g_strdup_printf(_("Unable to launch %s: %s"), "/usr/bin/passwd",
                              error->message);

    passdlg_error_dialog(GTK_WINDOW(parent), _("Unable to launch backend"),
                         _("A system error has occurred"), details);
...
static void passdlg_authenticate(GtkButton *button, PasswordDialog *pdialog) {
 ...
 * PASSDLG_ERROR_PASSWORDS_NOT_EQUAL
 */
static guint passdlg_validate_passwords(PasswordDialog *pdialog) {
...
static void passdlg_check_password(GtkEntry *entry, PasswordDialog *pdialog) {
  guint ret;
  ...
  switch (ret) {
    case PASSDLG_ERROR_NONE:
      passdlg_set_status(
          pdialog, _("Click <b>Change password</b> to change your password."));
      break;
    case PASSDLG_ERROR_NEW_PASSWORD_EMPTY:
      passdlg_set_status(
          pdialog,
          _("Please type your password in the <b>New password</b> field."));
      break;
    case PASSDLG_ERROR_RETYPED_PASSWORD_EMPTY:
      passdlg_set_status(pdialog, _("Please type your password again in the "
                                    "<b>Retype new password</b> field."));
      break;
...
static gboolean passdlg_process_response(PasswordDialog *pdialog,
                                         gint response_id) {
...
static void passdlg_activate(GtkEntry *entry, GtkWidget *w) {
  if (GTK_IS_BUTTON(w)) {
...
static void passdlg_init(PasswordDialog *pdialog, GtkWindow *parent) {
  GtkBuilder *dialog;
  GtkWidget *wpassdlg;
  GtkAccelGroup *group;
  GError *error = NULL;
  ...
  if (gtk_builder_add_from_resource(
          dialog, "/org/mate/mcc/am/mate-about-me-password.ui", &error) == 0) {
  ...
  pdialog->current_password =
      GTK_ENTRY(gtk_builder_get_object(dialog, "current-password"));
  pdialog->new_password =
      GTK_ENTRY(gtk_builder_get_object(dialog, "new-password"));
  pdialog->retyped_password =
      GTK_ENTRY(gtk_builder_get_object(dialog, "retyped-password"));
  pdialog->dialog_image =
      GTK_IMAGE(gtk_builder_get_object(dialog, "dialog-image"));
  pdialog->status_label =
      GTK_LABEL(gtk_builder_get_object(dialog, "status-label"));

  /* Initialize accelerators */
  gtk_widget_add_accelerator(GTK_WIDGET(pdialog->current_password), "activate",
                             group, GDK_KEY_Return, 0, 0);

  gtk_widget_add_accelerator(GTK_WIDGET(pdialog->new_password), "activate",
                             group, GDK_KEY_Return, 0, 0);
  ...
  g_signal_connect(gtk_builder_get_object(dialog, "authenticate-button"),
                   "clicked", G_CALLBACK(passdlg_authenticate), pdialog);
  ...
  g_signal_connect(gtk_builder_get_object(dialog, "retyped-password"),
                   "changed", G_CALLBACK(passdlg_check_password), pdialog);
